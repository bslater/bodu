// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ComplexJsonConverterFactory.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Creates <see cref="ComplexJsonConverter{T}" /> instances for closed <see cref="Complex{T}" /> types, applying a
/// configurable <see cref="NumericsJsonPolicy" /> to every closed converter the factory produces.
/// </summary>
/// <remarks>
/// The core <see cref="Complex{T}" /> type carries no <see cref="JsonConverterAttribute" /> so that
...
/// </remarks>
public sealed class ComplexJsonConverterFactory
    : JsonConverterFactory
{
    /// <summary>The policy passed to every <see cref="ComplexJsonConverter{T}" /> produced by this factory.</summary>
    private readonly NumericsJsonPolicy _policy;

    /// <summary>
    /// Initializes a new instance of the <see cref="ComplexJsonConverterFactory" /> class configured for the
    /// <see cref="NumericsJsonPolicy.Strict" /> shape. Invoked by <see cref="JsonConverterAttribute" />.
    /// </summary>
    public ComplexJsonConverterFactory()
        : this(NumericsJsonPolicy.Strict)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="ComplexJsonConverterFactory" /> class configured for the supplied
    ...
    /// </exception>
    public ComplexJsonConverterFactory(NumericsJsonPolicy policy)
    {
    ...
    /// <returns>
    /// <see langword="true" /> if <paramref name="typeToConvert" /> is a closed <see cref="Complex{T}" /> type;
    /// otherwise, <see langword="false" />.
    /// </returns>
    public override bool CanConvert(Type typeToConvert) =>
        typeToConvert is { IsGenericType: true }
        && typeToConvert.GetGenericTypeDefinition() == typeof(Complex<>);

    /// <summary>
    /// Creates a converter for the specified closed <see cref="Complex{T}" /> type.
    /// </summary>
    /// <param name="typeToConvert">The closed <see cref="Complex{T}" /> type to convert.</param>
    ...
    public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        Type converterType = typeof(ComplexJsonConverter<>).MakeGenericType(componentType);
