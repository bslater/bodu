...
/// <summary>
/// Compiles the Bodu.Numerics JSON documentation guide examples that are explicitly opted in, so a code sample cannot silently
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    [
        "docs/guides/numerics/json-serialization.md",
    ];
    ...
    /// <summary>
    /// Verifies that every opted-in Bodu.Numerics JSON guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void NumericsJsonGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
