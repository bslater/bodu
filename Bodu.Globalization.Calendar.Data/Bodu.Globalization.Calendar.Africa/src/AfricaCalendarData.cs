...
/// <remarks>
/// <para>
/// Each supported country is a self-contained embedded resource. A territory may be a country code (<c>ZA</c>) or a
/// subdivision; the subdivision selects the same country resource, and the resolver filters by the full territory at
/// query time. The packs draw on the Western and Orthodox Christian, civil, and Islamic catalogues, and the
/// North-African packs declare a Friday/Saturday working week.
/// </para>
/// <para>
...
///<![CDATA[
/// // Query South African public holidays for a year.
/// NotableDateService service = AfricaCalendarData.CreateService("ZA");
/// DateRange year = new(new DateOnly(2026, 1, 1), new DateOnly(2026, 12, 31));
/// IReadOnlyList<NotableDate> holidays = service.Resolve(year, "ZA");
///]]>
/// </code>
/// </example>
...
public static class AfricaCalendarData
{
    ...
    /// </summary>
    /// <param name="territory">A country code or subdivision (for example <c>ZA</c>).</param>
    ...
    public static NotableDateResource LoadResource(string territory)
    {
        ...
        using Stream stream = typeof(AfricaCalendarData).Assembly.GetManifestResourceStream(resourceName)
            ?? throw new ArgumentException(
                string.Format(CultureInfo.CurrentCulture, AfricaCalendarDataResourceStrings.Arg_Invalid_NoResourceForTerritory, territory),
                nameof(territory));
    ...
    private static string? ResolveResource(string resourceName) =>
        string.Equals(resourceName, "africa-common", StringComparison.OrdinalIgnoreCase)
            ? s_africaCommon.Value
            : CommonNotableDateResources.Resolve(resourceName);

    /// <summary>The lazily-read XML content of the embedded <c>africa-common</c> hub resource.</summary>
    private static readonly Lazy<string?> s_africaCommon = new(static () =>
    {
        using Stream? stream = typeof(AfricaCalendarData).Assembly
            .GetManifestResourceStream("Bodu.Globalization.Calendar.Resources.africa-common.xml");
        if (stream is null)
            return null;
