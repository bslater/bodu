...
/// <summary>
/// Compiles the recurrence documentation guide examples that are explicitly opted in, so a code sample cannot silently
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    /// <summary>
    /// Verifies that every opted-in recurrence guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void RecurrenceGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
