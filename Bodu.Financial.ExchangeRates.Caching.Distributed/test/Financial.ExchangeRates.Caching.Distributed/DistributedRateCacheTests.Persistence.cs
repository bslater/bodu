// ---------------------------------------------------------------------------------------------------------------
// <copyright file="DistributedRateCacheTests.Persistence.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies that <see cref="DistributedRateCache" /> persists its state to the backing
...
public sealed partial class DistributedRateCacheTests
{
    ...
    public void Store_WhenNoPrefixConfigured_ShouldWriteUnderProviderColonPairKey()
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        MemoryDistributedCache backingStore = CreateBackingStore();
        var cache = new DistributedRateCache(backingStore, new DistributedRateCacheOptions { Provider = Provider });
    ...
    public void Store_WhenPrefixConfigured_ShouldWriteUnderPrefixedKey()
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        MemoryDistributedCache backingStore = CreateBackingStore();
        var cache = new DistributedRateCache(backingStore, new DistributedRateCacheOptions { Provider = Provider, KeyPrefix = "fx:" });
    ...
    public void Store_WhenDifferentPairs_ShouldUseDistinctKeys()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now) }, Duration, now);
        cache.Store(other, new[] { new CachedRate(new DateOnly(2023, 1, 3), 1.1000m, now) }, Duration, now);
