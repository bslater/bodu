// ---------------------------------------------------------------------------------------------------------------
// <copyright file="DistributedRateCacheTests.RoundTrip.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies that <see cref="DistributedRateCache" /> round-trips decimals, dates, and instants losslessly
...
public sealed partial class DistributedRateCacheTests
{
    ...
    public void Store_WhenRateHasHighPrecision_ShouldRoundTripExactly()
    {
        ...
        DistributedRateCache writer = CreateCache(backingStore);
        writer.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), rate, now) }, Duration, now);

        DistributedRateCache reader = CreateCache(backingStore);
    ...
    public void Store_WhenRateHasTrailingZeros_ShouldPreserveScale()
    {
        ...
        DistributedRateCache reader = CreateCache(backingStore);
        CachedRate row = reader.GetRates(Pair, Duration, now)[0];
    ...
    public void Store_WhenDateIsFarFromPresent_ShouldRoundTripExactly()
    {
        ...
        DistributedRateCache writer = CreateCache(backingStore);
        writer.Store(Pair, new[] { new CachedRate(date, 0.42m, now) }, TimeSpan.FromDays(36500), now);

        DistributedRateCache reader = CreateCache(backingStore);
        IReadOnlyList<CachedRate> rows = reader.GetRates(Pair, TimeSpan.FromDays(36500), now);
    ...
    public void Store_WhenCachedInstantHasOffsetAndSubSeconds_ShouldRoundTripExactly()
    {
        ...
        DistributedRateCache writer = CreateCache(backingStore);
        writer.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5m, cachedAt) }, Duration, asOf);

        DistributedRateCache reader = CreateCache(backingStore);
        CachedRate row = reader.GetRates(Pair, Duration, asOf)[0];
    ...
    public void Store_WhenObservedAtUtcHasOffsetAndSubSeconds_ShouldRoundTripThroughBlob()
    {
        ...
        DistributedRateCache writer = CreateCache(backingStore);
        writer.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5m, now, observedAt) }, Duration, now);
    ...
    /// <see cref="CachedRate.ObservedAtUtc" /> through the JSON blob, so a missing instant survives as null.
    /// </summary>
    [TestMethod]
    public void Store_WhenObservedAtUtcNull_ShouldReadBackNullThroughBlob()
    {
        ...
        DistributedRateCache writer = CreateCache(backingStore);
        writer.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5m, now) }, Duration, now);
    ...
    public void Store_WhenObservedAtUtcNull_ShouldOmitPropertyFromBlob()
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        MemoryDistributedCache backingStore = CreateBackingStore();
        DistributedRateCache cache = CreateCache(backingStore);
