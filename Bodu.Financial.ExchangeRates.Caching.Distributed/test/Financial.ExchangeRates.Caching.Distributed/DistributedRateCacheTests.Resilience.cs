// ---------------------------------------------------------------------------------------------------------------
// <copyright file="DistributedRateCacheTests.Resilience.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies that <see cref="DistributedRateCache" /> degrades gracefully when the backing
/// <see cref="IDistributedCache" /> faults or holds a corrupt blob, surfacing an empty read or a skipped write rather
/// than an exception, as required by the <see cref="IRateCache" /> contract.
/// </summary>
public sealed partial class DistributedRateCacheTests
{
    ...
    {
        DistributedRateCache cache = CreateCache(new FailingDistributedCache());
