// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ImfFinancialServiceBuilderExtensions.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Provides the fluent registration of the IMF exchange-rate provider onto an <see cref="IFinancialServiceBuilder" />.
/// </summary>
public static class ImfFinancialServiceBuilderExtensions
{
    /// <summary>The name of the <see cref="HttpClient" /> configured for IMF requests.</summary>
    public const string HttpClientName = "Bodu.Financial.ExchangeRates.Imf";

    /// <summary>
    /// Registers the IMF exchange-rate provider, binding its options and configuring a named <see cref="HttpClient" />
    /// for its requests.
    /// </summary>
    ...
    /// bound into <see cref="ImfRateProviderOptions" />.
    /// </param>
    /// <param name="sectionName">The configuration section name. Defaults to <c>Financial:Imf</c>.</param>
    ...
    /// </example>
    /// </remarks>
    public static IFinancialServiceBuilder AddImfExchangeRates(
        this IFinancialServiceBuilder builder,
        IConfiguration? configuration = null,
        string sectionName = "Financial:Imf",
        Action<ImfRateProviderOptions>? configure = null,
        Action<HttpStandardResilienceOptions>? configureResilience = null)
        => builder.AddWebRateProvider<ImfRateProvider, ImfRateProviderOptions>(
            HttpClientName,
            configuration,
            sectionName,
            "IMF exchange-rate options are invalid.",
            configure,
            configureResilience,
            static (client, opts, loggerFactory, timeProvider) =>
                new ImfRateProvider(client, opts, loggerFactory?.CreateLogger<ImfRateProvider>(), timeProvider));
}
