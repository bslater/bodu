// ---------------------------------------------------------------------------------------------------------------
// <copyright file="FredFinancialServiceBuilderExtensions.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Provides the fluent registration of the FRED exchange-rate provider onto an <see cref="IFinancialServiceBuilder" />.
/// </summary>
public static class FredFinancialServiceBuilderExtensions
{
    /// <summary>The name of the <see cref="HttpClient" /> configured for FRED requests.</summary>
    public const string HttpClientName = "Bodu.Financial.ExchangeRates.Fred";

    /// <summary>
    /// Registers the FRED exchange-rate provider, binding its options and configuring a named <see cref="HttpClient" />
    /// for its requests.
    /// </summary>
    ...
    /// bound into <see cref="FredRateProviderOptions" />.
    /// </param>
    /// <param name="sectionName">The configuration section name. Defaults to <c>Financial:Fred</c>.</param>
    ...
    /// </example>
    /// </remarks>
    public static IFinancialServiceBuilder AddFredExchangeRates(
        this IFinancialServiceBuilder builder,
        IConfiguration? configuration = null,
        string sectionName = "Financial:Fred",
        Action<FredRateProviderOptions>? configure = null,
        Action<HttpStandardResilienceOptions>? configureResilience = null)
        => builder.AddWebRateProvider<FredRateProvider, FredRateProviderOptions>(
            HttpClientName,
            configuration,
            sectionName,
            "FRED exchange-rate options are invalid.",
            configure,
            configureResilience,
            static (client, opts, loggerFactory, timeProvider) =>
                new FredRateProvider(client, opts, loggerFactory?.CreateLogger<FredRateProvider>(), timeProvider));
}
