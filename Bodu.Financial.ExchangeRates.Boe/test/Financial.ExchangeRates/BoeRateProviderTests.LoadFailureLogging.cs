// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BoeRateProviderTests.LoadFailureLogging.cs" company="Bodu Pty. Ltd.">
...
public partial class BoeRateProviderTests
{
    ...
    private static (BoeRateProvider Provider, CapturingLogger Logger) CreateThrowing(Exception exception)
    {
        BoeRateProviderOptions options = new() { AllowSynchronousNetworkAccess = false, EnableDiskCache = false };
        CapturingLogger logger = new();

        return (new BoeRateProvider(new ThrowingBoeRateTableSource(exception), options, logger), logger);
    }
    ...
    {
        (BoeRateProvider provider, CapturingLogger logger) = CreateThrowing(new NullReferenceException());
    ...
    {
        (BoeRateProvider provider, CapturingLogger logger) = CreateThrowing(new HttpRequestException("boom"));
    ...
    {
        (BoeRateProvider provider, CapturingLogger logger) = CreateThrowing(new ExchangeRateFormatException("boom"));
    ...
    {
        (BoeRateProvider provider, CapturingLogger logger) = CreateThrowing(new OperationCanceledException());
