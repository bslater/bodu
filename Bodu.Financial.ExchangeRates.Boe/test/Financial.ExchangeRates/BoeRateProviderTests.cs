// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BoeRateProviderTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies the lookup behavior of <see cref="BoeRateProvider" /> using an offline fixture source.
/// </summary>
[TestClass]
public partial class BoeRateProviderTests
{
    ...
    private static (BoeRateProvider Provider, FixtureBoeRateTableSource Source) Create(bool allowSync = true)
    {
        BoeRateProviderOptions options = new() { AllowSynchronousNetworkAccess = allowSync, EnableDiskCache = false };
        FixtureBoeRateTableSource source = new(options);
        return (new BoeRateProvider(source, options), source);
    }
    ...
    private static async Task<BoeRateProvider> CreatePreloadedAsync()
    {
    ...
    {
        BoeRateProvider provider = await CreatePreloadedAsync();

        RateLookupResult result = provider.GetRate("GBP", "USD", new DateOnly(2023, 1, 3));

        Assert.AreEqual(1.2065m, result.Rate.Rate);
        Assert.AreEqual(BoeRateProvider.ProviderName, result.Rate.Provider);
