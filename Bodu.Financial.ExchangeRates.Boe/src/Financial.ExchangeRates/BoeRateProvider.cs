// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BoeRateProvider.cs" company="Bodu Pty. Ltd.">
...
/// this provider creates one. The IADB is queried by date range, so loading is range-based: each load fetches the
/// requested inclusive range and accumulates it. Use <see cref="LoadRangeAsync" /> to warm a range.
/// </para>
/// <para>
...
/// level is configurable through the corresponding <c>*LogLevel</c> property on <see cref="BoeRateProviderOptions" />;
...
/// </code>
/// </example>
public sealed class BoeRateProvider
    : WebRateProvider
{
    ...
    private readonly IBoeRateTableSource _source;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BoeRateProvider" /> class backed by an <see cref="HttpClient" />
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BoeRateProvider" /> class backed by the IADB CSV endpoint, queried
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BoeRateProvider" /> class backed by an explicit table source, used
    /// for testing.
    /// </summary>
    ...
    /// </exception>
    /// <exception cref="ArgumentException">Thrown when <paramref name="options" /> fails validation.</exception>
    internal BoeRateProvider(IBoeRateTableSource source, BoeRateProviderOptions options, ILogger? logger = null, TimeProvider? timeProvider = null)
        : this(source, options, ownedHttpClient: null, logger, timeProvider)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="BoeRateProvider" /> class from an owned client, building the table
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BoeRateProvider" /> class, the shared core all public and internal
    ...
    /// <param name="logger">The logger.</param>
    /// <param name="timeProvider">The time source.</param>
    private BoeRateProvider(
        IBoeRateTableSource source,
        BoeRateProviderOptions options,
    ...
    private static HttpClient CreateOwnedClient(BoeRateProviderOptions options)
    {
        ThrowHelper.ThrowIfNull(options);
        options.Validate();
