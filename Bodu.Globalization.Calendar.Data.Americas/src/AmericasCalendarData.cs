// ---------------------------------------------------------------------------------------------------------------
// <copyright file="AmericasCalendarData.cs" company="PlaceholderCompany">
...
/// <summary>
/// Provides factory entry points for the Americas region calendar data pack — embedded notable-date rules for the United States and
/// Canada — so consumers can compose a <see cref="NotableDateService" /> without knowing the underlying resource layout.
/// </summary>
/// <remarks>
/// <para>
/// The pack ships <c>region-us.xml</c> and <c>region-ca.xml</c> as embedded manifest resources under the historical
/// <c>Bodu.Globalization.Calendar.Resources.*</c> path so that <c>&lt;UseFrom resource="./global-all.xml"&gt;</c> directives inside
/// the region XML resolve against the main library's globals via the provider's assembly chain.
/// </para>
/// <para>
/// Use one of the pack-level entry points (for example <see cref="CreateUnitedStatesProvider" />) when you only need a single
...
{
    /// <summary>The logical resource name for the United States region payload.</summary>
    ...
    /// <summary>
    /// Creates an <see cref="INotableDateRuleProvider" /> for the United States region payload, configured with an assembly chain
    /// that resolves region-local resources from the pack and global cherry-pick targets from the main library.
    /// </summary>
    ...
    /// <summary>
    /// Creates an <see cref="INotableDateRuleProvider" /> for the Canada region payload, configured with an assembly chain that
    /// resolves region-local resources from the pack and global cherry-pick targets from the main library.
    /// </summary>
    /// <returns>A configured rule provider.</returns>
