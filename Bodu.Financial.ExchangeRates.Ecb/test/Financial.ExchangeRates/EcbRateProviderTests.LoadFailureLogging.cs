// ---------------------------------------------------------------------------------------------------------------
// <copyright file="EcbRateProviderTests.LoadFailureLogging.cs" company="Bodu Pty. Ltd.">
...
public partial class EcbRateProviderTests
{
    ...
    private const int FeedLoadUnexpectedErrorEventId = 4106;
    ...
    private static (EcbRateProvider Provider, CapturingLogger Logger) CreateThrowing(Exception exception)
    {
        EcbRateProviderOptions options = new() { AllowSynchronousNetworkAccess = false, EnableDiskCache = false };
        CapturingLogger logger = new();

        return (new EcbRateProvider(new ThrowingEcbRateTableSource(exception), options, logger), logger);
    }
    ...
    {
        (EcbRateProvider provider, CapturingLogger logger) = CreateThrowing(new HttpRequestException("boom"));
    ...
    {
        (EcbRateProvider provider, CapturingLogger logger) = CreateThrowing(new ExchangeRateFormatException("boom"));
    ...
    {
        (EcbRateProvider provider, CapturingLogger logger) = CreateThrowing(new OperationCanceledException());
