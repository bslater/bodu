// ---------------------------------------------------------------------------------------------------------------
// <copyright file="OandaResilienceRegistrationTests.NamedClient.cs" company="Bodu Pty. Ltd.">
...
public partial class OandaResilienceRegistrationTests
{
    /// <summary>
    ...
    public async Task NamedClient_WhenTransientFailuresThenSuccess_ShouldRetryAndSucceed()
    {
        ...
        services
            .AddFinancialService()
            .AddOandaExchangeRates(configureResilience: ConfigureFastRetry);
        services
            .AddHttpClient(OandaFinancialServiceBuilderExtensions.HttpClientName)
            .ConfigurePrimaryHttpMessageHandler(() => handler);

        using ServiceProvider provider = services.BuildServiceProvider();
        HttpClient client = provider
            .GetRequiredService<IHttpClientFactory>()
            .CreateClient(OandaFinancialServiceBuilderExtensions.HttpClientName);

        using HttpResponseMessage response = await client.GetAsync(new Uri("https://oanda.test/quote"));
    ...
    public async Task NamedClient_WhenFailuresAreSustained_ShouldOpenCircuit()
    {
        ...
        services
            .AddFinancialService()
            .AddOandaExchangeRates(configureResilience: ConfigureFastBreaker);
        services
            .AddHttpClient(OandaFinancialServiceBuilderExtensions.HttpClientName)
            .ConfigurePrimaryHttpMessageHandler(() => handler);
