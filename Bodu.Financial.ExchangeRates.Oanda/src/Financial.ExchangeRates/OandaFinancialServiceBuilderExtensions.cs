// ---------------------------------------------------------------------------------------------------------------
// <copyright file="OandaFinancialServiceBuilderExtensions.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Provides the fluent registration of the OANDA exchange-rate provider onto an <see cref="IFinancialServiceBuilder" />.
/// </summary>
public static class OandaFinancialServiceBuilderExtensions
{
    /// <summary>The name of the <see cref="HttpClient" /> configured for OANDA requests.</summary>
    public const string HttpClientName = "Bodu.Financial.ExchangeRates.Oanda";

    /// <summary>
    /// Registers the OANDA exchange-rate provider, binding its options and configuring a named
    /// <see cref="HttpClient" /> for history requests.
    /// </summary>
    ...
    ///<![CDATA[
    /// // Compose the OANDA provider onto an existing financial service builder, binding from
    /// // configuration and relaxing the resilience timeout for a slow downstream link.
    /// builder.AddOandaExchangeRates(
    ///     configuration,
    ///     configure: options => options.Price = "mid",
    ...
    public static IFinancialServiceBuilder AddOandaExchangeRates(
        ...
        => builder.AddWebRateProvider<OandaRateProvider, OandaRateProviderOptions>(
            ...
            configuration,
            sectionName,
            "OANDA exchange-rate options are invalid.",
            configure,
            configureResilience,
