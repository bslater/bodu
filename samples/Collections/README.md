...
| `Bodu.Collections.Samples.SpecializedStructures` | The `Bodu.Collections.Specialized` members — `BitSet` (bit addressing, the three disagreeing size properties, the scan primitives, and the bulk set algebra) and the RFC 6962 `Rfc6962MerkleTree` (roots and inclusion proofs with domain separation, the tree-size ambiguity closed by `BindRoot`/`VerifyInclusionBound`, append-only consistency proofs, and the blocked/streaming/parallel surface with `MerkleBlocks` and `VerifyBlockInclusion`) | `Bodu.Collections` |
