...
| `Bodu.Numerics.Samples.ComplexNumbers` | `Complex<T>` arithmetic, `Conjugate`/`Reciprocal`/`Magnitude`/`Phase`, polar construction, and the transcendental surface (`Sqrt` of a negative real, Euler's identity, `Log`, `Pow`, trig) — every result checked against `System.Numerics.Complex`; plus one generic algorithm run over `float`, `double`, and `Half` to show where the precision choice changes the answer | `Bodu.Numerics` |
