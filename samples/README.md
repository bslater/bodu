...
| Collections | [`samples/Collections/`](Collections/README.md) — the specialized collection catalogue (including the expiring cache, the dictionary decorators, and the two-key table), ranges/graphs/trees/Aho-Corasick, the probabilistic sketches (Bloom, count-min, HyperLogLog), and the specialized structures (`BitSet`, the RFC 6962 Merkle tree) |
