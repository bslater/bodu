...
/// <summary>
/// Converts a collection of <typeparamref name="TElement" /> to and from an array token, materializing the declared
...
internal sealed class CollectionConverter<TCollection, TElement>
    : FormatConverter<TCollection>
    where TCollection : class
{
    /// <summary>
    /// The converter for the element type.
    /// </summary>
    private readonly FormatConverter _elementConverter;
    ...
    /// </exception>
    public CollectionConverter(FormatConverter elementConverter, CollectionStrategy strategy)
    {
    ...
    /// <inheritdoc />
    public override TCollection Read(ISerializationReader reader, Type typeToConvert, FormatSerializerOptions options)
    {
        ThrowHelper.ThrowIfNull(reader);

        if (reader.TokenKind != SerializationValueKind.StartArray)
        {
            throw new FormatSerializationException(
                string.Format(CultureInfo.CurrentCulture, SerializationResourceStrings.Op_Invalid_ExpectedArray, reader.TokenKind),
                reader.Location);
        }

        List<TElement> items = [];
        while (reader.Read() && reader.TokenKind != SerializationValueKind.EndArray)
            items.Add((TElement)_elementConverter.ReadAsObject(reader, typeof(TElement), options) !);

        return Materialize(items);
    }

    /// <inheritdoc />
    public override void Write(ISerializationWriter writer, TCollection value, FormatSerializerOptions options)
    {
