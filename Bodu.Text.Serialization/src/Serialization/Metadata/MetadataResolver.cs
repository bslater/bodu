...
internal static class MetadataResolver
{
    ...
    internal static TypeMetadata Resolve(Type type, FormatSerializerOptions options)
    {
        List<Draft> drafts = [];
        ...
        foreach (PropertyInfo property in type.GetProperties(MemberFlags))
        {
            ...
            FormatIgnoreAttribute? ignore = property.GetCustomAttribute<FormatIgnoreAttribute>(inherit: true);
            if (ignore is not null && ignore.Condition == FormatIgnoreCondition.Always)
                continue;

            var wireName = property.GetCustomAttribute<FormatPropertyNameAttribute>(inherit: true)?.Name
                ?? options.PropertyNamingPolicy?.ConvertName(property.Name)
                ?? property.Name;

            FormatConverter converter = ResolvePropertyConverter(property, options);
            var order = property.GetCustomAttribute<FormatPropertyOrderAttribute>(inherit: true)?.Order ?? 0;
            FormatIgnoreCondition? conditional = ignore?.Condition;
    ...
    private static FormatConverter ResolvePropertyConverter(PropertyInfo property, FormatSerializerOptions options)
    {
        FormatConverterAttribute? attribute = property.GetCustomAttribute<FormatConverterAttribute>(inherit: true);
    ...
    private static ConstructorInfo? ChooseConstructor(Type type)
    {
        ...
        ConstructorInfo? attributed = Array.Find(constructors, static c => c.IsDefined(typeof(FormatConstructorAttribute), inherit: false));
    ...
    private sealed class Draft
    {
        ...
        internal Draft(
            ...
            FormatIgnoreCondition? conditionalIgnore,
            int order,
        ...
        internal FormatIgnoreCondition? ConditionalIgnore { get; }
