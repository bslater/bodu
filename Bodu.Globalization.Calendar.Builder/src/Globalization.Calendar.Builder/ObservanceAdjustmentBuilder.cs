// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ObservanceAdjustmentBuilder.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Provides a fluent interface for constructing an <see cref="ObservanceAdjustment" /> and its corresponding XML or
/// JSON representation.
/// </summary>
/// <remarks>
/// <para>
/// An <see cref="ObservanceAdjustmentBuilder" /> is obtained via
/// <see cref="NotableDateRuleBuilder.AddAdjustment(string, System.Action{ObservanceAdjustmentBuilder})" />. The builder
/// accumulates adjustment properties and produces a domain object, a schema-valid <c>&lt;Adjustment&gt;</c> XML
/// element, and a schema-valid <c>adjustment</c> JSON object when the enclosing rule is built.
/// </para>
/// <para>
/// Only <see cref="When(AdjustmentTrigger)" /> and <see cref="Action(AdjustmentAction)" /> are required; all other
/// properties are optional and default to the same values as their <see cref="ObservanceAdjustment" /> counterparts.
/// </para>
/// <para>
/// <see cref="AddHandlerParameter(string, string)" /> and <see cref="MaxAdjustmentReachDays(int)" /> populate
/// <see cref="ObservanceAdjustment.HandlerParameters" /> and <see cref="ObservanceAdjustment.MaxAdjustmentReachDays" />
/// respectively. Both fields are programmatic-only — they are not part of the <c>NotableDates.xsd</c> or
/// <c>NotableDates.schema.json</c> schemas, so values supplied here are honoured by <see cref="Build(string)" /> but
...
{
    /// <summary>
    /// The activation condition set via <see cref="When(AdjustmentTrigger)" />, or <see langword="null" /> until
    /// configured.
    /// </summary>
    private AdjustmentTrigger? _trigger;

    /// <summary>
    /// The modification action set via <see cref="Action(AdjustmentAction)" />, or <see langword="null" /> until
    /// configured.
    /// </summary>
    private AdjustmentAction? _action;

    /// <summary>
    /// The day-of-week qualifier set via <see cref="OnDayOfWeek(DayOfWeek)" />, consumed by
    /// <see cref="AdjustmentTrigger.IfDayOfWeek" />.
    /// </summary>
    private DayOfWeek? _dayOfWeek;

    /// <summary>
    /// The non-working override set via <see cref="NonWorking(bool)" />, or <see langword="null" /> to inherit the
    /// rule's value.
    /// </summary>
    private bool? _isNonWorkingDay;

    /// <summary>
    /// The signed day offset set via <see cref="OffsetDays(int)" />, consumed by
    /// <see cref="AdjustmentAction.AddDays" />. Defaults to <c>0</c>.
    /// </summary>
    private int _offsetDays;

    /// <summary>
    /// The territory scope set via <see cref="Territory(string)" />, or <see langword="null" /> when unscoped.
    /// </summary>
    private string? _territoryCode;

    /// <summary>
    /// The calendar scope set via <see cref="CalendarType(Type)" />, or <see langword="null" /> when unscoped.
    /// </summary>
    private Type? _calendarType;

    /// <summary>
    /// The inclusive earliest effective year set via <see cref="FromYear(int)" />, or <see langword="null" /> for no
    /// lower bound.
    /// </summary>
    private int? _effectiveFromYear;

    /// <summary>
    /// The inclusive latest effective year set via <see cref="ToYear(int)" />, or <see langword="null" /> for no upper
    /// bound.
    /// </summary>
    private int? _effectiveToYear;

    /// <summary>
    /// The month component of the comparison date set via <see cref="ComparisonDate(int, int)" />, paired with
    /// <see cref="_comparisonDay" />.
    /// </summary>
    private int? _comparisonMonth;

    /// <summary>
    /// The day component of the comparison date set via <see cref="ComparisonDate(int, int)" />, paired with
    /// <see cref="_comparisonMonth" />.
    /// </summary>
    private int? _comparisonDay;

    /// <summary>
    /// The ordinal occurrence set via <see cref="OrdinalOccurrence(WeekOfMonthOrdinal)" />, consumed by
    /// <see cref="AdjustmentTrigger.IfNthOccurrenceInMonth" />.
    /// </summary>
    private WeekOfMonthOrdinal? _weekOrdinal;

    /// <summary>
    /// The target rule name set via <see cref="Target(string)" />, consumed by
    /// <see cref="AdjustmentAction.ReplaceWithNamedDate" />.
    /// </summary>
    private string? _targetRuleName;

    /// <summary>
    /// The evaluation priority set via <see cref="Priority(int)" /> (lower wins). Defaults to <c>100</c> to mirror
    /// <see cref="ObservanceAdjustment.Priority" />.
    /// </summary>
    private int _priority = 100;

    /// <summary>
    /// The custom-handler registry key set via <see cref="HandlerKey(string)" />, consumed by
    /// <see cref="AdjustmentTrigger.Custom" />/<see cref="AdjustmentAction.Custom" />.
    /// </summary>
    private string? _handlerKey;

    /// <summary>
    /// The handler parameter accumulator populated by <see cref="AddHandlerParameter(string, string)" />, or
    /// <see langword="null" /> when none are authored.
    /// </summary>
    private Dictionary<string, string>? _handlerParameters;

    /// <summary>
    /// The symmetric reach envelope in days set via <see cref="MaxAdjustmentReachDays(int)" />, or
    /// <see langword="null" /> to use the action-specific default heuristic.
    /// </summary>
    ...
    /// </summary>
    /// <param name="value">
    /// <see langword="true" /> to mark the adjusted date as non-working; <see langword="false" /> otherwise.
    /// </param>
    /// <returns>This builder instance, for method chaining.</returns>
    ...
    /// The CLR <see cref="Type" /> of the calendar (must derive from <see cref="System.Globalization.Calendar" />).
    ...
    /// <see cref="AdjustmentTrigger.IfAfterFixedDate" />. Only the month and day are significant; the year is replaced
    /// at resolution time.
    /// </summary>
    ...
    /// </summary>
    /// <param name="ordinal">
    /// The week-of-month ordinal (e.g. <see cref="WeekOfMonthOrdinal.Second" /> for the second occurrence).
    /// </param>
    /// <returns>This builder instance, for method chaining.</returns>
    ...
    /// <summary>
    /// Sets the name of another <see cref="NotableDateRule" /> referenced by
    /// <see cref="AdjustmentAction.ReplaceWithNamedDate" />.
    /// </summary>
    /// <param name="ruleName">
    /// The canonical name of the target rule. Must not be <see langword="null" /> or whitespace.
    /// </param>
    /// <returns>This builder instance, for method chaining.</returns>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="ruleName" /> is <see langword="null" />, empty, or whitespace.
    /// </exception>
    ...
    /// <param name="value">
    /// The parameter value. Must not be <see langword="null" />; an empty string is permitted.
    /// </param>
    /// <returns>This builder instance, for method chaining.</returns>
    /// <remarks>
    /// <para>
    ...
    /// </summary>
    /// <returns>
    /// A new <see cref="ObservanceAdjustmentBuilder" /> with the same configuration as this instance.
    /// </returns>
    /// <remarks>
    /// <para>
    /// Scalar fields are copied by value. The handler-parameter dictionary, when authored, is cloned so that later
    /// additions to either builder do not bleed across the boundary.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <param name="key">
    /// The adjustment key used for inheritance merging. Must not be <see langword="null" /> or whitespace.
    /// </param>
    ...
    /// Thrown when <see cref="When(AdjustmentTrigger)" /> or <see cref="Action(AdjustmentAction)" /> has not been
    /// called.
    /// </exception>
    ...
    /// <param name="key">The adjustment key. Must not be <see langword="null" /> or whitespace.</param>
    /// <returns>
    /// The constructed <see cref="JsonObject" /> conforming to the adjustment entry of <c>NotableDates.schema.json</c>.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown when <see cref="When(AdjustmentTrigger)" /> or <see cref="Action(AdjustmentAction)" /> has not been
    /// called.
