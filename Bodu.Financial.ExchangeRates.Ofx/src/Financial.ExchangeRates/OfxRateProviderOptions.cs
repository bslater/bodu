    ...
    : WebRateProviderOptions
{
    ...
    /// against an endpoint clock that trails UTC.
    /// </summary>
    /// <remarks>
    ...
    /// <see cref="TimeSpan.Zero" /> keeps maximum recency, and a small positive value compensates for an endpoint clock
    /// observed to sit behind UTC.
    /// </remarks>
    ...
    protected override bool TryValidateCore(out string? error)
    {
        ...
        // A negative margin would push the cap past the current instant, reintroducing the future ToDate the cap exists
        // to prevent.
