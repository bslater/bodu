...
public readonly struct DotEnvParseOptions
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DotEnvParseOptions" /> struct with all properties set to
    /// their defaults.
    /// </summary>
    ...
    /// <see langword="true" />, the literal word <c>export</c> followed by one or more spaces is stripped before
    /// the key is parsed. Only one level of <c>export</c> is consumed per line.
    /// </summary>
    /// <returns><see langword="true" /> if <c>export </c> lines are accepted; otherwise, <see langword="false" />. The default is <see langword="true" />.</returns>
    public bool AllowExportPrefix { get; init; } = true;

    /// <summary>
    /// Gets a value indicating whether inline comments are recognised in unquoted values. When
    /// <see langword="true" />, a <c>#</c> character that is preceded by at least one whitespace character
    /// truncates an unquoted value at that point; trailing whitespace is then also trimmed.
    /// </summary>
