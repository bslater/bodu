...
public enum IniDuplicateSectionBehavior
{
    ...
    /// <summary>
    /// Retain duplicate sections as separate entries in source order so that downstream consumers can apply
    /// their own precedence rules (for example, EditorConfig's last-section-wins).
    /// </summary>
    Preserve = 2,

    /// <summary>
    /// Merge a duplicate section into the immediately preceding section only when the two share the same name
    /// and no other section appears between them; otherwise, retain the section as a separate occurrence.
    /// </summary>
    MergeAdjacent = 3,

    /// <summary>
    /// Explicit alias for <see cref="Merge" />: merge every duplicate occurrence into the first regardless of
    /// position. Provided so callers selecting from a finer-grained set of behaviours can opt in by name.
    /// </summary>
    MergeAll = Merge,
}
