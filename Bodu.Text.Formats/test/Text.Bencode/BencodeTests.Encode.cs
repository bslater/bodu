// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BencodeTests.Format.cs" company="Bodu Pty. Ltd.">
...
public sealed partial class BencodeTests
{
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> produces the canonical encoded form of an
    /// integer value.
    /// </summary>
    [TestMethod]
    public void Encode_WhenBencodedInteger_ShouldReturnCanonicalEncoding()
    {
        var actual = Bencode.Format(new BencodedInteger(42));

        CollectionAssert.AreEqual(CanonicalIntegerBytes, actual);
    }

    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> produces the canonical encoded form of a UTF-8
    /// string.
    /// </summary>
    [TestMethod]
    public void Encode_WhenBencodedString_ShouldReturnCanonicalEncoding()
    {
        var actual = Bencode.Format(BencodedString.FromUtf8("spam"));

        CollectionAssert.AreEqual(CanonicalSpamBytes, actual);
    }

    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> emits dictionary keys in canonical raw byte
    ...
    public void Encode_WhenDictionaryWithUnsortedKeysOnConstruction_ShouldEmitSortedKeys()
    {
        ...
        var actual = Bencode.Format(dict);

        CollectionAssert.AreEqual(Bytes("d3:cow3:moo4:spam4:eggse"), actual);
    }

    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> emits <c>de</c> for an empty dictionary.
    /// </summary>
    [TestMethod]
    public void Encode_WhenEmptyBencodedDictionary_ShouldReturnEmptyDictionaryEncoding()
    {
        var actual = Bencode.Format(new BencodedDictionary(Array.Empty<KeyValuePair<BencodedString, BencodedValue>>()));

        CollectionAssert.AreEqual(CanonicalEmptyDictBytes, actual);
    }

    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> emits <c>le</c> for an empty list.
    /// </summary>
    [TestMethod]
    public void Encode_WhenEmptyBencodedList_ShouldReturnEmptyListEncoding()
    {
        var actual = Bencode.Format(new BencodedList(Array.Empty<BencodedValue>()));

        CollectionAssert.AreEqual(CanonicalEmptyListBytes, actual);
    }

    /// <summary>
    /// Verifies that <see cref="Bencode.Format(BencodedValue)" /> emits the canonical form of a nested list of
    ...
    public void Encode_WhenListWithStringAndInteger_ShouldReturnCanonicalEncoding()
    {
        ...
        var actual = Bencode.Format(list);
