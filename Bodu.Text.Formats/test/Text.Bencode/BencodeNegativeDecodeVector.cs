...
/// <summary>
/// Represents a negative Known Answer Test vector — a malformed bencoded input that the decoder must reject with a
/// specific exception type. Negative vectors capture the contract that <see cref="Bencode" /> makes about which inputs
/// are invalid: leading zeros, missing separators, unsorted dictionary keys, etc.
/// </summary>
...
/// Optional name of the <see cref="FormatsResourceStrings" /> resource that backs the expected exception message, for
/// tests that want to assert message stability.
/// </param>
    ...
    string? Source = null)
{
    ...
    /// <summary>
    /// Builds a negative vector from an ASCII string input expected to fail with <see cref="BencodeFormatException" />.
    /// </summary>
    /// <param name="description">A short label.</param>
    /// <param name="ascii">The malformed ASCII input.</param>
    /// <param name="expectedResourceKey">
    /// The resource key whose value should appear in the thrown exception message.
    /// </param>
    ...
    /// <summary>
    /// Builds a negative vector from hex-encoded bytes — useful for malformed inputs that contain non-printable bytes.
    /// Reuses <see cref="Base16.Decode(string)" />.
    /// </summary>
    /// <param name="description">A short label.</param>
