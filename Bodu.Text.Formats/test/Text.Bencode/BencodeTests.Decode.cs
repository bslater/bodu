// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BencodeTests.Parse.cs" company="Bodu Pty. Ltd.">
...
public sealed partial class BencodeTests
{
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> on empty input throws
    ...
    public void Decode_WhenEmptyInput_ShouldThrowExactly()
    {
        ...
        {
            _ = Bencode.Parse(ReadOnlySpan<byte>.Empty);
        });
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> decodes <c>i42e</c> into a
    /// <see cref="BencodedInteger" /> with value 42.
    /// </summary>
    [TestMethod]
    public void Decode_WhenSpanForCanonicalInteger_ShouldReturnBencodedInteger()
    {
        BencodedValue value = Bencode.Parse(CanonicalIntegerBytes);

        Assert.IsInstanceOfType<BencodedInteger>(value);
        Assert.AreEqual(42, ((BencodedInteger)value).Value);
    }
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> decodes the canonical BEP 3 string example
    ...
    {
        BencodedValue value = Bencode.Parse(CanonicalSpamBytes);
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> decodes <c>de</c> into an empty
    /// <see cref="BencodedDictionary" />.
    /// </summary>
    [TestMethod]
    public void Decode_WhenSpanForEmptyDictionary_ShouldReturnEmptyBencodedDictionary()
    {
        BencodedValue value = Bencode.Parse(CanonicalEmptyDictBytes);
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> decodes <c>le</c> into an empty
    /// <see cref="BencodedList" />.
    /// </summary>
    [TestMethod]
    public void Decode_WhenSpanForEmptyList_ShouldReturnEmptyBencodedList()
    {
        BencodedValue value = Bencode.Parse(CanonicalEmptyListBytes);
    ...
    /// <summary>
    /// Verifies that <see cref="Bencode.Parse(ReadOnlySpan{byte})" /> handles a nested list containing a string
    /// and an integer.
    /// </summary>
    [TestMethod]
    public void Decode_WhenSpanForNestedListOfStringAndInteger_ShouldReturnExpectedValues()
    {
        BencodedValue value = Bencode.Parse(Bytes("l4:spami42ee"));
    ...
    public void Decode_WhenTrailingData_ShouldThrowExactly()
    {
        BencodeFormatException ex = Assert.ThrowsExactly<BencodeFormatException>(() =>
        {
            _ = Bencode.Parse(Bytes("i3e0:trailing"));
        });
