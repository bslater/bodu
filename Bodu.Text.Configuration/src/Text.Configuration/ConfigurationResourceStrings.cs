...
/// <summary>
/// Centralizes the diagnostic and exception message templates used by the configuration reader, writer, and resolver so
/// that wording and formatting remain consistent across the assembly.
/// </summary>
/// <remarks>
/// Messages are exposed as <see langword="const" /> strings rather than via a RESX-backed resource manager. The
/// configuration format does not currently require localized diagnostics; if localization is later added, callers can
/// replace this class without affecting public API. Key names follow the
/// <c>&lt;Group&gt;_&lt;ExceptionTypeShort&gt;_&lt;Condition&gt;</c> convention used by every RESX-backed counterpart
...
{
    /// <summary>
    /// An argument value contained an empty segment within a configuration key.
    /// </summary>
    internal const string Arg_Invalid_ConfigKeyEmptySegment = "A configuration key contains an empty segment.";

    /// <summary>
    /// An argument value contained an illegal control character in a configuration key.
    /// </summary>
    ...
    /// <summary>
    /// A configuration key contained an illegal character.
    /// </summary>
    internal const string Format_Invalid_InvalidKeyCharacter = "Configuration key contains the illegal character '{0}'.";

    /// <summary>
    /// A configuration property key ran past the configured maximum length.
    /// </summary>
    ...
    /// on a parsed-from-string document without supplying a path root.
    /// </summary>
