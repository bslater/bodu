...
/// <remarks>
/// <para>
/// Each <see cref="IniComment" /> records the prefix character that introduced the line (either <c>'#'</c> or
/// <c>';'</c>), the trimmed text, and the 1-based source line at which it appeared.
/// </para>
/// <para>
    ...
    : IEquatable<IniComment>
{
    ...
    public IniComment(char prefix, string text, int lineNumber = 0)
    {
        ...
        if (prefix is not '#' and not ';')
            throw new ArgumentException(ConfigurationResourceStrings.Arg_Invalid_IniCommentPrefix, nameof(prefix));
