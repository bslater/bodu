...
public abstract class IniDocumentBase
{
    ...
    /// taking ownership of both. Used by the configuration parser to avoid re-copying its working collections.
    /// </summary>
    ...
    protected IniDocumentBase(IniSection globalSection, IEnumerable<IniSection> sections, bool caseSensitiveSections)
    {
        ...
        if (globalSection.Name.Length != 0)
            throw new ArgumentException(ConfigurationResourceStrings.Arg_Invalid_IniGlobalSectionNotEmpty, nameof(globalSection));
        ...
        {
            if (section is null)
                throw new ArgumentException(ConfigurationResourceStrings.Arg_Invalid_IniSectionsContainsNull, nameof(sections));
    ...
    protected void AddSectionCore(IniSection section)
    {
        ...
        if (section.Name.Length == 0)
            throw new ArgumentException(ConfigurationResourceStrings.Arg_Invalid_IniSectionCannotAddGlobal, nameof(section));
    ...
    protected IniSection GetOrAddSectionCore(string name)
    {
        ThrowHelper.ThrowIfNull(name);
        if (name.Length == 0)
            throw new ArgumentException(ConfigurationResourceStrings.Arg_Invalid_IniSectionNameEmpty, nameof(name));
