...
/// dotted-to-colon key mapping) onto the underlying <see cref="IniDocument" />, <see cref="IniSection" />,
/// and <see cref="IniEntry" /> primitives.
/// </summary>
public static class BoduConfigurationExtensions
{
    ...
    /// path using the default Bodu resolve options.
    /// </summary>
    ...
    /// <summary>
    /// Projects the document into a resolved <see cref="BoduConfigurationView" /> for the supplied target
    /// path using the supplied options.
    /// </summary>
    /// <param name="document">The document to resolve.</param>
    /// <param name="targetPath">The path the resolved view is evaluated for, or <see langword="null" />.</param>
    ...
    /// <summary>
    /// Computes the colon-delimited configuration key for an entry's raw key using the supplied key options.
    /// Mirrors <see cref="BoduConfigurationKey.Parse(string, BoduConfigurationKeyOptions?)" />.
    /// </summary>
    ...
    /// <summary>
    /// Convenience alias for <see cref="IniDocument.GlobalSection" /> — returns the same instance under the
    /// "preamble" name familiar from EditorConfig terminology.
    /// </summary>
