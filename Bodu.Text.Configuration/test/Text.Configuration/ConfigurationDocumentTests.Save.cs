// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ConfigurationDocumentTests.Save.cs" company="PlaceholderCompany">
...
public partial class ConfigurationDocumentTests
{
    ...
    {
        IniDocument original = ConfigurationDocument.Parse(ConfigurationFixtures.Representative);
        var outPath = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName() + ".boduconfig");
        try
        {
            ConfigurationDocument.Save(original, outPath);

            IniDocument reloaded = ConfigurationDocument.Load(outPath);
