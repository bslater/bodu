// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BoduConfigurationKnownAnswerData.cs" company="PlaceholderCompany">
...
/// </para>
/// </remarks>
public static class BoduConfigurationKnownAnswerData
{
    /// <summary>
    /// Gets the full KAT catalogue.
    /// </summary>
    public static IReadOnlyList<BoduConfigurationKat> All { get; } = CreateAll();

    /// <summary>
    /// Gets parser and lexer KATs.
    /// </summary>
    public static IEnumerable<object[]> ParserData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.Parse)
    ...
    public static IEnumerable<object[]> KeyData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.KeyMapping)
           .Select(kat => new object[] { kat });
    ...
    public static IEnumerable<object[]> PatternData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.Pattern)
           .Select(kat => new object[] { kat });
    ...
    public static IEnumerable<object[]> ResolutionData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.Resolve)
           .Select(kat => new object[] { kat });
    ...
    public static IEnumerable<object[]> WriterData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.Write or BoduConfigurationKatKind.RoundTrip)
           .Select(kat => new object[] { kat });
    ...
    public static IEnumerable<object[]> TypedValueData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.TypedValue)
           .Select(kat => new object[] { kat });
    ...
    public static IEnumerable<object[]> BridgeData =>
        All.Where(kat => kat.Kind is BoduConfigurationKatKind.ConfigurationBridge)
           .Select(kat => new object[] { kat });

    private static IReadOnlyList<BoduConfigurationKat> CreateAll()
    {
        var items = new List<BoduConfigurationKat>();
        ...
        return new ReadOnlyCollection<BoduConfigurationKat>(items);
    }

    private static void AddParserKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.ParsePass(
            id: "PARSE-0001",
    ...
    private static void AddKeyKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.KeyPass("KEY-0001", "Single segment key is unchanged", "name", "name", ["name"]));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0002", "Dotted key maps to colon hierarchy", "application.name", "application:name", ["application", "name"]));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0003", "Deep dotted key maps to deep hierarchy", "logging.level.default", "logging:level:default", ["logging", "level", "default"]));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0004", "Underscore remains literal", "format.new_line.before_where_clause", "format:new_line:before_where_clause", ["format", "new_line", "before_where_clause"]));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0005", "Hyphen remains literal", "tool.my-option.enabled", "tool:my-option:enabled", ["tool", "my-option", "enabled"]));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0006", "Existing colon lookup normalizes to same logical key", "logging:level:default", "logging:level:default", ["logging", "level", "default"], mapping: "DotToColon"));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0007", "Identity mapping preserves dotted key", "logging.level.default", "logging.level.default", ["logging", "level", "default"], mapping: "Identity"));
        items.Add(BoduConfigurationKat.KeyPass("KEY-0008", "Case-sensitive option preserves casing", "Logging.Level.Default", "Logging:Level:Default", ["Logging", "Level", "Default"], caseSensitive: true));

        items.Add(BoduConfigurationKat.KeyFail("KEY-1001", "Null key is rejected", null, "ArgumentNullException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1002", "Empty key is rejected", "", "ArgumentException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1003", "Whitespace-only key is rejected", "   ", "ArgumentException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1004", "Leading dot creates empty segment and is rejected", ".logging", "ArgumentException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1005", "Trailing dot creates empty segment and is rejected", "logging.", "ArgumentException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1006", "Consecutive dots create empty segment and are rejected", "logging..level", "ArgumentException"));
        items.Add(BoduConfigurationKat.KeyFail("KEY-1007", "Control character in key is rejected", "logging..level", "ArgumentException"));
    }

    private static void AddPatternKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.PatternPass("PAT-0001", "Star matches filename in same directory", "*.cs", "Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0002", "Star without slash matches at any depth", "*.cs", "src/Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0003", "Star does not cross slash inside same pattern segment", "src/*.cs", "src/Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0004", "Star does not cross slash into nested directory", "src/*.cs", "src/Generated/Program.cs", false));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0005", "Double-star crosses directories", "src/**/*.cs", "src/Generated/Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0006", "Double-star can match zero directories", "src/**/*.cs", "src/Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0007", "Question matches one non-slash character", "file?.txt", "file1.txt", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0008", "Question does not match zero characters", "file?.txt", "file.txt", false));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0009", "Brace alternation matches first choice", "*.{json,yaml,yml}", "appsettings.json", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0010", "Brace alternation matches later choice", "*.{json,yaml,yml}", "workflow.yaml", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0011", "Brace alternation rejects non-choice", "*.{json,yaml,yml}", "Program.cs", false));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0012", "Escaped star is literal", @"file\*.txt", "file*.txt", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0013", "Escaped star does not behave as wildcard", @"file\*.txt", "file1.txt", false));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0014", "Character class matches included character", "file[123].txt", "file2.txt", true, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0015", "Character class rejects excluded character", "file[123].txt", "file4.txt", false, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0016", "Negated character class rejects listed character", "file[!123].txt", "file2.txt", false, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0017", "Negated character class accepts unlisted character", "file[!123].txt", "file4.txt", true, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0018", "Numeric range matches interior value", "file{1..10}.txt", "file7.txt", true, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0019", "Numeric range rejects outside value", "file{1..10}.txt", "file11.txt", false, stage: "P1"));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0020", "Backslash is normalized to forward slash before matching", "*.cs", @"src\Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0021", "Forward slash is canonical separator", "src/*.cs", "src/Program.cs", true));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0022", "Pattern with slash is anchored to path root", "src/*.cs", "other/src/Program.cs", false));
        items.Add(BoduConfigurationKat.PatternPass("PAT-0023", "Pattern without slash matches basename at any depth", "*.cs", "other/src/Program.cs", true));

        items.Add(BoduConfigurationKat.PatternFail("PAT-1001", "Unbalanced brace is rejected", "*.{cs,json", "BoduConfigurationParseException"));
        items.Add(BoduConfigurationKat.PatternFail("PAT-1002", "Unbalanced character class is rejected", "file[abc.txt", "BoduConfigurationParseException", stage: "P1"));
    }

    private static void AddResolutionKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.ResolvePass(
            id: "RES-0001",
        ...
        items.Add(BoduConfigurationKat.ResolveFail(
            id: "RES-1001",
    ...
    private static void AddWriterKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.WritePass(
            id: "WRITE-0001",
        ...
        items.Add(BoduConfigurationKat.RoundTripPass(
            id: "ROUND-0001",
    ...
    private static void AddTypedValueKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0001", "Boolean true parses", "feature.enabled", "true", "Boolean", true));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0002", "Boolean false parses", "feature.enabled", "false", "Boolean", false));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0003", "Boolean parsing is case-insensitive", "feature.enabled", "TrUe", "Boolean", true));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0004", "Int32 positive parses", "indent.size", "4", "Int32", 4));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0005", "Int32 negative parses", "offset", "-1", "Int32", -1));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0006", "Int64 parses large value", "max.bytes", "9223372036854775807", "Int64", 9223372036854775807L));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0007", "Enum parses named value", "severity", "Warning", "Enum", "Warning"));
        items.Add(BoduConfigurationKat.TypedPass("TYPE-0008", "Fallback returns only when key is missing", "missing.value", null, "Int32WithFallback", 42));

        items.Add(BoduConfigurationKat.TypedFail("TYPE-1001", "Malformed Int32 throws even with fallback", "indent.size", "four", "Int32WithFallback", "FormatException"));
        items.Add(BoduConfigurationKat.TypedFail("TYPE-1002", "Overflowing Int32 throws", "indent.size", "2147483648", "Int32", "FormatException"));
        items.Add(BoduConfigurationKat.TypedFail("TYPE-1003", "Malformed Boolean throws", "feature.enabled", "yes please", "Boolean", "FormatException"));
        items.Add(BoduConfigurationKat.TypedFail("TYPE-1004", "Malformed Enum throws", "severity", "Maybe", "Enum", "FormatException"));
        items.Add(BoduConfigurationKat.TypedFail("TYPE-1005", "Required missing key throws", "missing.value", null, "Int32", "KeyNotFoundException"));
    }

    private static void AddBridgeKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.BridgePass(
            id: "BRIDGE-0001",
        ...
        items.Add(BoduConfigurationKat.BridgeFail(
            id: "BRIDGE-1001",
    ...
    private static void AddFailureKats(List<BoduConfigurationKat> items)
    {
        items.Add(BoduConfigurationKat.ParseFail(
            ...
            source:
                ...
                indent.size 4
                """,
            expectedException: "BoduConfigurationParseException",
...
/// </summary>
public enum BoduConfigurationKatKind
{
...
/// </summary>
public enum BoduConfigurationKatOutcome
{
...
/// </summary>
public sealed record BoduConfigurationKat
{
    private BoduConfigurationKat(
        string id,
        string title,
        BoduConfigurationKatKind kind,
        BoduConfigurationKatOutcome outcome)
    {
    ...
    public BoduConfigurationKatKind Kind { get; }

    /// <summary>Gets the expected pass/fail outcome.</summary>
    public BoduConfigurationKatOutcome Outcome { get; }
    ...
    public static BoduConfigurationKat ParsePass(
        string id,
        string title,
        string source,
        ...
        new(id, title, BoduConfigurationKatKind.Parse, BoduConfigurationKatOutcome.Pass)
        {
            Source = source,
            ExpectedDocument = expected,
            Profile = profile,
    ...
    public static BoduConfigurationKat ParseFail(
        string id,
        string title,
        string? source,
        ...
        string? duplicateKeyMode = null,
        string? diagnosticMode = null) =>
        new(id, title, BoduConfigurationKatKind.Parse, BoduConfigurationKatOutcome.Fail)
        {
            Source = source,
    ...
    public static BoduConfigurationKat KeyPass(
        string id,
        string title,
        string rawKey,
        ...
        new(id, title, BoduConfigurationKatKind.KeyMapping, BoduConfigurationKatOutcome.Pass)
        {
            Key = rawKey,
    ...
    public static BoduConfigurationKat KeyFail(
        string id,
        string title,
        string? rawKey,
        string expectedException,
        string mapping = "DotToColon") =>
        new(id, title, BoduConfigurationKatKind.KeyMapping, BoduConfigurationKatOutcome.Fail)
        {
            Key = rawKey,
            ExpectedException = expectedException,
            Options = mapping,
    ...
    public static BoduConfigurationKat PatternPass(
        string id,
        string title,
        string pattern,
        string targetPath,
        ...
        string stage = "P0",
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.Pattern, BoduConfigurationKatOutcome.Pass)
        {
            Pattern = pattern,
    ...
    public static BoduConfigurationKat PatternFail(
        string id,
        string title,
        string pattern,
        string expectedException,
        string stage = "P0",
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.Pattern, BoduConfigurationKatOutcome.Fail)
        {
            Pattern = pattern,
            ExpectedException = expectedException,
            Stage = stage,
    ...
    public static BoduConfigurationKat ResolvePass(
        string id,
        string title,
        string targetPath,
        string source,
        ...
        new(id, title, BoduConfigurationKatKind.Resolve, BoduConfigurationKatOutcome.Pass)
        {
            TargetPath = targetPath,
            Source = source,
    ...
    public static BoduConfigurationKat ResolveFail(
        string id,
        string title,
        string? targetPath,
        string source,
        string expectedException,
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.Resolve, BoduConfigurationKatOutcome.Fail)
        {
            TargetPath = targetPath,
            Source = source,
            ExpectedException = expectedException,
    ...
    public static BoduConfigurationKat WritePass(
        string id,
        string title,
        string source,
        string expectedText,
        string profile = "Bodu",
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.Write, BoduConfigurationKatOutcome.Pass)
        {
            Source = source,
    ...
    public static BoduConfigurationKat RoundTripPass(
        string id,
        string title,
        string source,
        string profile = "Bodu",
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.RoundTrip, BoduConfigurationKatOutcome.Pass)
        {
            Source = source,
            ExpectedText = source,
            Profile = profile,
    ...
    public static BoduConfigurationKat TypedPass(
        string id,
        string title,
        string key,
        string? rawValue,
        ...
        new(id, title, BoduConfigurationKatKind.TypedValue, BoduConfigurationKatOutcome.Pass)
        {
            Key = key,
            RawValue = rawValue,
    ...
    public static BoduConfigurationKat TypedFail(
        string id,
        string title,
        string key,
        string? rawValue,
        string accessor,
        string expectedException) =>
        new(id, title, BoduConfigurationKatKind.TypedValue, BoduConfigurationKatOutcome.Fail)
        {
            Key = key,
            RawValue = rawValue,
            TypedAccessor = accessor,
    ...
    public static BoduConfigurationKat BridgePass(
        string id,
        string title,
        string? source,
        string? targetPath,
        IReadOnlyList<ExpectedValue> expectedValues,
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.ConfigurationBridge, BoduConfigurationKatOutcome.Pass)
        {
            Source = source,
            TargetPath = targetPath,
            ExpectedValues = expectedValues,
    ...
    public static BoduConfigurationKat BridgeFail(
        string id,
        string title,
        string expectedException,
        string? source = null,
        ...
        string? options = null) =>
        new(id, title, BoduConfigurationKatKind.ConfigurationBridge, BoduConfigurationKatOutcome.Fail)
        {
            Source = source,
            TargetPath = targetPath,
            ExpectedException = expectedException,
    ...
    char? InlineCommentPrefix)
{
    ...
    public static ExpectedProperty Of(
        ...
        new(key, value, configurationKey ?? key.Replace('.', ':'), leadingComments ?? [], inlineComment, inlineCommentPrefix);
}
