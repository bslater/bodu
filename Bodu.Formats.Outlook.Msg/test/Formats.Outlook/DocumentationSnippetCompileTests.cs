...
/// <summary>
/// Compiles the Outlook documentation guide examples that are explicitly opted in, so a code sample cannot silently
/// drift from the public API it documents.
/// </summary>
/// <remarks>
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    /// <summary>
    /// Verifies that every opted-in Outlook guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void OutlookGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
    ...
    /// Outlook guides directory beneath it.
    /// </summary>
    ...
    private static string? FindGuidesDirectory()
    {
        for (DirectoryInfo? dir = new(AppContext.BaseDirectory); dir is not null; dir = dir.Parent)
        {
            ...
            {
                string guides = Path.Combine(dir.FullName, "docs", "guides", "outlook");
    ...
    /// lines are dropped first — guide snippets often show them for context, but the wrapper method already imports
    /// the covered namespaces.
    /// </summary>
