...
// carry two rows per observance). Laylat al-Qadr is 1 Ramadan + 26 days, keyed by the Gregorian year the resulting
// date falls in. Every emitted row is independently cross-checked astronomically by
// tools/verify-islamic-observance-vectors.py; the CSV provenance headers record the derivations.
//
// Usage:
...
//     dotnet run --project tools/IslamicObservanceVectorGenerator -- hijri > \
//         Bodu.Globalization.Calendar/test/Globalization.Calendar/Fixtures/Vectors/TabularHijriObservances-1990-2039.csv

const int FirstGregorianYear = 1990;
const int LastGregorianYear = 2039;

bool tabular = args is ["hijri"];

// (observanceId, Hijri month, Hijri day, day offset) mirroring both catalogues: nine fixed anchors plus Laylat
// al-Qadr as the catalogues' OffsetFromRule (1 Ramadan + 26 days, i.e. the night of 27 Ramadan).
...
Calendar calendar = tabular ? new HijriCalendar() : new UmAlQuraCalendar();

if (tabular)
{
    Console.WriteLine("# Tabular-Hijri observance vectors for the bundled global-islamic catalogue.");
    ...
    Console.WriteLine("# Source: the base class library's arithmetic HijriCalendar at the default HijriAdjustment = 0");
    Console.WriteLine("#   (the conventional tabular reckoning; an arithmetic convention has no external gazette to");
    Console.WriteLine("#   reconcile against), projected by tools/IslamicObservanceVectorGenerator replicating the");
    Console.WriteLine("#   engine's calendar-year sweep; Laylat al-Qadr is 1 Ramadan + 26 days.");
    ...
    Console.WriteLine("#   2023-2025 rows in GlobalIslamicCatalogueKnownAnswerTests; see");
    Console.WriteLine("#   NotableDateCatalogueVerification.md for the measured gap distribution.");
    Console.WriteLine("# Regenerate: dotnet run --project tools/IslamicObservanceVectorGenerator -- hijri > <this file>");
...
for (int year = FirstGregorianYear; year <= LastGregorianYear; year++)
{
    ...
    {
        foreach (DateOnly date in Occurrences(month, day, offset, year))
...
List<DateOnly> Occurrences(int month, int day, int offset, int gregorianYear)
{
...
// Replicates CalendarSystems.ResolveCalendarYearSweep: project the fixed Hijri date through every Hijri year
