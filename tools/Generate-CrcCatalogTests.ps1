...
param(
    ...
    [string]$OutputPath = (Join-Path $PSScriptRoot '..' 'Bodu.IO.Hashing' 'test' 'IO.Hashing.Checksums' 'CrcTests.Catalog.cs'),
    [int]$MaxSize = 64
)
...
[void]$sb.AppendLine('// ---------------------------------------------------------------------------------------------------------------')
[void]$sb.AppendLine('// <copyright file="CrcTests.Catalog.cs" company="Bodu Pty. Ltd.">')
[void]$sb.AppendLine('//     Copyright (c) Bodu Pty. Ltd.. All rights reserved.')
...
[void]$sb.AppendLine('//     Source: Bodu.IO.Hashing/src/crc-specs.json (derived from https://reveng.sourceforge.io/crc-catalogue/all.htm).')
[void]$sb.AppendLine('// </auto-generated>')
[void]$sb.AppendLine()
...
foreach ($spec in $emitted) {
    ...
    [void]$sb.AppendLine("        yield return new object[] { ""$($spec.name)"", CrcStandards.$constName, $expected };")
}
[void]$sb.AppendLine('    }')
[void]$sb.AppendLine()
[void]$sb.AppendLine('    /// <summary>')
[void]$sb.AppendLine('    /// Gets the display name used by <see cref="DynamicDataAttribute" /> for a test case row.')
[void]$sb.AppendLine('    /// </summary>')
[void]$sb.AppendLine('    /// <param name="dataRow">')
[void]$sb.AppendLine('    /// The test case data row. The first element is expected to contain the human-readable standard name.')
[void]$sb.AppendLine('    /// </param>')
[void]$sb.AppendLine('    /// <returns>')
[void]$sb.AppendLine('    /// The standard name for the current test case, or <c>"Unknown Standard"</c> if the first element is not a <see cref="string" />.')
[void]$sb.AppendLine('    /// </returns>')
[void]$sb.AppendLine('    public static string GetCatalogCheckDisplayName(MethodInfo methodInfo, object[] data)')
[void]$sb.AppendLine('    {')
[void]$sb.AppendLine('        string name = (string)data[0];')
...
[void]$sb.AppendLine('    [DynamicData(nameof(CatalogCheckVectors), DynamicDataDisplayName = nameof(GetCatalogCheckDisplayName))]')
[void]$sb.AppendLine('    public void ComputeHash_WhenInputIsCheckVector_ForCatalogStandard_ShouldMatchPublishedCheck(string displayName, CrcStandards standardId, ulong expectedCheck)')
[void]$sb.AppendLine('    {')
[void]$sb.AppendLine('        CrcStandard standard = CrcStandard.Get(standardId);')
[void]$sb.AppendLine('        var crc = new Crc(standard);')
