    ...
    : NotableDateCacheBase<SqliteNotableDateCacheOptions>, IDisposable
{
    ...
    /// keep-alive connection. Clearing the pool is required because <c>Microsoft.Data.Sqlite</c> pools closed
    /// connections by default: without it, the pooled handles from per-operation connections (and the returned
    /// keep-alive) keep the database file open past disposal, which blocks deletion on Windows and leaves the
    /// write-ahead-log sidecar unpurged everywhere.
    /// </remarks>
    public void Dispose()
    {
