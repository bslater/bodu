...
/// <summary>
/// Compiles the delimited documentation guide examples that are explicitly opted in, so a code sample cannot silently
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    [
        "docs/guides/formats/delimited.md",
    ];
    ...
    /// <summary>
    /// Verifies that every opted-in delimited guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void DelimitedGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
