...
/// in the same low-cost niche and produce comparable distribution; the choice between them is usually driven by interop
/// with an existing system that standardizes on one.
/// </para>
/// <para>
    ...
    : NonCryptographicHashAlgorithm
{
    ...
    /// <summary>The constant initial value used to seed the running hash.</summary>
    private const uint Seed = 0xAAAAAAAAu;

    /// <summary>The running count of input bytes processed, whose parity selects the per-byte mixing pattern.</summary>
    private ulong _size;

    /// <summary>The running hash accumulator, updated as each input byte is mixed in and seeded from <see cref="Seed" />.</summary>
