// ---------------------------------------------------------------------------------------------------------------
// <copyright file="CrcStandard.cs" company="Bodu Pty. Ltd.">
...
/// <description>
/// The <see cref="CrcStandard(string, int, ulong, ulong, bool, bool, ulong)" /> constructor accepts custom parameters
/// for variants that are not in the RevEng catalogue. Width must lie within <see cref="MinSize" />..
/// <see cref="MaxSize" />.
/// </description>
/// </item>
/// </list>
...
/// <see cref="ReflectIn" /> to fetch (or build) a precomputed lookup table from <see cref="CrcLookupTableCache" />,
/// seeds the running register with <see cref="InitialValue" />, and applies <see cref="ReflectOut" /> +
/// <see cref="XOrOut" /> when emitting the final digest. Multiple <see cref="Crc" /> instances configured with the same
/// <see cref="CrcStandard" /> share the same lookup table, so creating fresh engines for the same standard is
/// inexpensive.
/// </para>
/// <para>
      ...
      System.IEquatable<CrcStandard>
{
    ...
    /// <summary>
    /// Gets the <c>CRC-16/ARC</c> CRC standard (aliases include <c>CRC-16</c>, <c>CRC-IBM</c>, <c>CRC-16/LHA</c>). The
    /// de-facto generic 16-bit CRC.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC16_ARC" />
    public static CrcStandard CRC16_ARC => Get(CrcStandards.CRC16_ARC);

    /// <summary>
    /// Gets the <c>CRC-16/IBM-3740</c> CRC standard (aliases <c>CRC-16/AUTOSAR</c>, <c>CRC-16/CCITT-FALSE</c>). The
    /// widely-used "CCITT-false" variant.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC16_IBM3740" />
    public static CrcStandard CRC16_IBM3740 => Get(CrcStandards.CRC16_IBM3740);

    /// <summary>
    /// Gets the <c>CRC-16/KERMIT</c> CRC standard (aliases <c>CRC-16/BLUETOOTH</c>, <c>CRC-16/CCITT</c>,
    /// <c>CRC-CCITT</c>). Used by Kermit and Bluetooth.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC16_KERMIT" />
    public static CrcStandard CRC16_KERMIT => Get(CrcStandards.CRC16_KERMIT);

    /// <summary>
    /// Gets the <c>CRC-16/MODBUS</c> CRC standard (alias <c>MODBUS</c>). Used by Modbus RTU over serial.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC16_MODBUS" />
    public static CrcStandard CRC16_MODBUS => Get(CrcStandards.CRC16_MODBUS);

    /// <summary>
    /// Gets the <c>CRC-16/XMODEM</c> CRC standard (aliases <c>CRC-16/ACORN</c>, <c>XMODEM</c>, <c>ZMODEM</c>). Used by
    /// XMODEM and ZMODEM.
    /// </summary>
    ...
    /// <summary>
    /// Gets the <c>CRC-32/BZIP2</c> CRC standard (aliases <c>CRC-32/AAL5</c>, <c>CRC-32/DECT-B</c>, <c>B-CRC-32</c>).
    /// Used by bzip2 and AAL5.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC32_BZIP2" />
    public static CrcStandard CRC32_BZIP2 => Get(CrcStandards.CRC32_BZIP2);

    /// <summary>
    /// Gets the <c>CRC-32/ISCSI</c> CRC standard (aliases <c>CRC-32C</c>, <c>CRC-32/CASTAGNOLI</c>, <c>CRC-32/NVME</c>,
    /// <c>CRC-32/BASE91-C</c>, <c>CRC-32/INTERLAKEN</c>). Castagnoli polynomial used by iSCSI, SCTP, Btrfs, ext4, NVMe,
    ...
    /// <summary>
    /// Gets the <c>CRC-32/ISO-HDLC</c> CRC standard (aliases <c>CRC-32</c>, <c>PKZIP</c>, <c>CRC-32/XZ</c>,
    /// <c>CRC-32/ADCCP</c>, <c>CRC-32/V-42</c>). The canonical CRC-32 used by zlib, PNG, Ethernet, PKZIP, and the
    /// default standard used by <see cref="Crc.Crc()" />.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Width 32, polynomial <c>0x04C11DB7</c>, initial value <c>0xFFFFFFFF</c>, reflected input and output, final XOR
    /// <c>0xFFFFFFFF</c>.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Gets the <c>CRC-64/ECMA-182</c> CRC standard (alias <c>CRC-64</c>). Specified by ECMA-182 for DLT tape formats.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC64_ECMA182" />
    public static CrcStandard CRC64_ECMA182 => Get(CrcStandards.CRC64_ECMA182);

    /// <summary>
    /// Gets the <c>CRC-64/XZ</c> CRC standard (alias <c>CRC-64/GO-ECMA</c>). Used by the XZ compressed file format and
    /// the Go <c>crc64</c> ECMA table.
    /// </summary>
    /// <seealso cref="CrcStandards.CRC64_XZ" />
    public static CrcStandard CRC64_XZ => Get(CrcStandards.CRC64_XZ);

    /// <summary>
    /// Gets the <c>CRC-8/MAXIM-DOW</c> CRC standard (aliases <c>CRC-8/MAXIM</c>, <c>DOW-CRC</c>). Used by Maxim /
    ...
    /// <summary>
    /// Gets the <c>CRC-8/SMBUS</c> CRC standard (alias <c>CRC-8</c>). Width 8, polynomial <c>0x07</c>, initial value
    /// <c>0x00</c>, no reflection, XOR out <c>0x00</c>.
    /// </summary>
    /// <remarks>
    /// The canonical "CRC-8" used by SMBus (System Management Bus) and the de-facto generic 8-bit CRC.
    /// </remarks>
    ...
    /// <summary>
    /// Determines whether the current <see cref="CrcStandard" /> object is equal to another <see cref="CrcStandard" />
    /// object.
    /// </summary>
