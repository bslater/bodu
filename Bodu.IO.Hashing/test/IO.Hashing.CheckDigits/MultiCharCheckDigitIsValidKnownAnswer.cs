...
/// <see cref="MultiCharCheckDigitAlgorithm" />, pairing a complete sequence (in its canonical form) with the validity
/// verdict the algorithm is expected to render.
/// </summary>
/// <remarks>
/// Concrete test classes use these vectors to exercise <c>IsValid</c> with inputs that may not match the simple
/// <c>Body + ExpectedCheck</c> concatenation used for streaming and <c>Compute</c> tests. For example, IBAN's canonical
