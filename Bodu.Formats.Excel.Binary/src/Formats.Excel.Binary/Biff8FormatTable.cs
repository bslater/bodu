// ---------------------------------------------------------------------------------------------------------------
// <copyright file="Biff8FormatTable.cs" company="Bodu Pty. Ltd.">
// Copyright (c) Bodu Pty. Ltd. All rights reserved.
// </copyright>
// ---------------------------------------------------------------------------------------------------------------

using System.Buffers.Binary;
using System.Text;

namespace Bodu.Formats.Excel.Binary;

/// <summary>
/// Resolves a cell's extended-format index to its number-format index and code, built from the extended-format (XF) and
/// number-format (FORMAT) records of the workbook globals.
/// </summary>
/// <remarks>
/// A cell record references a format through its <c>ixfe</c> field, which indexes the ordered list of XF records. Each
/// XF in turn carries a number-format index, which a FORMAT record (or a built-in default) maps to a format code. This
/// table performs that two-step resolution and classifies the result as a date format where applicable.
/// </remarks>
internal sealed class Biff8FormatTable
{
    /// <summary>An empty table used when the workbook declares no globals.</summary>
    public static readonly Biff8FormatTable Empty = new([], new Dictionary<ushort, string>());

    /// <summary>The number-format index of each XF, indexed by XF position.</summary>
    private readonly ushort[] _xfFormatIndex;

    /// <summary>The custom format codes declared by FORMAT records, keyed by number-format index.</summary>
    private readonly Dictionary<ushort, string> _formatCodes;

    /// <summary>
    /// Initializes a new instance of the <see cref="Biff8FormatTable" /> class.
    /// </summary>
    /// <param name="xfFormatIndex">The number-format index of each XF, in XF order.</param>
    /// <param name="formatCodes">The custom format codes, keyed by number-format index.</param>
    private Biff8FormatTable(ushort[] xfFormatIndex, Dictionary<ushort, string> formatCodes)
    {
        _xfFormatIndex = xfFormatIndex;
        _formatCodes = formatCodes;
    }

    /// <summary>
    /// Builds a format table from the XF and FORMAT records of the workbook globals substream.
    /// </summary>
    /// <param name="records">The full ordered record list of the workbook.</param>
    /// <param name="start">The inclusive record index at which the globals substream begins.</param>
    /// <param name="endExclusive">The exclusive record index at which the globals substream ends.</param>
    /// <returns>The constructed format table.</returns>
    public static Biff8FormatTable Build(IReadOnlyList<Biff8Record> records, int start, int endExclusive)
    {
        List<ushort> xfFormatIndex = new();
        Dictionary<ushort, string> formatCodes = new();

        for (int i = start; i < endExclusive; i++)
        {
            Biff8Record record = records[i];
            switch (record.Type)
            {
                case Biff8RecordType.Xf when record.Payload.Length >= 4:
                    xfFormatIndex.Add(BinaryPrimitives.ReadUInt16LittleEndian(record.Payload.Span.Slice(2)));
                    break;

                case Biff8RecordType.Format when record.Payload.Length >= 5:
                    ReadFormatRecord(record.Payload.Span, formatCodes);
                    break;

                default:
                    break;
            }
        }

        return new Biff8FormatTable([.. xfFormatIndex], formatCodes);
    }

    /// <summary>
    /// Resolves the number-format index referenced by a cell's extended-format index.
    /// </summary>
    /// <param name="xfIndex">The cell's <c>ixfe</c> value.</param>
    /// <returns>The number-format index, or <c>0</c> (General) when the XF index is out of range.</returns>
    public ushort GetFormatIndex(ushort xfIndex) =>
        xfIndex < _xfFormatIndex.Length ? _xfFormatIndex[xfIndex] : (ushort)0;

    /// <summary>
    /// Gets the format code for a number-format index, falling back to the well-known built-in codes.
    /// </summary>
    /// <param name="formatIndex">The number-format index.</param>
    /// <returns>The format code, or <see langword="null" /> when none is known for the index.</returns>
    public string? GetFormatCode(ushort formatIndex) =>
        _formatCodes.TryGetValue(formatIndex, out string? code) ? code : ExcelNumberFormat.GetBuiltInFormatCode(formatIndex);

    /// <summary>
    /// Determines whether the cell referenced by an extended-format index is formatted as a date or time.
    /// </summary>
    /// <param name="xfIndex">The cell's <c>ixfe</c> value.</param>
    /// <returns><see langword="true" /> when the resolved number format is a date or time format.</returns>
    public bool IsDateFormatted(ushort xfIndex)
    {
        ushort formatIndex = GetFormatIndex(xfIndex);
        _formatCodes.TryGetValue(formatIndex, out string? code);

        return ExcelNumberFormat.IsDateFormat(formatIndex, code);
    }

    /// <summary>
    /// Decodes a FORMAT record and stores its format code under the record's number-format index.
    /// </summary>
    /// <param name="payload">The FORMAT record payload.</param>
    /// <param name="formatCodes">The map to populate.</param>
    private static void ReadFormatRecord(ReadOnlySpan<byte> payload, Dictionary<ushort, string> formatCodes)
    {
        ushort formatIndex = BinaryPrimitives.ReadUInt16LittleEndian(payload);
        int charCount = BinaryPrimitives.ReadUInt16LittleEndian(payload.Slice(2));
        bool highByte = (payload[4] & 0x01) != 0;

        int byteCount = highByte ? charCount * 2 : charCount;
        if (5 + byteCount > payload.Length)
            return;

        formatCodes[formatIndex] = highByte
            ? Encoding.Unicode.GetString(payload.Slice(5, byteCount))
            : DecodeCompressed(payload.Slice(5, charCount));
    }

    /// <summary>
    /// Decodes a compressed (8-bit) BIFF8 character run, one ISO-8859-1 code point per byte.
    /// </summary>
    /// <param name="bytes">The compressed character bytes.</param>
    /// <returns>The decoded string.</returns>
    private static string DecodeCompressed(ReadOnlySpan<byte> bytes)
    {
        char[] characters = new char[bytes.Length];
        for (int i = 0; i < bytes.Length; i++)
            characters[i] = (char)bytes[i];

        return new string(characters);
    }
}
