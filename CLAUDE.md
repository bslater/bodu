...
| `Bodu.Collections.Merkle` | `Bodu.Collections.Merkle/` | RFC 6962 Merkle trees and proofs (namespace `Bodu.Collections.Merkle`), **Core-only — no cipher dependency**: `Rfc6962MerkleTree` (Merkle Tree Hash over entries or fixed-size blocks, inclusion and consistency proofs, length-bound roots that close RFC 6962's tree-size ambiguity, single-pass streaming with a logarithmic-memory fold, optional parallel leaf hashing), `MerkleComputation`, `MerkleBlocks`. Deliberately separate from `Bodu.Security.Cryptography` so a consumer needing commitments does not take the cipher catalogue; that library's `MerkleTreeHash` / `ParallelMerkleTreeHash` share the domain separation but **not** the tree shape. Carries a `shared/` folder on the `Bodu.IO.Hashing/shared` pattern: `MerkleTreeFormat` (the three domain-separation prefixes, source-compiled into `Bodu.Security.Cryptography` under the `SECURITY_CRYPTOGRAPHY` symbol so the values cannot drift) and `MerkleTreeCore` (the stateless RFC 6962 primitives behind the public facades — split point, `Mth`, the inclusion and subproof walks, block arithmetic, the streaming fold — compilable into any assembly with only `Bodu.Core`, no package reference). Shared source carries no resources, so its guards throw without messages and the facades own the resx-backed validation. A companion **`Bodu.Collections.Merkle.SourceOnly.Test`** is a separate test assembly that source-compiles `shared/**` under the `SECURITY_CRYPTOGRAPHY` symbol with **no** reference to `Bodu.Collections.Merkle`, holding the core to the published RFC 6962 vectors — so adding a resource lookup or any non-`Bodu.Core` dependency to the shared source fails that build while leaving the owning package green. |
