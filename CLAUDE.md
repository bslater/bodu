...
| `Bodu.Text.Formats` | `Bodu.Text.Formats/` | Document formats: Bencode, Delimited (RFC 4180 CSV/TSV), DotEnv, INI, TOML. |
| `Bodu.Text.Serialization` | `Bodu.Text.Serialization/` | Common serialization engine: a lossless concrete-syntax-tree abstraction (`SyntaxNode` with an integer `RawKind` "type code", tokens, trivia) and a `System.Text.Json`-style POCO mapper (`FormatConverter<T>`, `FormatConverterFactory`, `FormatSerializerOptions`, naming policies, attributes) over a format-neutral reader/writer seam (`ISerializationReader` / `ISerializationWriter`). The per-format serializers build on it. |
| `Bodu.Text.Serialization.Bencode` | `Bodu.Text.Serialization.Bencode/` | `BencodeSerializer` — POCO ↔ Bencode (BEP 3) on the common engine, over a lossless Bencode CST (`BencodeSyntaxTree`) with canonical dictionary-key ordering. |
| `Bodu.Text.Serialization.Toml` | `Bodu.Text.Serialization.Toml/` | `TomlSerializer` — POCO ↔ TOML (v1.0.0 / v1.1.0) on the common engine, over a source-preserving CST (`TomlSyntaxTree`) with canonical TOML output. |
...
- **Bodu.Text.Serialization.Test** + **Bodu.Text.Serialization.{Toml,Bencode}.Test**: the common engine is tested through an in-memory token format (`FakeSerializationReader` / `FakeSerializationWriter` / `FakeFormat` under `test/Serialization/Infrastructure/`), so the binder, converters, options, and attributes are exercised without a real format; the format test projects drive their `*SyntaxTree` lossless round-trip (`ToFullString()` / `ToByteArray()` reproduces the source) and malformed-input sweeps under `[TestCategory("Regression")]`, plus the `*Serializer` POCO-mapping suite. The shared `RoundTripKat<TValue,TWire>` lives in `Bodu.Test.Kat`; the reusable serializer contract bases stay colocated per project until a second consumer appears.
