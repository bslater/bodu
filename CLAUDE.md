...
- **Bodu.Text.Delimited** (namespaces `Bodu.Text.Delimited[.Reader/.Writer/.Nodes/.Document]`): `Utf8DelimitedReader` / `Utf8DelimitedWriter`, `DelimitedReaderOptions` / `DelimitedWriterOptions` (dialect policies: `DelimitedFieldCountBehavior`, `DelimitedMalformedRecordBehavior`, `DelimitedDuplicateHeaderBehavior`), `DelimitedSerializer` (+`Options`/`Defaults`, `DeserializeAsyncEnumerableAsync<TRecord>`), `DelimitedNode`/`DelimitedArray`/`DelimitedObject`/`DelimitedValue`, `DelimitedDocument`/`DelimitedElement`/`DelimitedProperty`, `DelimitedFormatException` / `DelimitedSerializationException`.
