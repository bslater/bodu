...
- **Bodu.Formats.Excel.Binary** (namespace `Bodu.Formats.Excel.Binary`): `Biff8WorkbookReader`, `Biff8WorksheetReader`, `Biff8Record` / `Biff8RecordReader` / `Biff8RecordType`, `Biff8SharedStringTable`, `Biff8SheetInfo` / `Biff8SheetDimensions`, `Biff8WorkbookProperties`, `ExcelWorksheet` / `ExcelRow` / `ExcelCellReference`, `ExcelCell` / `ExcelCellKind` / `ExcelErrorCode`, `ExcelSerialDate` / `ExcelDateSystem` (number-format/date detection via the internal `Biff8FormatTable` / `ExcelNumberFormat`); exceptions `Biff8EncryptedWorkbookException` (et al.).
