...
- **`Bodu.Test.Kat`** namespace — generic KAT (known-answer test) records: `IKat` (marker interface exposing `Name`), `ValidKat<TInput,TExpected>`, `InvalidKat<TInput>`, `RoundTripKat<TValue,TWire>`, `BinaryKat<TInput,TExpected>`, `ExceptionKat<TInput>`, `GuardValidKat<T>`, `GuardInvalidKat<T>`, `EnumerableKat<TInput,TExpected>`, `BinaryEncodingKat`, `InvalidEncodedTextKat`, `BinaryDocumentKat<TDocument,TOptions>`, `InvalidBinaryDocumentKat<TOptions>`, `TextDocumentKat<TDocument,TOptions>`, `InvalidTextDocumentKat<TOptions>`, `HashKat`, `HashStreamingKat`, `ChecksumKat`, `CrcCatalogKat`, `CheckDigitKat`, `InvalidCheckDigitKat`, `HashExtensionKat`, `BlockCipherKat`, `BlockCipherModeKat`, `AeadKat`, `AeadTamperKat` (with `AeadTamperKind` enum), `CryptoHashKat`, `CalendarAlgorithmKat`, `RangeResolutionKat`, `RuleParseKat`, `InvalidRuleParseKat`, `CalendarDataKat`, `CalendarBuilderOutputKat`, `CalendarBuilderInvalidKat`, `ServiceRegistrationKat`. Plus `KatDisplayName.GetDisplayName(MethodInfo, object?[])` for `[DynamicData(... DynamicDataDisplayName = nameof(KatDisplayName.GetDisplayName), DynamicDataDisplayNameDeclaringType = typeof(KatDisplayName))]` wiring so failures show the row's `Name` instead of an opaque index.
- **`Bodu.Test.Contracts`** namespace — reusable abstract contract test bases that concrete subclasses pin to a specific implementation: `CollectionContractTests<TCollection,TItem>`, `ReadOnlyCollectionContractTests<TCollection,TItem>`, `SetContractTests<TSet,TItem>`, `EnumeratorContractTests<TEnumerable,TItem>`, `DebugViewContractTests<TCollection>`, `NonGenericCollectionContractTests<TCollection>` (with `SyncRootSupported` opt-out for concurrent collections), `NonCryptographicHashAlgorithmContractTests<TAlgorithm>`, `CheckDigitContractTests<TAlgorithm>`, `MultiCharCheckDigitContractTests<TAlgorithm>`, `BlockCipherContractTests<TCipher>` (tweak-aware), `BlockCipherModeContractTests<TCipher>` (with optional `RoundTripCases` for ciphers without published mode-specific vectors), `CryptoTransformContractTests<TTransform>`, `AeadContractTests<TAead>`, `CryptoHashContractTests<THash>`, `BinaryEncodingContractTests<TEncoding>`, `TextDocumentFormatContractTests<TDocument,TOptions>`, `BinaryDocumentFormatContractTests<TDocument,TOptions>`, `ParseFormatContractTests<T>`, `StreamRoundTripContractTests<T>`, `NotableDateTemporalExtensionContractTests<TDate>`, `ServiceLifetimeContractTests`. Concretes live in each domain test project alongside the bespoke tests.
