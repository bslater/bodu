...
| `Bodu.Collections` | `Bodu.Collections/` | The specialized generic-collection catalogue, split out of `Bodu.Core` (namespaces unchanged; references `Bodu.Core`): `Bodu.Collections.Generic` (circular buffer, deque, evicting dictionary, sequenced/multi-value dictionaries, multiset, indexed/ordered sets, indexed priority queue, range set/dictionary, segmented buffer), `Bodu.Collections.Specialized` (`BitSet` and the RFC 6962 Merkle tree — see below), `.Generic.Graphs` (graph, algorithms, disjoint set), `.Generic.Trees` (tree, tries), and `Bodu.Collections.Probabilistic` (Bloom filter, count-min sketch, HyperLogLog — approximate sketches over comparer-derived hashing). `ShuffleHelpers` and the internal `SequenceUtility` stay in `Bodu.Core` because the staying `IEnumerableExtensions` partials depend on them.<br/><br/>**`Bodu.Collections.Specialized`** holds the members of the package that serve a specialised purpose rather than acting as general-purpose containers: `BitSet` (packed bit set) and the RFC 6962 Merkle tree. The dividing line is coupling — neither references anything else in the package and nothing references them, whereas every other non-generic type in `Bodu.Collections.Generic` is either a collection in its own right or a policy/option satellite of one (`EvictingDictionaryPolicy`, `DequeOverflowPolicy`, `GraphAlgorithms`, …) that would be orphaned by a move. No `Bodu.Collections.Merkle` namespace or package exists. The files sit in `src/Collections.Specialized/` per the folder-follows-namespace rule. Five public types, `Bodu.Core`-only, no container semantics: `Rfc6962MerkleTree` (Merkle Tree Hash over entries or fixed-size blocks, inclusion and consistency proofs, length-bound roots that close RFC 6962's tree-size ambiguity, single-pass streaming with a logarithmic-memory fold, optional parallel leaf hashing), `MerkleBlockComputation`, `MerkleBlocks`, and the optional trace recorder `MerkleTreeDiagnostics` / `MerkleTreeDiagnosticNode`. It is deliberately **not** in `Bodu.Security.Cryptography` so a consumer needing commitments does not take the cipher catalogue; that library's `MerkleTreeHash` / `ParallelMerkleTreeHash` are facades over the same shared fold — at their default fan-out of two they produce RFC 6962's roots, bit-identical to `ComputeRootOfBlocks`, and a wider fan-out is an explicit non-RFC mode. The package carries a `shared/` folder on the `Bodu.IO.Hashing/shared` pattern: `MerkleTreeFormat` (the three domain-separation prefixes, source-compiled into `Bodu.Security.Cryptography` under the `SECURITY_CRYPTOGRAPHY` symbol so the values cannot drift), `MerkleTreeCore` (the stateless RFC 6962 primitives behind the public facades — split point, `Mth`, the inclusion and subproof walks, block arithmetic, the streaming leaf reader — compilable into any assembly with only `Bodu.Core`, no package reference), `MerkleLevelFold` (the k-ary level-by-level streaming fold with lone-node promotion that both packages reduce with — at fan-out two it visits exactly RFC 6962's nodes, holding at most `fanOut − 1` pending hashes per level), the `IMerkleTreeObserver` seam it reports leaves and nodes through, and the public `MerkleTreeDiagnostics` / `MerkleTreeDiagnosticNode` recorder implementing it. The whole `shared/**` glob is compiled into `Bodu.Security.Cryptography`, whose `MerkleTreeHash` and `ParallelMerkleTreeHash` are thin facades over the fold (the parallel one hashes each batch of leaves concurrently and folds in order). Shared source carries no resources, so its guards throw without messages and the facades own the resx-backed validation. A companion **`Bodu.Collections.SourceOnly.Test`** is a separate test assembly that source-compiles `shared/**` under the `SECURITY_CRYPTOGRAPHY` symbol with **no** reference to `Bodu.Collections`, holding the core to the published RFC 6962 vectors — so adding a resource lookup or any non-`Bodu.Core` dependency to the shared source fails that build while leaving the owning package green. |
...
- **Bodu.Collections.Specialized**: `BitSet` (growable packed bit set, Java semantics); `MerkleTreeDiagnostics` / `MerkleTreeDiagnosticNode` (optional per-node trace recorder, also compiled into `Bodu.Security.Cryptography`); `Rfc6962MerkleTree` (`HashLeaf` / `HashNode`, `ComputeRoot` / `ComputeRootOfLeafHashes`, `ComputeBlocked` / `ComputeRootOfBlocks`, `ComputeBlockedParallel` / `ComputeRootParallel`, `AuthenticationPath`, `VerifyInclusion` / `VerifyInclusionOfLeafHash` / `VerifyInclusionBound` / `VerifyBlockInclusion`, `BindRoot`, `ConsistencyProof` / `ConsistencyProofOfLeafHashes` / `VerifyConsistency`), `MerkleBlockComputation`, `MerkleBlocks`. Verification is total — every malformed input returns `false` and only a null path or proof throws — and `VerifyInclusion`'s `treeSize` is documented as trusted input, with `VerifyInclusionBound` / `VerifyBlockInclusion` as the fail-closed alternatives.
...
- **Bodu.Collections.Test** (`test/Collections.Specialized/`): the `BitSetTests` partials plus the RFC 6962 suite — member-named backbone partials on `Rfc6962MerkleTreeTests` (`.ComputeRoot`, `.AuthenticationPath`, `.VerifyInclusion`, `.VerifyConsistency`, `.Parallel`, …) plus `MerkleBlocksTests`. Published vectors reuse the shared `ValidKat<int,string>`; the path and consistency rows need the local `MerkleInclusionKat` / `MerkleConsistencyKat` records, because their input is a pair of sizes. Negative coverage is a **systematic mutation matrix** rather than a fixed list (shifted and flipped indices and sizes, wrong/empty/swapped roots, each root injected as a proof step at either end, every step bit-flipped, removed, duplicated and mis-sized), plus seeded malformed-input sweeps asserting only that verification never throws.
