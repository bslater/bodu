...
| `Bodu.Collections.Merkle` | `Bodu.Collections.Merkle/` | RFC 6962 Merkle trees and proofs (namespace `Bodu.Collections.Merkle`), **Core-only — no cipher dependency**: `Rfc6962MerkleTree` (Merkle Tree Hash over entries or fixed-size blocks, inclusion and consistency proofs, length-bound roots that close RFC 6962's tree-size ambiguity, single-pass streaming with a logarithmic-memory fold, optional parallel leaf hashing), `MerkleComputation`, `MerkleBlocks`. Deliberately separate from `Bodu.Security.Cryptography` so a consumer needing commitments does not take the cipher catalogue; that library's `MerkleTreeHash` / `ParallelMerkleTreeHash` share the domain separation but **not** the tree shape. |
