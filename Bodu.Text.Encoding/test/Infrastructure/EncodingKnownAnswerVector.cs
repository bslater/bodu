// ---------------------------------------------------------------------------------------------------------------
// <copyright file="EncodingKnownAnswerVector.cs" company="Bodu Pty. Ltd.">
...
/// implementation. A vector pairs a binary input with the canonical encoded form so that data-driven tests can verify
/// <see cref="Base16" />, <see cref="Base32" />, <see cref="Base64" />, <see cref="Base58" />, and
    ...
    string? Source = null) : IKat
{
    ...
    /// <summary>
    /// Builds a vector from an ASCII string input, used for the many RFC 4648 §10 examples that express the input as a
    /// readable ASCII fragment ("f", "fo", "foo", "foob", "fooba", "foobar").
    /// </summary>
    ...
    /// </summary>
    /// <param name="description">A short label.</param>
    /// <param name="decodedHex">
    /// The binary input expressed as hex characters. An empty string yields an empty byte array.
    /// </param>
    /// <param name="encoded">The expected encoded form.</param>
    /// <param name="source">The citation.</param>
