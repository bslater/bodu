...
/// <summary>
/// Compiles the text-encoding documentation guide examples that are explicitly opted in, so a code sample cannot
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    /// <summary>
    /// Verifies that every opted-in text-encoding guide snippet compiles against the current public API, with at
    /// least one snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void EncodingGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
    ...
    /// text-encoding guides directory beneath it.
    /// </summary>
    ...
    private static string? FindGuidesDirectory()
    {
        for (DirectoryInfo? dir = new(AppContext.BaseDirectory); dir is not null; dir = dir.Parent)
        {
            ...
            {
                string guides = Path.Combine(dir.FullName, "docs", "guides", "text-encoding");
