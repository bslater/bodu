...
public sealed partial class Base58Tests
{
    ...
    public void RoundTrip_ForEverySingleByteValueAndVariant_ShouldRecover(Base58Variant variant)
    {
        for (var value = 0; value <= 255; value++)
        {
            ...
            var encoded = Base58.Encode(original, variant);
            var decoded = Base58.Decode(encoded, variant);
    ...
    public void RoundTrip_WhenSpanTryPath_ShouldRecoverOriginal()
    {
        ...
        var charBuffer = new char[Base58.GetMaxEncodedLength(original.Length)];
        var byteBuffer = new byte[Base58.GetMaxDecodedLength(charBuffer.Length)];

        var encOk = Base58.TryEncode(original.AsSpan(), charBuffer, out var charsWritten);
        var decOk = Base58.TryDecode(charBuffer.AsSpan(0, charsWritten), byteBuffer, out var bytesWritten);
