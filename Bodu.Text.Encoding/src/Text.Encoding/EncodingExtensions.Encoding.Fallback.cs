...
public static partial class EncodingExtensions
{
    ...
    /// </exception>
    /// <remarks>
    /// The returned encoding is a fresh instance — singleton encodings such as
    /// <see cref="System.Text.Encoding.UTF8" /> are not mutated. Use this method to opt a specific call-site
    /// into strict validation without altering the global default.
    /// </remarks>
    ...
    /// The replacement string used when an encoder cannot represent a character. Defaults to <c>"?"</c>, which
    /// every supported encoding can represent.
    /// </param>
    /// <param name="decoderReplacement">
    /// The replacement string used when a decoder cannot interpret a sequence. Defaults to <c>"?"</c> rather
    /// than the Unicode replacement character (<c>U+FFFD</c>) because non-Unicode encodings — including ASCII —
    /// cannot represent <c>U+FFFD</c>.
    /// </param>
    /// <returns>
    /// A new <see cref="System.Text.Encoding" /> instance configured with
    /// <see cref="System.Text.EncoderReplacementFallback" /> and
    /// <see cref="System.Text.DecoderReplacementFallback" />.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    ...
    /// Thrown when <paramref name="encoderReplacement" /> cannot be encoded by <paramref name="encoding" />.
    /// The exception's <see cref="Exception.InnerException" /> is the
    /// <see cref="System.Text.EncoderFallbackException" /> reported by a strict-fallback clone of
    /// <paramref name="encoding" /> when asked to encode the replacement.
    /// </exception>
    ...
    /// <summary>
    /// Verifies that <paramref name="replacement" /> can be encoded by <paramref name="encoding" /> by routing
    /// through a clone configured with <see cref="System.Text.EncoderExceptionFallback" /> and re-throwing as
        ...
        catch (System.Text.EncoderFallbackException ex)
        {
            throw new ArgumentException(
                string.Format(
                    ...
                    EncodingResourceStrings.Arg_Invalid_ReplacementNotEncodable,
                    replacement),
                paramName,
                ex);
        }
    ...
    /// <see cref="System.Text.EncoderExceptionFallback" /> and
    /// <see cref="System.Text.DecoderExceptionFallback" />.
    /// </summary>
    /// <param name="encoding">The encoding to inspect.</param>
    /// <returns>
    /// <see langword="true" /> when both the encoder and decoder fallbacks are the BCL exception fallbacks;
    ...
    /// <see cref="System.Text.DecoderExceptionFallback" /> types are recognised. Custom fallback subclasses
    ...
    /// <param name="encoding">The encoding to inspect.</param>
    /// <returns>
    /// <see langword="true" /> when both the encoder and decoder fallbacks are the BCL replacement fallbacks;
    /// otherwise <see langword="false" />.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="encoding" /> is <see langword="null" />.
    ...
    /// <see cref="System.Text.DecoderReplacementFallback" /> types are recognised. Custom fallback subclasses
    /// return <see langword="false" />.
    /// </remarks>
