// ---------------------------------------------------------------------------------------------------------------
// <copyright file="PairWebRateProviderTests.LoadFailureLogging.cs" company="Bodu Pty. Ltd.">
...
public partial class PairWebRateProviderTests
{
    ...
    private static (TestPairWebRateProvider Provider, CapturingLogger Logger) CreateThrowing(Exception exception)
    {
        CapturingLogger logger = new();
        TestPairWebRateProvider provider = new(new ThrowingPairSource(exception), new TestWebRateProviderOptions(), logger);
    ...
    {
        (TestPairWebRateProvider provider, CapturingLogger logger) = CreateThrowing(new HttpRequestException("boom"));
    ...
    {
        (TestPairWebRateProvider provider, CapturingLogger logger) = CreateThrowing(new ExchangeRateFormatException("boom"));
    ...
    {
        (TestPairWebRateProvider provider, CapturingLogger logger) = CreateThrowing(new OperationCanceledException());
