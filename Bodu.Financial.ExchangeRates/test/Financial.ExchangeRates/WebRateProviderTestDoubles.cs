// ---------------------------------------------------------------------------------------------------------------
// <copyright file="WebRateProviderTestDoubles.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// A minimal single-base <see cref="WebRateProvider" /> that loads its whole catalogue in one shot, modelling a
/// bulk feed (such as RBA, ECB, or Bank of England) so the base warm-up surface can be exercised directly against a
/// provider that is not built on <see cref="PairWebRateProvider{TSeries}" />.
/// </summary>
internal sealed class TestBulkWebRateProvider
    : WebRateProvider
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="TestBulkWebRateProvider" /> class.
    /// </summary>
    /// <param name="catalogue">The observations the feed serves once loaded.</param>
    public TestBulkWebRateProvider(params ExchangeRate[] catalogue)
