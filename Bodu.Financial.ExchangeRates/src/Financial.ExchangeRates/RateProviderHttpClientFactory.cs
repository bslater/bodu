...
{
    /// <summary>The default cap, in bytes, applied to <see cref="HttpClient.MaxResponseContentBufferSize" /> for a provider-owned client (64 MiB).</summary>
    /// <remarks>
    /// The exchange-rate feeds these clients address return small payloads (a CSV, XML, or JSON document), so a finite
    /// ceiling well above any legitimate response prevents a compromised or man-in-the-middle endpoint from driving
    /// unbounded memory use, while never affecting a well-formed response.
    /// </remarks>
    ...
    /// The maximum number of bytes to buffer when reading a response. Values not greater than zero are ignored, leaving
    /// the framework default; the default caps the client at <see cref="DefaultMaxResponseContentBufferSize" />.
    /// </param>
    ...
    /// resilience pipeline supplies its client to the provider instead and does not use this factory. The buffer cap
    /// bounds the memory a single response can consume, so an oversized body fails rather than exhausting memory.
    /// </remarks>
