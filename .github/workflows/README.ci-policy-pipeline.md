...
The **Policy Gate** (Tier 1) is live as soon as this lands: it is deterministic, needs no secrets,
and costs nothing per PR. The two agent arms — **Claude Policy Review** and **Claude Policy Fix** —
arrive **dormant**, because the fix arm pushes commits to the pull request's branch and the review
arm is designed to block a merge. Both skip every run until you arm them:

0. **Arm the agents:** set the repository variable `ENABLE_CLAUDE_POLICY_AGENTS` to `true`
   (Settings → Secrets and variables → Actions → Variables). Do this *after* step 1, not before:
   armed without the secrets, both jobs fail rather than skip. To disarm, unset the variable —
   neither workflow needs editing.
...
2. **Branch protection:** mark **Build and Test** and **Policy Gate** as required status checks.
   Add **Claude Policy Review** to that list only once step 0 has armed it — a dormant job never
   reports a conclusion to satisfy. Keep auto-merge off — a human still reviews and merges.
