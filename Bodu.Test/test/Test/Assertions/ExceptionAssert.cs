...
/// <summary>
/// Provides reusable exception assertion helpers for MSTest test projects, including the guard-contract matrix helper
/// used across <c>ThrowHelper</c>-style tests and a strongly typed <c>ParamName</c> assertion.
/// </summary>
public static class ExceptionAssert
{
    /// <summary>
    /// Asserts that <paramref name="action" /> throws <typeparamref name="TException" /> exactly, with the expected
    /// <see cref="ArgumentException.ParamName" />, and returns the captured exception for further inspection.
    /// </summary>
    ...
    /// The expected value of <see cref="ArgumentException.ParamName" /> on the thrown exception.
    /// </param>
    ...
    /// <summary>
    /// Asserts the standard guard contract for a single helper invocation: the action either completes silently (when
    /// <paramref name="expectedExceptionType" /> is <see langword="null" />) or throws exactly the requested exception
    /// type, optionally with a specific <see cref="ArgumentException.ParamName" />.
    /// </summary>
    /// <param name="testName">
    /// A descriptive label included in assertion failure messages so a failing row is identifiable in
    /// <c>[DynamicData]</c> or <c>[DataRow]</c> output.
    /// </param>
    /// <param name="act">The guard invocation under test.</param>
    /// <param name="expectedExceptionType">
    /// The exact <see cref="Exception" />-derived type that the guard must throw, or <see langword="null" /> if the
    /// invocation must complete without throwing.
    /// </param>
    /// <param name="expectedParamName">
    ...
    /// </param>
    /// <exception cref="ArgumentNullException">
    /// Thrown if <paramref name="act" /> is <see langword="null" />.
    /// </exception>
    /// <remarks>
    /// <para>
    /// The helper deliberately accepts a base <see cref="Exception" /> type rather than a generic so that data rows can
    /// mix multiple expected exception types in a single matrix.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Asserts that <paramref name="invoke" /> applied to the operands in <paramref name="kat" /> completes silently,
