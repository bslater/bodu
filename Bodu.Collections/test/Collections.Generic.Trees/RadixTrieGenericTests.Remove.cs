...
public sealed partial class RadixTrieGenericTests
{
    ...
    /// <summary>
    /// Verifies that the span <see cref="RadixTrie{TValue}.Remove(ReadOnlySpan{char})" /> overload removes a present
    /// key and reports success, mirroring the string overload.
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// Verifies that the span <see cref="RadixTrie{TValue}.Remove(ReadOnlySpan{char})" /> overload returns
    /// <see langword="false" /> for an absent key.
    /// </summary>
    [TestMethod]
    public void Remove_WhenSpanKeyAbsent_ShouldReturnFalse()
    {
        var sut = new RadixTrie<int> { ["tea"] = 1 };
