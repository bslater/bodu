...
public sealed partial class NavigableDictionary<TKey, TValue>
{
    ...
    /// <remarks>
    /// An interior node with two children is reduced to its in-order successor (the key and value are copied down), so
    /// the physically unlinked node always has at most one child. When that node is a childless black leaf it stays
    /// linked as a phantom during the fixup — its size of 1 keeps every rotation's size recomputation consistent — and
    /// is unlinked afterwards, at which point the ancestor sizes are decremented.
    /// </remarks>
    private void RemoveNode(Node node)
    {
        // Interior node: copy the successor's entry into place and remove the successor instead.
    ...
    /// <summary>
    /// Builds a balanced red-black subtree from the key-sorted slice of <paramref name="entries" /> bounded by
    ...
    private static Node? BuildFromSortedArray(KeyValuePair<TKey, TValue>[] entries, int startIndex, int endIndex, Node? redNode)
    {
        ...
        {
            case 1:
                root = new Node(entries[startIndex].Key, entries[startIndex].Value);
                Link(root, redNode, asLeft: true);
                break;

            case 2:
                root = new Node(entries[startIndex].Key, entries[startIndex].Value);
                Link(root, new Node(entries[endIndex].Key, entries[endIndex].Value) { IsRed = true }, asLeft: false);
                Link(root, redNode, asLeft: true);
                break;

            case 3:
                root = new Node(entries[startIndex + 1].Key, entries[startIndex + 1].Value);
                Link(root, new Node(entries[startIndex].Key, entries[startIndex].Value), asLeft: true);
                Link(root, new Node(entries[endIndex].Key, entries[endIndex].Value), asLeft: false);
            ...
            default:
                ...
                root = new Node(entries[midpoint].Key, entries[midpoint].Value);
                Link(root, BuildFromSortedArray(entries, startIndex, midpoint - 1, redNode), asLeft: true);

                Node? rightSubtree = size % 2 == 0
                    ? BuildFromSortedArray(entries, midpoint + 2, endIndex, new Node(entries[midpoint + 1].Key, entries[midpoint + 1].Value) { IsRed = true })
                    : BuildFromSortedArray(entries, midpoint + 1, endIndex, null);
    ...
    /// key/value pair of fields, per the follow-on section of the navigable-collections design note. Duplicating the
    /// rotation/fixup core was chosen over genericizing the set's private nodes to keep the committed set untouched.
    /// </remarks>
    [Serializable]
