    ...
    where T : notnull
{
    ...
    public IEnumerable<T> Distinct()
    {
        ...
        {
            yield return version != _version ? throw new InvalidOperationException(CollectionsResourceStrings.Op_Invalid_CollectionModified) : key;
        }
    }
    ...
    public IEnumerable<KeyValuePair<T, int>> Frequencies()
    {
        ...
        foreach (KeyValuePair<T, int> pair in _items)
        {
            yield return version != _version ? throw new InvalidOperationException(CollectionsResourceStrings.Op_Invalid_CollectionModified) : pair;
        }
    }
