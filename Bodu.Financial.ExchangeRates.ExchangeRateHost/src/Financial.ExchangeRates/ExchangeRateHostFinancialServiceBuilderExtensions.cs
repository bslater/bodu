// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ExchangeRateHostFinancialServiceBuilderExtensions.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Provides the fluent registration of the exchangerate.host exchange-rate provider onto an
/// <see cref="IFinancialServiceBuilder" />.
/// </summary>
public static class ExchangeRateHostFinancialServiceBuilderExtensions
{
    /// <summary>The name of the <see cref="HttpClient" /> configured for exchangerate.host requests.</summary>
    public const string HttpClientName = "Bodu.Financial.ExchangeRates.ExchangeRateHost";

    /// <summary>
    /// Registers the exchangerate.host exchange-rate provider, binding its options and configuring a named
    ...
    /// bound into <see cref="ExchangeRateHostRateProviderOptions" />.
    /// </param>
    /// <param name="sectionName">The configuration section name. Defaults to <c>Financial:ExchangeRateHost</c>.</param>
    ...
    public static IFinancialServiceBuilder AddExchangeRateHostExchangeRates(
        ...
        => builder.AddWebRateProvider<ExchangeRateHostRateProvider, ExchangeRateHostRateProviderOptions>(
            HttpClientName,
            configuration,
            sectionName,
            "exchangerate.host exchange-rate options are invalid.",
            configure,
            configureResilience,
            static (client, opts, loggerFactory, timeProvider) =>
                new ExchangeRateHostRateProvider(client, opts, loggerFactory?.CreateLogger<ExchangeRateHostRateProvider>(), timeProvider));
}
