// ---------------------------------------------------------------------------------------------------------------
// <copyright file="TomlParser.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// A single-pass recursive-descent parser that materializes TOML source text into a <see cref="TomlTableSyntax" />
/// document model, enforcing the specification's key, value, table, and array-of-tables rules for the configured
...
internal sealed class TomlParser
{
    ...
    /// <summary>
    /// The current one-based source line.
    /// </summary>
    private int _line = 1;

    /// <summary>
    /// The offset within <see cref="_src" /> at which the current line begins.
    /// </summary>
    ...
    /// </summary>
    private TomlTableSyntax _current;

    /// <summary>
    /// Initializes a new instance of the <see cref="TomlParser" /> class over the supplied source.
    /// </summary>
    ...
    internal TomlParser(string source, TomlSpecVersion specVersion)
    {
        _src = source;
        _current = _root;
    ...
    internal TomlTableSyntax Parse()
    {
        ...
        while (!Eof)
        {
            ...
            {
                if (!AtNewline())
                    throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExpectedNewline);
                ConsumeNewline();
            }
    ...
    private void SkipComment()
    {
        ...
        while (!Eof && !AtNewline())
        {
            var c = Current;
            if (c == '\r')
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlControlCharacter);
    ...
    /// <summary>
    /// Skips whitespace, comments, and newlines — the <c>ws-comment-newline</c> production used inside arrays.
    /// </summary>
    ...
    private void ParseTableHeader()
    {
        ...
        if (Eof || Current != ']')
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlUnterminatedTable);
        Advance();
    ...
    private void ParseKeyValue(TomlTableSyntax target)
    {
        var path = ParseKeyPath();
        SkipInlineWhitespace();
        if (Eof || Current != '=')
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExpectedEquals);
        Advance();
        SkipInlineWhitespace();

        TomlSyntaxNode value = ParseValue();
    ...
    {
        List<string> keys = [ParseSimpleKey()];
        while (true)
        {
            SkipInlineWhitespace();
            if (!Eof && Current == '.')
    ...
    {
        if (Eof)
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExpectedKey);

        var c = Current;
        if (c == '"')
        {
            if (Peek(1) == '"' && Peek(2) == '"')
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlMultilineKey);
    ...
    private TomlSyntaxNode ParseValue()
    {
        if (Eof)
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExpectedValue);

        return Current switch
        {
            '"' => String(Peek(1) == '"' && Peek(2) == '"' ? ParseMultilineBasicString() : ParseBasicString()),
            '\'' => String(Peek(1) == '\'' && Peek(2) == '\'' ? ParseMultilineLiteralString() : ParseLiteralString()),
            '[' => ParseArray(),
            '{' => ParseInlineTable(),
            't' or 'f' => ParseBoolean(),
    ...
    /// <summary>
    /// Creates a string scalar node.
    /// </summary>
    /// <param name="value">The string value.</param>
    /// <returns>The scalar node.</returns>
    private static TomlScalarSyntax String(string value) =>
        new(TomlSyntaxKind.String, value, default);

    /// <summary>
    /// Parses a boolean literal.
    /// </summary>
    /// <returns>The boolean value.</returns>
    private TomlSyntaxNode ParseBoolean()
    {
        if (TryConsumeKeyword("true"))
            return new TomlScalarSyntax(TomlSyntaxKind.Boolean, true, default);
        if (TryConsumeKeyword("false"))
            return new TomlScalarSyntax(TomlSyntaxKind.Boolean, false, default);
        throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExpectedValue);
    }
    ...
    private string ParseBasicString()
    {
        ...
        {
            if (Eof || AtNewline())
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlUnterminatedString);
    ...
    /// <summary>
    /// Indicates whether the backslash at the cursor is a line-ending backslash.
    /// </summary>
    ...
    /// <summary>
    /// Appends the escape sequence beginning at the cursor (positioned just after the backslash).
    /// </summary>
    /// <param name="sb">The destination builder.</param>
    private void AppendEscape(StringBuilder sb)
    {
        if (Eof)
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidEscape);
        ...
        switch (e)
        {
            ...
            default: throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidEscape);
        }
    }
    ...
    private TomlArraySyntax ParseArray()
    {
        Advance();
        TomlArraySyntax array = new();
        while (true)
        {
            SkipValueSeparators();
            if (Eof)
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidArray);
            if (Current == ']')
            {
                Advance();
                return array;
    ...
    /// <summary>
    /// Parses an inline table (its opening brace at the cursor).
    /// </summary>
    /// <returns>The inline table.</returns>
    private TomlTableSyntax ParseInlineTable()
    {
        Advance();
        TomlTableSyntax table = new();
        ...
        while (true)
        {
            ...
            if (Eof)
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidInlineTable);
            ...
            if (Current == ',')
            {
                ...
                {
                    if (_specVersion != TomlSpecVersion.V1_1)
                        throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInlineTableTrailingComma);
    ...
    /// </summary>
    /// <returns>The parsed value.</returns>
    private TomlSyntaxNode ParseNumberOrDateTime()
    {
    ...
    /// <summary>
    /// Reads a bare value token up to the next value terminator.
    /// </summary>
    ...
    private TomlSyntaxNode ParseNumber()
    {
    ...
    /// <summary>
    /// Creates a float scalar node.
    /// </summary>
    /// <param name="value">The float value.</param>
    /// <returns>The scalar node.</returns>
    private static TomlScalarSyntax Float(double value) =>
        new(TomlSyntaxKind.Float, value, default);

    /// <summary>
    /// Creates an integer scalar node.
    /// </summary>
    /// <param name="value">The integer value.</param>
    /// <returns>The scalar node.</returns>
    private static TomlScalarSyntax Integer(long value) =>
        new(TomlSyntaxKind.Integer, value, default);
    ...
    private TomlSyntaxNode ParseDecimalInteger(string token)
    {
        var i = 0;
        var negative = false;
        if (token[0] == '+' || token[0] == '-')
        ...
        if (!TryStripUnderscores(body, allowHexLetters: false, out var clean) || clean.Length == 0 || !AllAsciiDigits(clean))
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidNumber);
        if (clean.Length > 1 && clean[0] == '0')
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidNumber);

        if (!ulong.TryParse(clean, NumberStyles.None, CultureInfo.InvariantCulture, out var magnitude))
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlIntegerOutOfRange);

        if (negative)
        {
            if (magnitude > (ulong)long.MaxValue + 1)
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlIntegerOutOfRange);
            return Integer(magnitude == (ulong)long.MaxValue + 1 ? long.MinValue : -(long)magnitude);
        }

        if (magnitude > long.MaxValue)
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlIntegerOutOfRange);
        return Integer((long)magnitude);
    }
    ...
    /// <returns>The integer value.</returns>
    private TomlSyntaxNode ParseRadixInteger(string body, int radix)
    {
    ...
    private TomlSyntaxNode ParseFloat(string token)
    {
    ...
    private TomlSyntaxNode ParseDateTime()
    {
        ...
        if (!hasTime)
            return new TomlScalarSyntax(TomlSyntaxKind.LocalDate, MakeDate(year, month, day), default);
        ...
        if (!Eof && (Current == 'Z' || Current == 'z' || Current == '+' || Current == '-'))
        {
            ...
            try
            {
                return new TomlScalarSyntax(TomlSyntaxKind.OffsetDateTime, new DateTimeOffset(local, offset), default);
            }
            catch (ArgumentException)
            {
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlInvalidDateTime);
            }
        }

        return new TomlScalarSyntax(TomlSyntaxKind.LocalDateTime, MakeDateTime(year, month, day, hour, minute, second, fractionTicks), default);
    }
    ...
    private TomlSyntaxNode ParseTimeValue()
    {
        var (hour, minute, second, fractionTicks) = ReadPartialTime();
        return new TomlScalarSyntax(TomlSyntaxKind.LocalTime, MakeTime(hour, minute, second, fractionTicks), default);
    }
    ...
    private (int Hour, int Minute, int Second, long FractionTicks) ReadPartialTime()
    {
        ...
        {
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlSecondsRequired);
        }
    ...
    {
        if (second == 60)
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlLeapSecond);
    ...
    /// When <see langword="true" />, hexadecimal letters count as adjacent digits.
    /// </param>
    ...
    /// <summary>
    /// Indicates whether <paramref name="c" /> is an ASCII hexadecimal digit.
    /// </summary>
    /// <param name="c">The character.</param>
    ...
    private void DefineStandardTable(List<string> path)
    {
        ...
        {
            if (existing is not TomlTableSyntax child)
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlDuplicateTable);
            if (_inline.Contains(child))
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExtendInlineTable);
    ...
    private void DefineArrayTable(List<string> path)
    {
        ...
        var key = path[^1];
        TomlArraySyntax array;
        if (table.TryGetValue(key, out var existing))
        {
            if (existing is not TomlArraySyntax found || !_tableArrays.Contains(found))
            {
                throw Error(existing is TomlArraySyntax
                    ? TomlSerializationResourceStrings.Format_Invalid_TomlAppendToStaticArray
                    : TomlSerializationResourceStrings.Format_Invalid_TomlArrayTableConflict);
            }
        ...
        {
            array = new TomlArraySyntax();
            _tableArrays.Add(array);
            table[key] = array;
        }

        TomlTableSyntax element = new();
        array.Add(element);
        _current = element;
    }
    ...
    private TomlTableSyntax WalkHeaderSegment(TomlTableSyntax table, string key)
    {
        ...
        if (table.TryGetValue(key, out var existing))
        {
            switch (existing)
            {
                case TomlTableSyntax child when !_inline.Contains(child):
                    return child;
                case TomlTableSyntax:
                    throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExtendInlineTable);
                case TomlArraySyntax array when _tableArrays.Contains(array) && array.Count > 0:
                    return (TomlTableSyntax)array[array.Count - 1];
                default:
                    throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlDuplicateTable);
            }
        }
    ...
    private void AssignKeyValue(TomlTableSyntax target, List<string> path, TomlSyntaxNode value)
    {
        var table = target;
        for (var i = 0; i < path.Count - 1; i++)
        ...
        if (table.ContainsKey(key))
            throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlDuplicateKey);
    ...
    private TomlTableSyntax WalkDottedSegment(TomlTableSyntax table, string key)
    {
        if (table.TryGetValue(key, out var existing))
        {
            if (existing is not TomlTableSyntax child)
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlKeyOnValue);
            if (_inline.Contains(child))
                throw Error(TomlSerializationResourceStrings.Format_Invalid_TomlExtendInlineTable);
