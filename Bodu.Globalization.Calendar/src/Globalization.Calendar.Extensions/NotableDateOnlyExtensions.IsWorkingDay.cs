...
public static partial class NotableDateOnlyExtensions
{
    ...
    /// <param name="date">The <see cref="DateOnly" /> value to evaluate.</param>
    /// <param name="service">
    /// The <see cref="INotableDateService" /> consulted for evaluation. Must not be <see langword="null" />.
    /// </param>
    ...
    /// The working-week pattern that determines which days-of-week are treated as working days.
    /// </param>
    /// <param name="territoryCode">An optional territory scope.</param>
    /// <param name="calendarType">An optional calendar scope forwarded to the service for rule resolution.</param>
    /// <returns>
    /// <see langword="true" /> when <paramref name="date" />'s day-of-week is selected in
    ...
    public static bool IsWorkingDay(this DateOnly date, INotableDateService service, WeekPattern workingWeek, string? territoryCode = null, Type? calendarType = null)
    {
        ...
        return !workingWeek.Contains(date.DayOfWeek)
            ? false
            : !service.IsHolidayNonWorkingDay(date.ToDateTime(TimeOnly.MinValue), territoryCode, calendarType);
    }
    ...
    /// <param name="calendarType">An optional calendar scope forwarded to the service for rule resolution.</param>
    /// <returns>
    /// <see langword="true" /> when <paramref name="date" />'s day-of-week is in the working week and no non-working
    /// notable date covers it; otherwise, <see langword="false" />.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="service" /> is <see langword="null" />.
    /// </exception>
