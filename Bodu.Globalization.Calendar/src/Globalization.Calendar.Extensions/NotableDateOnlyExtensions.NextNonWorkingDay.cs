...
public static partial class NotableDateOnlyExtensions
{
    ...
    /// non-working days strictly after the supplied <paramref name="date" />, evaluated against the ambient
    ...
    /// <summary>
    /// Returns a new <see cref="DateOnly" /> representing the non-working day that is <paramref name="count" />
    /// non-working days strictly after the supplied <paramref name="date" />, evaluated against the supplied
    ...
    public static DateOnly NextNonWorkingDay(this DateOnly date, INotableDateService service, int count = 1, string? territoryCode = null, Type? calendarType = null)
    {
        ...
        while (remaining > 0)
        {
            if (dayNumber >= DateOnly.MaxValue.DayNumber)
                throw new ArgumentOutOfRangeException(nameof(count), string.Format(System.Globalization.CultureInfo.InvariantCulture, CalendarResourceStrings.Arg_OutOfRange_AdvanceOverrunNonWorkingDays, "DateOnly.MaxValue"));
