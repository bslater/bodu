// ---------------------------------------------------------------------------------------------------------------
// <copyright file="HinduLunarNotableDateCalculator.cs" company="PlaceholderCompany">
...
/// <summary>
/// Provides a calculator for determining the approximate Gregorian date of a Hindu festival defined by a lunar month, paksha
/// (fortnight), and tithi (lunar day).
/// </summary>
/// <remarks>
/// <para>
/// Hindu festivals are traditionally defined in terms of the Hindu lunisolar (panchanga) calendar. A date in this calendar is
/// specified as a tithi (one of thirty lunar days in a synodic month) within a paksha (bright or dark fortnight) of a named lunar
/// month. The lunar month itself is identified by the solar sign (rashi) that the sun occupies at the new moon — a calculation that
/// requires both solar and lunar positional astronomy.
/// </para>
/// <para>
/// <strong>Calculation approach:</strong> This calculator uses the following approximation method, accurate to within one or two
/// calendar days for years in the range 1900–2100:
/// </para>
/// <list type="number">
///   <item>
///     <description>
///       Estimate the Gregorian month in which the target Hindu lunar month is likely to begin, using a fixed month-offset table
///       (since Hindu months advance by approximately 11 days per Gregorian year in a regular 19-year Metonic cycle, but reset
///       by the intercalary month approximately every 32 months, the offset is treated as approximately fixed for single-year
///       lookup purposes).
///     </description>
///   </item>
///   <item>
///     <description>
///       Find the new moon on or after the first day of that estimated Gregorian month. This new moon is taken as the start of
///       the Amanta (new-moon reckoning) lunar month.
///     </description>
///   </item>
///   <item>
///     <description>
...
/// </list>
/// <para>
/// <strong>Accuracy note:</strong> The actual tithi at any moment depends on the precise instantaneous positions of the sun and
/// moon. The method used here approximates tithis as equal subdivisions of the synodic month and approximates the month start by
/// an astronomical new moon rather than the precise panchanga calculation based on sunrise at a specific geographic location.
/// Results may differ from locally computed Indian panchanga dates by zero, one, or (rarely) two calendar days. Consumers who
/// require panchanga-exact dates should register a custom implementation using a full Surya Siddhanta or Drik calculation.
/// </para>
/// </remarks>
public sealed class HinduLunarNotableDateCalculator
	: INotableDateCalculator
{
	...
	/// <summary>
	/// Initialises a new instance of the <see cref="HinduLunarNotableDateCalculator" /> class for the given Hindu lunar month,
	/// paksha, and tithi.
	/// </summary>
	...
	/// <exception cref="ArgumentOutOfRangeException"><paramref name="tithi" /> is less than 1 or greater than 15.</exception>
	/// <exception cref="ArgumentException">
	/// <paramref name="month" /> or <paramref name="paksha" /> is not a defined enumeration value.
	/// </exception>
	public HinduLunarNotableDateCalculator(HinduLunarMonth month, HinduPaksha paksha, int tithi)
	{
	...
	/// </param>
	/// <returns>
	/// A <see cref="DateTime" /> representing the approximate date of the festival, or <see langword="null" /> if the date cannot be
	...
	public DateTime? GetDate(int year, SysGlobal.Calendar? calendar = null)
	{
		...
		DateTime? monthNewMoon = LunarPhaseCalculator.GetNewMoonOnOrAfter(searchStart);
		...
		else
		{
			...
			DateTime? fullMoon = LunarPhaseCalculator.GetFullMoonOnOrAfter(fullMoonApprox.AddDays(-2));
			if (fullMoon is null)
				return null;
