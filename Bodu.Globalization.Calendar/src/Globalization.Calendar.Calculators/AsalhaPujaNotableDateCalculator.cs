// ---------------------------------------------------------------------------------------------------------------
// <copyright file="AsalhaPujaNotableDateCalculator.cs" company="PlaceholderCompany">
...
/// <summary>
/// Provides a calculator for determining the Gregorian date of Asalha Puja (Dharma Day) for a given year.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// This calculator returns the first full moon that falls on or after 15 June of the given year, which consistently matches the
/// Thai Asanha Bucha observance. The next-day observance that begins Vassa (Buddhist Lent) is provided separately by
/// <see cref="VassaStartNotableDateCalculator" />.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public sealed class AsalhaPujaNotableDateCalculator
	: INotableDateCalculator
{
	...
	public DateTime? GetDate(int year, SysGlobal.Calendar? calendar = null)
	{
		...
		DateTime? fullMoon = LunarPhaseCalculator.GetFullMoonOnOrAfter(new DateTime(year, 6, 15));
		if (fullMoon is null)
			return null;
