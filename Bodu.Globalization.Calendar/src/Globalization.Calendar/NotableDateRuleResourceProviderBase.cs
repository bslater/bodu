// ---------------------------------------------------------------------------------------------------------------
// <copyright file="NotableDateRuleResourceProviderBase.cs" company="Bodu Pty. Ltd.">
...
/// <see cref="INotableDateRuleProvider" /> implementations such as <see cref="XmlResourceNotableDateRuleProvider" />
/// and <see cref="JsonResourceNotableDateRuleProvider" />.
/// </summary>
/// <remarks>
/// <para>
/// The base class owns the entire cherry-pick resolution algorithm: it loads the configured root resource, recursively
/// walks every <c>UseFrom</c> directive, applies per-directive overrides via <see cref="NotableDateRuleMerger" />, and
/// produces a single flattened rule list that locally declared rules can override.
/// </para>
/// <para>
/// Resource lookup walks the configured assembly chain in order: the first assembly containing a requested resource
/// wins. This lets a companion data pack be layered over the main library so that a <c>UseFrom</c> directive can
/// resolve targets that live in a different assembly.
/// </para>
/// <para>
/// Document parsing dispatches by file extension: resources whose names end with <c>.json</c> (case-insensitive) are
/// parsed by <see cref="NotableDateRuleJsonParser" />; everything else falls back to
...
{
    /// <summary>
    /// The logical path of the root resource file that seeds the flatten pipeline.
    /// </summary>
    private readonly string _rootResourceName;

    /// <summary>
    /// The path resolver used to translate relative <c>UseFrom</c> paths into fully qualified resource names.
    /// </summary>
    private readonly IResourcePathResolver _resourcePathResolver;

    /// <summary>
    /// The ordered list of assemblies searched for embedded manifest resources during flattening; the first assembly
    /// containing a requested resource wins.
    /// </summary>
    private readonly IReadOnlyList<Assembly> _assemblies;

    /// <summary>
    /// Thread-safe lazy backing store for the fully flattened rule list; populated on first call to
    /// <see cref="LoadRules" />.
    /// </summary>
    ...
    /// </summary>
    /// <param name="rootResourceName">
    /// The logical resource path of the root payload (for example
    ...
    /// Thrown when <paramref name="rootResourceName" /> or <paramref name="resourcePathResolver" /> is
    /// <see langword="null" />.
    /// </exception>
    protected NotableDateRuleResourceProviderBase(
        ...
        Assembly? assembly)
        : this(rootResourceName, resourcePathResolver, [assembly ?? Assembly.GetExecutingAssembly()])
    {
    }
    ...
    /// </summary>
    /// <param name="rootResourceName">
    /// The logical resource path of the root payload. Must not be <see langword="null" />.
    /// </param>
    /// <param name="resourcePathResolver">
    /// The resolver used to translate relative <c>UseFrom</c> paths into fully qualified resource names. Must not be
    /// <see langword="null" />.
    ...
    /// </param>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="rootResourceName" />, <paramref name="resourcePathResolver" />, or
    ...
    /// <summary>
    /// Loads every configured resource and flattens the rule definitions — including cross-file <c>UseFrom</c>
    /// references — into a single rule list.
    /// </summary>
        ...
        HashSet<string> inProgress)
    {
        ...
        if (!inProgress.Add(resourceName))
            throw new InvalidOperationException(
                string.Format(
                    ...
                    CalendarResourceStrings.Op_Invalid_CircularReferenceInResource,
                    resourceName));
        ...
        try
        {
            ...
            foreach (NotableDateRuleUseGroup group in document.UseGroups)
            {
                ...
                foreach (NotableDateRuleUseDirective directive in group.Uses)
                {
                    ...
                    List<NotableDateRule> matches = FindSourceRules(sourceRules, directive.SourceRuleName);
                    if (matches.Count == 0)
                    {
                        throw new InvalidOperationException(
                            string.Format(
                                CultureInfo.InvariantCulture,
                                CalendarResourceStrings.Op_Invalid_RuleNotFoundInSource,
                                directive.SourceRuleName,
                                group.SourceResource,
                                resourceName));
                    }
                    ...
                    NotableDateRuleUseDirective directiveScalarsOnly = directive with { OverrideBody = null };
                    foreach (NotableDateRule sourceRule in matches)
                    {
            ...
            foreach (NotableDateRule rule in document.LocalRules)
            {
    ...
    /// The logical resource path to convert. Must not be <see langword="null" />, empty, or whitespace.
    /// </param>
    /// <returns>
    /// The manifest resource name with slashes replaced by dots and leading/trailing separators stripped.
    /// </returns>
    ...
    /// <summary>
    /// Returns every rule in <paramref name="sourceRules" /> whose canonical <see cref="NotableDateRule.Name" />
    /// matches <paramref name="name" /> (case-insensitive), preserving dictionary enumeration order.
    /// </summary>
    ...
    /// <see cref="NotableDateRuleMerger.Apply" />. The merge algorithm lives in a dedicated helper so it can be
    /// exercised in isolation without bootstrapping an assembly loader.
    /// </summary>
    ...
    /// <see cref="ParsedNotableDateDocument" />. Resources whose names end with <c>.json</c> (case-insensitive) are
    /// parsed by <see cref="NotableDateRuleJsonParser" />; everything else is parsed by
    /// <see cref="NotableDateRuleParser" />.
    /// </summary>
    /// <param name="resourceName">The fully-qualified embedded resource name.</param>
    /// <param name="documentCache">
    /// A per-call cache used to deduplicate resource loads when multiple <c>Use</c> directives reference the same
    /// source.
    /// </param>
        ...
        Dictionary<string, ParsedNotableDateDocument> documentCache)
    {
        ...
        if (documentCache.TryGetValue(resourceName, out ParsedNotableDateDocument? cached))
            return cached;
        ...
        using Stream stream = OpenManifestResourceStream(resourceName)
            ?? throw new FileNotFoundException(
                string.Format(
                    CultureInfo.InvariantCulture,
                    CalendarResourceStrings.IO_FileNotFound_EmbeddedXmlResource,
                    resourceName,
                    FormatAssemblyChain(_assemblies)));
    ...
    /// <summary>
    /// Walks the configured assembly chain in order and returns the first manifest stream found for the supplied
    /// resource name.
    /// </summary>
    /// <param name="resourceName">The fully-qualified manifest resource name.</param>
    /// <returns>
    /// The opened stream, or <see langword="null" /> when no assembly in the chain contains the resource.
    /// </returns>
    ...
    /// <summary>
    /// Formats the configured assembly chain as a comma-separated list of full names for inclusion in diagnostic
    /// messages.
    /// </summary>
    ...
    private static string FormatAssemblyChain(IReadOnlyList<Assembly> assemblies) =>
        assemblies.Count == 1
            ? assemblies[0].FullName ?? string.Empty
            : string.Join(", ", assemblies.Select(a => a.FullName ?? string.Empty));
    ...
    /// <returns>
    /// <see langword="true" /> when the resource name ends with <c>.json</c> (case-insensitive); otherwise
    /// <see langword="false" />.
    /// </returns>
