...
/// <summary>
/// A trust policy that admits plugin assemblies whose SHA-256 file hash exactly matches a value pinned by the consumer
/// under the candidate's assembly name.
/// </summary>
/// <remarks>
/// <para>
/// The consumer builds a dictionary mapping each trusted assembly name (matched case-insensitively against
/// <see cref="System.Reflection.AssemblyName.Name" />) to the exact SHA-256 digest of the bytes on disk. The loader has
/// already computed the candidate's hash by the time this policy runs; the policy simply asserts equality.
/// </para>
/// <para>
/// This policy gives byte-level tamper resistance without requiring the plugin to be strong-named. It does not
/// automatically roll with plugin versions — pinning a new hash when the plugin is updated is the consumer's
/// responsibility.
/// </para>
/// </remarks>
...
{
    /// <summary>
    /// A normalized, case-insensitive map from assembly name to the pinned SHA-256 digest.
    /// </summary>
    ...
    /// </summary>
    /// <param name="allowedHashesByAssemblyName">
    /// Dictionary of trusted assembly names mapped to their SHA-256 digests. Assembly names are compared
    /// case-insensitively. Must not be <see langword="null" />.
    /// </param>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="allowedHashesByAssemblyName" /> is <see langword="null" />.
    /// </exception>
