...
/// <summary>
/// Compiles the calendar documentation guide examples that are explicitly opted in, so a code sample cannot silently
...
/// this test references (the calendar engine, the regional data packs, the Builder, and the DependencyInjection
/// companion), so an opted-in block must be method-body statements that resolve against those references.
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    /// <summary>
    /// Verifies that every opted-in calendar guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void CalendarGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
    ...
    /// calendar guides directory beneath it.
    /// </summary>
    ...
    private static string? FindGuidesDirectory()
    {
        for (DirectoryInfo? dir = new(AppContext.BaseDirectory); dir is not null; dir = dir.Parent)
        {
            ...
            {
                string guides = Path.Combine(dir.FullName, "docs", "guides", "calendar");
    ...
    /// </summary>
    /// <param name="snippet">The snippet body (method-body statements).</param>
