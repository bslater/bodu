...
/// <summary>
/// Identifies a single <see cref="INotableDateAlgorithm" /> implementation under test by pairing a short human-readable
/// label with a factory delegate that constructs a fresh instance per row.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// The delegate is invoked on every row so test isolation is preserved — no shared instance is mutated across rows,
/// which keeps the per-instance caches inside algorithms such as <see cref="EasterSundayNotableDateAlgorithm" /> from
/// leaking state between assertions.
/// </para>
/// </remarks>
public sealed record AlgorithmFactoryCase : IKat
{
    /// <inheritdoc />
    /// <remarks>
    /// Returns <see cref="Algorithm" /> so the row participates in <see cref="KatDisplayName" />-driven formatting
    /// alongside the existing <see cref="NotableDateAlgorithmKnownAnswers.GetDisplayName" /> path.
    /// </remarks>
    string IKat.Name => Algorithm;

    /// <summary>
    /// Gets the short label rendered by <see cref="NotableDateAlgorithmKnownAnswers.GetDisplayName" /> for rows
    /// produced from this case — for example <c>"Losar"</c>, <c>"Easter"</c>, or <c>"HinduLunar(Diwali)"</c>.
    /// </summary>
