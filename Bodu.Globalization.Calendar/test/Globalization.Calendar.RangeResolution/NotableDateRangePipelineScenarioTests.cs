// ---------------------------------------------------------------------------------------------------------------
// <copyright file="NotableDateRangePipelineScenarioTests.cs" company="Bodu Pty. Ltd.">
...
public sealed partial class NotableDateRangePipelineScenarioTests
{
    ...
    public void Resolve_WhenWholeYearRequestedWithMixedRuleSet_ShouldEmitEverythingExactlyOnce()
    {
        ...
        var names = resolved.Select(n => n.Name).ToArray();
    ...
    {
        var needsAlgorithm = rules.Any(r => r.Strategy == DateResolutionStrategy.Algorithm);
        ...
        return new NotableDateService(
            ruleProviders: [(INotableDateRuleProvider)new InMemoryRuleProvider(rules)],
