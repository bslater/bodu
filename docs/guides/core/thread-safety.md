...
| <xref:Bodu.Collections.Generic.Concurrent.ConcurrentLruCache`2> | **Safe** — lock-free reads | Point-in-time snapshot of the backing dictionary; order unspecified | Reads take no lock; queue maintenance is amortized onto writers. `GetOrAdd` follows `ConcurrentDictionary` semantics — racing callers may each run the factory, so it is *not* a stampede guard. `Count` may transiently exceed `Capacity` by at most the number of in-flight writers. [Guide](concurrent-collections.md) |
