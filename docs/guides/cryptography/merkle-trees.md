...
**Bodu.Security.Cryptography** ships two Merkle implementations, and the **Bodu.Collections** package ships a third, in its `Bodu.Collections.Merkle` namespace:
...
> [!TIP]
> If you want inclusion proofs, reach for <xref:Bodu.Collections.Merkle.Rfc6962MerkleTree> rather than assembling them by hand — see **[RFC 6962 Merkle trees and proofs](../core/rfc6962-merkle-trees.md)**. The hand-rolled approach in this section remains documented because it is the only way to obtain a proof over the *level-by-level* tree shape that `MerkleTreeHash` and `ParallelMerkleTreeHash` build.
