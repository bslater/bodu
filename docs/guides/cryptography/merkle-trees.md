...
**Bodu.Security.Cryptography** ships two Merkle implementations, and the **Bodu.Collections** package ships a third, in its `Bodu.Collections.Generic` namespace:
...
| <xref:Bodu.Collections.Generic.Rfc6962MerkleTree> — [guide](../core/rfc6962-merkle-trees.md) | RFC 6962 binary tree *(different package)* | Interoperating with a transparency log or any other RFC 6962 implementation, and the only one that produces **inclusion and consistency proofs**. Ships in the **Bodu.Collections** package, namespace `Bodu.Collections.Generic`, which depends only on Bodu.Core — so a consumer that needs commitments does not take the cipher catalogue with it. |
...
> [!TIP]
> If you want inclusion proofs, reach for <xref:Bodu.Collections.Generic.Rfc6962MerkleTree> rather than assembling them by hand — see **[RFC 6962 Merkle trees and proofs](../core/rfc6962-merkle-trees.md)**. The hand-rolled approach in this section remains documented because it is the only way to obtain a proof over the *level-by-level* tree shape that `MerkleTreeHash` and `ParallelMerkleTreeHash` build.
...
- <xref:Bodu.Collections.Generic.Rfc6962MerkleTree> · <xref:Bodu.Collections.Generic.MerkleBlocks> · <xref:Bodu.Security.Cryptography.MerkleTreeHash> · <xref:Bodu.Security.Cryptography.ParallelMerkleTreeHash>.
