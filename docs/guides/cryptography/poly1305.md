...
The type is <xref:Bodu.Security.Cryptography.Poly1305>. It sits on Bodu's <xref:Bodu.Security.Cryptography.KeyedBlockHashAlgorithm> — a <xref:System.Security.Cryptography.HashAlgorithm?displayProperty=nameWithType> that adds a `Key` property — rather than the BCL `KeyedHashAlgorithm`.
