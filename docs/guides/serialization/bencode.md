...
<xref:Bodu.Text.Bencode.BencodeSerializer> maps your types to and from [Bencode (BEP 3)](https://www.bittorrent.org/beps/bep_0003.html), the binary encoding used by BitTorrent, shaped after `System.Text.Json`.
...
For full control with no allocations, drive the <xref:Bodu.Text.Bencode.Reader.Utf8BencodeReader> / <xref:Bodu.Text.Bencode.Writer.Utf8BencodeWriter> ref-struct pair directly — the `Utf8JsonReader` / `Utf8JsonWriter` analogues. The reader accepts only canonical BEP 3 (no leading or negative zeros, ascending unique dictionary keys, a single root with no trailing bytes).
