...
<xref:Bodu.Formats.Excel.ExcelErrorCode> names the documented BIFF8 spreadsheet error codes. An undocumented code is surfaced as the raw byte cast to the enumeration, so compare against the named members before relying on the symbol.
...
<xref:Bodu.Formats.Excel.ExcelCell.IsDateFormatted> is set when the cell's number format is a date or time format (and date detection is enabled). <xref:Bodu.Formats.Excel.ExcelSerialDate.FromSerialDate(System.Double,Bodu.Formats.Excel.ExcelDateSystem)> converts the serial number to a <xref:System.DateOnly>; use <xref:Bodu.Formats.Excel.ExcelSerialDate.ToDateTime(System.Double,Bodu.Formats.Excel.ExcelDateSystem)> when you need the fractional time-of-day. Always pass the workbook's <xref:Bodu.Formats.Excel.ExcelBinaryWorkbook.DateSystem> — the 1900 and 1904 systems differ by 1,462 days.
...
<xref:Bodu.Formats.Excel.ExcelBinaryWorkbook.GetDateTime(Bodu.Formats.Excel.ExcelCell)> converts a numeric cell using the workbook's date system and returns `null` for any non-numeric cell, so it is safe to call on any cell. `GetNumberFormatCode` resolves a cell's `FormatIndex` to the format code string (built-in or custom), or `null` when the index is unknown.
