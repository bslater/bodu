...
| R20 | Add canonical checked-arithmetic helper to `Bodu.Core` substrate | `06-...duplication.md`; `F-003` | S | `Bodu.Core` + call sites | The one genuine substrate gap. Add `CheckedAdd`/`CheckedMultiply`/`ValidateRange` (size/offset guards) to `Bodu.Core` alongside `ThrowHelper`; adopt at input-controlled size/offset boundaries. Primary fix vehicle for R2 (CFB) and the general size/offset-overflow discipline. Public surface → goes through the API gate. |
