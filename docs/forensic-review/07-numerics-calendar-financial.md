...
| 2 | `Bodu.Financial.Currencies/CurrencyRegistry.cs:95,98` | Convention/Usability | Low | CONFIRMED → **resolved by documenting (not case-insensitive)** | Lookups use `StringComparer.Ordinal`, so `Get("usd")`/`TryGet` are case-sensitive — only canonical uppercase ISO codes resolve. Implementation review showed this is the **intended, uniform** contract: the exchange-rate providers already reject a lower-case code as malformed (`FixedDatedRateProviderTests`), so making the registry case-insensitive would create a worse inconsistency. Resolved by documenting the uppercase-only contract on `Get`/`TryGet` and pinning it with a characterization test — the comparer is unchanged. |
