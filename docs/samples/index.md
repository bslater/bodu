...
`Globalization.Recurrence/`, `IO.Biff/`, `IO.Compound/`, `IO.Pst/`, `IO.Hashing/`, `Numerics/`,
...
| [Formats.Excel](excel.md) | 1 project | The read-only BIFF5 and BIFF8 `.xls` reader over a real ~18,000-cell workbook: the session/sheet-directory surface, constant-memory forward-only streaming, the materialized worksheet, and format-classified serial-date decoding |
...
| [IO.Biff](io-biff.md) | 1 project | The BIFF record codec beneath the Excel reader: walking a workbook stream record by record with `BiffReader`, decoding cells and the shared string table, extracting the `Workbook` stream from an `.xls` with `Bodu.IO.Compound`, and authoring a BIFF8 sheet with `BiffWriter` that reads back through the codec |
