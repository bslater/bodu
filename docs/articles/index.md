...
- **[Bodu.Collections.Merkle — RFC 6962 trees, inclusion proofs, and consistency proofs](xref:Bodu.Collections.Merkle)**
  The Merkle Tree Hash exactly as RFC 6962 §2.1 specifies it — split at the largest power of two strictly below *n*, lone subtree roots promoted unchanged — in entry, fixed-size-block, and parallel-leaf-hashing modes, with authentication paths, consistency proofs, total (never-throwing) verifiers, and length-bound roots that close the standard's tree-size ambiguity. A namespace in the `Bodu.Collections` package, depending on `Bodu.Core` alone.
