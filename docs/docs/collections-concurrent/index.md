...
| <xref:Bodu.Collections.Generic.Concurrent.ConcurrentLruCache`2> | Read-optimized bounded cache in the BitFaster `ConcurrentLru` style: lock-free lookups over a `ConcurrentDictionary` with hot/warm/cold pseudo-LRU queues, write-amortized maintenance with over-capacity back-pressure, striped hit/miss/`HitRatio` telemetry, and a post-commit `ItemEvicted` event. Keys must be non-null (`TKey : notnull`). |
...
| Read-dominated cache where lookup latency and scalability rule | <xref:Bodu.Collections.Generic.Concurrent.ConcurrentLruCache`2> (lock-free reads, pseudo-LRU) |
| Hit-ratio telemetry for cache health dashboards | <xref:Bodu.Collections.Generic.Concurrent.ConcurrentLruCache`2> (`HitCount` / `MissCount` / `HitRatio`) |

## Design notes

- **Lock-free ring, lock-striped set and cache, lock-free-read LRU.** The buffer coordinates producers and consumers with per-slot sequence numbers (CAS updates, 64-byte cache-line padding against false sharing) — no `lock` on the hot path. The set stripes its buckets across independently locked regions; the evicting dictionary stripes its capacity across independently locked policy segments; the LRU cache keeps its entire read path lock-free and amortizes queue maintenance onto writers.
