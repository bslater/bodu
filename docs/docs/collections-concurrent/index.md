...
- **Lock-free ring, lock-free set.** The buffer coordinates producers and consumers with per-slot sequence numbers (CAS updates, 64-byte cache-line padding against false sharing); the set applies single-CAS mutations to a split-ordered list. Neither type takes a `lock` anywhere — a preempted thread can never stall the others.
