...
TOML has no null, so a `null` member is omitted on write by default. The full per-type catalog lives in the [type-mapping table](../../guides/serialization/toml.md) and the [built-in converter catalog](../../guides/serialization/builtin-converters.md).
...
- **[Core concepts](concepts.md)** — the family vocabulary, including the full TOML value-mapping table.
- **[Getting started](getting-started.md)** — install and the first round trip.
- **[Using TOML](../../guides/serialization/toml.md)** — worked patterns: type mapping, spec-version selection, both DOMs, raw tokens.
