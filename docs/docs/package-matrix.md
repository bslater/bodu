...
| Package | Status | Purpose | Depends on |
|---|---|---|---|
| `Bodu.IO.Compound` | Stable | Read-only reader for the OLE2 / Compound File Binary (CFB) container — the structured-storage envelope used by legacy Office files. Exposes the embedded named streams with no application-format knowledge. | `Bodu.Core` |
