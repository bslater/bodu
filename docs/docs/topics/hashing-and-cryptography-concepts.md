...
| Bodu.Collections.Generic *(the Merkle types, in Bodu.Collections)* | [Merkle commitments](../collections/concepts.md) — the Merkle Tree Hash, the split point, domain separation and the attacks it stops, inclusion and consistency proofs, the tree-size ambiguity and length-bound roots, and total verification. |
