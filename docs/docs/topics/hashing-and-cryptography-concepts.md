...
| Bodu.Collections.Merkle | [Bodu.Collections.Merkle — Core concepts](../collections-merkle/concepts.md) — the Merkle Tree Hash, the split point, domain separation and the attacks it stops, authentication paths and consistency proofs, the tree-size ambiguity and length-bound roots, total verification, and the logarithmic streaming fold. |
