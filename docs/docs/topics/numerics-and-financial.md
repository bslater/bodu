...
The **Numerics & Financial** topic covers the `Bodu.Numerics` and `Bodu.Financial` package families, built around one idea: **exact arithmetic**. [`Bodu.Numerics`](../numerics/index.md) ships <xref:Bodu.Numerics.Fraction`1> for rational arithmetic with no binary-floating-point drift and <xref:Bodu.Numerics.Interval`1> for generic-math ranges with independent endpoint inclusivity, unbounded endpoints, and full set algebra — including the binary-result <xref:Bodu.Numerics.IntervalPair`1>, the N-ary <xref:Bodu.Numerics.IntervalSet`1>, and the integer-domain <xref:Bodu.Numerics.DiscreteInterval`1>. [`Bodu.Financial`](../financial/index.md) builds the monetary layer on top — typed and runtime-tagged money, the ISO 4217 currency catalogue, fair allocation, and an exchange-rate provider stack with audit-grade provenance. [`Bodu.Financial.DependencyInjection`](../../guides/financial/dependency-injection.md) registers that stack in a `Microsoft.Extensions.DependencyInjection` container, `Bodu.Numerics.Serialization.Json` and `Bodu.Financial.Serialization.Json` add the `System.Text.Json` converters (both core libraries are serialization-agnostic), and the `Bodu.Financial.ExchangeRates` package family supplies the live web feeds and the caching / aggregation layer in front of them.
...
|---|---|---|---|
| `Bodu.Numerics` | Stable | `Fraction<T>` — canonical, GCD-reduced rational arithmetic with `BigInteger` intermediates, mixed-number and Unicode-vulgar-fraction formatting, continued fractions, best rational approximation. `Interval<T>` — closed / open / half-open / unbounded intervals with membership, intersection, union, adjacency, difference, and the `&` / `|` operators; plus the integer-domain `DiscreteInterval<T>`, the binary-result `IntervalPair<T>` / `DiscreteIntervalPair<T>`, and the N-ary `IntervalSet<T>`. Also `BigDecimal` (arbitrary-precision decimal), `Complex<T>` (generic complex numbers), and the streaming statistics aggregates `RunningStatistics<T>`, `RunningQuantile<T>`, `MovingSum<T>`, `MovingMinMax<T>`. Serialization-agnostic. | [Intro](../numerics/index.md) · [Concepts](../numerics/concepts.md) · [Get started](../numerics/getting-started.md) |
| `Bodu.Numerics.Serialization.Json` | Preview | `System.Text.Json` companion: `options.AddNumericsJsonConverters(...)` registers the converters and factories for `Fraction<T>`, `BigDecimal`, `Complex<T>`, `Interval<T>`, `DiscreteInterval<T>`, and `IntervalSet<T>` under a `NumericsJsonPolicy`. | [Guide](../../guides/numerics/json-serialization.md) |
| `Bodu.Financial` | Stable | `Money<TCurrency>` (compile-time currency) and `Money` (runtime-tagged), `MoneyBag` multi-currency portfolios, the ISO 4217 catalogue of 184 currency tags, fair allocation, cash rounding, and the exchange-rate core — timeless and dated provider contracts with provenance, series and in-memory tables. Serialization-agnostic; no HTTP machinery. | [Intro](../financial/index.md) · [Concepts](../financial/concepts.md) · [Get started](../financial/getting-started.md) |
| `Bodu.Financial.Serialization.Json` | Stable | `System.Text.Json` companion: `options.AddFinancialJsonConverters(policy)` registers the six converters (`Money<TCurrency>`, `Money`, `CalculatedMoney`, `MoneyBag`, `ExchangeRate`, `CurrencyPair`) under one of three `FinancialJsonPolicy` wire shapes (`Strict`, `Lenient`, `Compact`); `services.AddFinancialJson(policy)` registers a keyed `JsonSerializerOptions`. | [Reference](xref:Bodu.Financial.Serialization.Json) |
| `Bodu.Financial.ExchangeRates` | Preview | The web-provider machinery every live feed builds on: the abstract `WebRateProvider` / `PairWebRateProvider<TSeries>` bases, `WebRateProviderOptions`, single-flight fetch coalescing, and the on-disk raw-response cache base. | [Reference](xref:Bodu.Financial.ExchangeRates) |
| `Bodu.Financial.ExchangeRates.<Source>` | Stable | Eleven per-source live-feed packages over that machinery — `Boe`, `Ecb`, `Rba`, `Yahoo`, `Ofx`, `Xe`, `Oanda`, `Fixer`, `ExchangeRateHost`, `Fred`, `Imf` — each shipping its own `Add<Source>ExchangeRates` DI registration. | [Providers guide](../../guides/financial/exchange-rate-providers.md) |
| `Bodu.Financial.ExchangeRates.Caching` (+ `.Sqlite`, `.Distributed`) | Stable | Provider-agnostic read-through caching (`CachingRateProvider` over in-memory / TOML / JSON / SQLite / `IDistributedCache` backends) and multi-source grouping (`AggregatingRateProvider` — priority fallback, averaging, per-pair routing). | [Caching guide](../../guides/financial/exchange-rate-caching.md) |
| `Bodu.Financial.DependencyInjection` | Stable | `IServiceCollection` extensions: `AddFinancialService(...)`, the fluent `IFinancialServiceBuilder`, named monetary contexts, FX provider registration, and `FinancialOptions` binding. (JSON registration is `AddFinancialJson` in `Bodu.Financial.Serialization.Json`.) | [Guide](../../guides/financial/dependency-injection.md) |
