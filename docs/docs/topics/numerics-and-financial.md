...
| `Bodu.Numerics.Serialization.Json` | Stable | `System.Text.Json` companion: `options.AddNumericsJsonConverters(...)` registers the converters and factories for `Fraction<T>`, `BigDecimal`, `Complex<T>`, `Interval<T>`, `DiscreteInterval<T>`, and `IntervalSet<T>` under a `NumericsJsonPolicy`. | [Guide](../../guides/numerics/json-serialization.md) |
...
| `Bodu.Financial.ExchangeRates` | Stable | The web-provider machinery every live feed builds on: the abstract `WebRateProvider` / `PairWebRateProvider<TSeries>` bases, `WebRateProviderOptions`, single-flight fetch coalescing, and the on-disk raw-response cache base. | [Reference](xref:Bodu.Financial.ExchangeRates) |
