...
| **[Bodu.Text.Formats](formats/index.md)** | Self-framing text document formats with strongly-typed value models and span- and stream-friendly codecs. An umbrella over three sibling packages — **Delimited** (CSV / TSV), **DotEnv**, and **Ini** — each shipping a `Utf8…Reader` / `Utf8…Writer` pair, a `…Serializer` POCO mapper, and mutable and read-only DOMs, with strict invariant enforcement. | `net8.0` |
| **[Bodu.Text.Bencode](serialization/bencode/index.md)** · **[Bodu.Text.Toml](serialization/toml/index.md)** · **[Bodu.Text.Yaml](serialization/yaml/index.md)** | Three serializers that map your own types to and from a format — a shared architecture over the `Bodu.Text.Serialization` engine and a `System.Text.Json`-aligned shape, each shipping a `…Serializer`, a mutable `…Node` and a read-only `…Document` DOM, and a low-level `Utf8…Reader` / `Utf8…Writer` pair. **Bencode** covers BitTorrent BEP 3; **TOML** covers v1.0.0 / v1.1.0; **YAML** the 1.2 core schema with block / flow collections, anchors, and multi-document streams. See the [shared family introduction](serialization/index.md). | `net8.0` |
...
<div class="bodu-card">
  ...
  <p>The thread-safe collection companion — a lock-free MPMC <code>ConcurrentCircularBuffer&lt;T&gt;</code>, a lock-free split-ordered <code>ConcurrentHashSet&lt;T&gt;</code>, both with snapshot enumeration, and the lock-striped <code>ConcurrentEvictingDictionary&lt;TKey,TValue&gt;</code> bounded cache. Depends on <code>Bodu.Collections</code>.</p>
  <div class="bodu-card-links">
...
<div class="bodu-card">
  ...
  <p>Three serializers — POCO ↔ format — for Bencode (BEP 3), TOML, and YAML. A shared architecture over <code>Bodu.Text.Serialization</code>, each with a serializer, a mutable and a read-only DOM, and a low-level <code>Utf8…Reader</code> / <code>Utf8…Writer</code> pair.</p>
  <div class="bodu-card-links">
...
<div class="bodu-card">
  <h3><a href="io-biff/index.md">Bodu.IO.Biff</a></h3>
  <p>A low-level codec for the BIFF5 and BIFF8 record streams inside legacy <code>.xls</code> workbooks — the substrate beneath <code>Bodu.Formats.Excel.Binary</code>. Walk a workbook stream record by record with the allocation-free <code>BiffReader</code> and its typed accessors, decode the shared string table with <code>BiffSstReader</code>, and emit BIFF5 or BIFF8 records with <code>BiffWriter</code>; no compound-file dependency, no workbook or cell model.</p>
  <div class="bodu-card-links">
    <a href="io-biff/index.md">Introduction</a>
    <a href="io-biff/getting-started.md">Getting started</a>
...
- **MIT licensed**, with minimal external runtime dependencies — the core libraries depend only on the BCL; the dependency-injection, caching, JSON, and Outlook packages pull in a few permissively licensed Microsoft packages (see [Licensing](licensing.md)).
