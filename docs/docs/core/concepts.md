...
- **Parsing** — `WeekPattern.Parse("MTuWThF")` produces the weekday set; the format uses two-letter abbreviations for Tuesday, Thursday, and Sunday to disambiguate. Presets `WeekPattern.Empty`, `WeekPattern.Weekdays`, `WeekPattern.Weekend`, and `WeekPattern.AllDays` cover the common cases, and the regional working-week presets (`MondayToFriday`, `MondayToSaturday`, `SundayToThursday`, `SaturdayToWednesday`, …) mirror the members of <xref:Bodu.WorkingDaysOfWeek>.
