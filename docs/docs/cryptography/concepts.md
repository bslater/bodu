...
Input is split into fixed-size **leaves**; each leaf is hashed; pairs of digests are concatenated and re-hashed at each level until a single root remains, with a lone node promoted unchanged. **Fan-out** is the number of children per internal node — 2 by default, which is RFC 6962's tree; anything wider is an explicit non-RFC mode. Bodu's <xref:Bodu.Security.Cryptography.MerkleTreeHash> wraps any inner `HashAlgorithm` (SHA-256, Blake2b, …) as the leaf / node compressor; <xref:Bodu.Security.Cryptography.ParallelMerkleTreeHash> hashes leaves concurrently for high-throughput hashing of large inputs and folds them into the same tree.

Both types apply RFC 6962's **domain separation** (`0x00` for leaves, `0x01` for internal nodes) and build its **tree shape** — they are facades over the construction `Rfc6962MerkleTree` exposes in full — but neither produces a proof; see the [Merkle trees guide](../../guides/cryptography/merkle-trees.md) for the construction, parallel leaf hashing, the diagnostics recorder, and how a proof from `Rfc6962MerkleTree` verifies against their roots.
