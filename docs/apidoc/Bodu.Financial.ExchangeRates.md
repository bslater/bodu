...
**IMF (mapped pairs; SDMX Data API; keyless, daily, USD/SDR-anchored)**
...
- <xref:Bodu.Financial.ExchangeRates.ImfRateProviderOptions> — the SDMX data endpoint, dataflow, version, and the `SeriesMap` mapping each pair to a daily SDMX series key (built-in map for a set of USD pairs).
