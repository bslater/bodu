...
For self-framing document formats (CSV / TSV, DotEnv, INI), see the companion `Bodu.Text.Formats` umbrella package — namespaces <xref:Bodu.Text.Delimited>, <xref:Bodu.Text.DotEnv>, and <xref:Bodu.Text.Ini>. For `System.Text.Json`-style object mapping to TOML, Bencode, or YAML, see the <xref:Bodu.Text.Toml>, <xref:Bodu.Text.Bencode>, and <xref:Bodu.Text.Yaml> serializers.
...
- **Determinism and portability.** Encoders are deterministic — given a byte sequence and an option set they always produce the same canonical output across platforms and architectures, including the BCL-delegated Base64 path.
