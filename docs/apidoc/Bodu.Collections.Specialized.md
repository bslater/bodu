...
- <xref:Bodu.Collections.Specialized.MerkleBlockComputation> — the result of a block-mode pass: the `Root`, the `InputLength` and `BlockSize` it was computed over, and the ordered `LeafHashes`. Returning the leaf hashes from the same pass is the point — an authentication path needs them, and without them a large object would have to be streamed twice.
...
- <xref:Bodu.Collections.Specialized.MerkleTreeDiagnostics> / <xref:Bodu.Collections.Specialized.MerkleTreeDiagnosticNode> — an optional recorder that `ComputeRootOfBlocks` accepts and fills with every leaf and internal node the fold produces (`Level`, `Index`, `Hash`, `ChildHashes`), queried by level or as a whole, re-validated against a fresh `HashAlgorithm` with `Validate`, and dumped one node per line with `WriteTo`. A promoted node is recorded once, at the level it was produced. The same source compiles into `Bodu.Security.Cryptography`, where both hashers accept it, so a trace reads the same on either side of the package boundary.
