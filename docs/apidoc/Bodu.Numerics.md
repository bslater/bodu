...
- **Cross-package.** `Bodu.Financial.Money<TCurrency>` round-trips through `Fraction<BigInteger>` via `ToFraction()` / `FromFraction()` / `MultiplyExact()` for sub-minor-unit-precise monetary chains; see the [Bodu.Financial overview](~/apidoc/Bodu.Financial.md).
