...
**Bodu.Security.Cryptography** is a small, self-contained collection of managed block-cipher, cryptographic hash, and keyed-hash implementations that plug into the standard .NET cryptography contracts (<xref:System.Security.Cryptography.HashAlgorithm?displayProperty=nameWithType> and <xref:System.Security.Cryptography.SymmetricAlgorithm?displayProperty=nameWithType>).

Reach for this library when you need a tweakable block cipher that isn't in the BCL (Threefish), a collision-resistant keyed hash for hash-table protection (SipHash), a 64-bit-optimised cryptographic hash (Tiger), or a reference Skipjack implementation for research. For non-cryptographic checksums and hash-table hashes (Fletcher, Adler, FNV, CRC, CityHash, Pearson, Bernstein, etc.) see the companion **Bodu.IO.Hashing** package.
...
- <xref:Bodu.Security.Cryptography.SipHash128> — the 128-bit output variant of SipHash.
- <xref:Bodu.Security.Cryptography.Poly1305> — one-time MAC for authenticated encryption constructions.
...
- <xref:Bodu.Security.Cryptography.CubeHash> — Bernstein's SHA-3 candidate.
...
- **Security caveats.**
  ...
  - For error-detection and hash-table distribution (Fletcher, Adler, FNV, CRC, CityHash, Pearson, and the classic short hashes) use the non-cryptographic types in <xref:Bodu.IO.Hashing>.
