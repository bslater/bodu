...
- <xref:Bodu.Globalization.Calendar.NotableDateServiceOptions> — the options object that bundles the optional collaborators (algorithm registry, collision resolver, adjustment registries, code-first providers) when constructing a service.
...
- <xref:Bodu.Globalization.Calendar.NotableDateSequenceExtensions> — sequence transforms over resolved occurrences: `WithActualOccurrences()` expands observed-only results into the full actual + observed timeline (the consumer-side equivalent of an `ActualAndObserved` emission), preserving the standard `Resolve` ordering.
