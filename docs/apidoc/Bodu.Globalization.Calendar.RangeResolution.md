...
- **Reach envelope.** Each `RuleStaticProfile` records the worst-case day-delta envelope that the rule's adjustment chain can produce. The planner uses this to decide which adjacent civil years must be materialised — for example, a rule observed in January with a `MoveToNextMonday` adjustment may require materialising the previous December.
