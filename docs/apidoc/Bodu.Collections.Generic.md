...
- <xref:Bodu.Collections.Specialized> — the specialized structures that are not part of the general container catalogue: `BitSet` (packed bit set). It ships in `Bodu.Collections` alongside this namespace and needs only a second `using`.
