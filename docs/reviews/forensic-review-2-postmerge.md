...
> [!NOTE]
> **This is a point-in-time record, not a live action list.** The review was written against a
> `master` that predates the repository's history re-root, and its findings have not been
> re-verified wholesale against the current tree. The three High-severity items spot-checked when
> this document was recovered are all **since fixed**: H-1 (the GCM-SIV timing side-channel) now
> dispatches through a constant-time multiply, the calendar's untrusted-XML entry point now
> carries `XmlReaderSettings` with `MaxCharactersInDocument`, and the compound-file reader now
> caps stream payloads via `MaxVersion3StreamSize`. Treat any remaining item as a lead to confirm
> against today's source before acting on it.
