...
| 2 — Package landing pages and namespace overviews | **Done** (2026-09-09) | Every companion package now has a landing page in `docs/docs` and a TOC node: Outlook (trio), Recurrence (trio), `Bodu.Text.Serialization` (trio), `Bodu.Financial.ExchangeRates` (trio), both JSON companions (trios), calendar Caching (trio), calendar DI (two pages), Builder / Plugins / data packs / Tool+Build (one page each), the generators page plus a worked guide, and Delimited / DotEnv / INI landing stubs. All 35 missing namespace overviews are written and the debt list in `bld/docs-checks/namespace-overview-allowlist.txt` is empty. Writers compiled or ran the samples for Recurrence, the calendar companions, and the Text.Serialization / generator pages; the Outlook and financial samples were verified by signature only. Three source-contradicting claims found on the way were fixed in existing guides (caching backend composition, `AllowSynchronousNetworkAccess` default, which providers cache payloads). Remaining follow-up: `Bodu.Text.Formats.Generators` is documented as project-reference only until it is made packable. (The `Bodu.Numerics.Serialization.Json` csproj description — which claimed the interval pair types are serialized — was corrected in Phase 4.) |
| 3 — Guides | **Done** (2026-09-09) | All P1 and P2 guides written and wired into `guides/toc.yml`: binary formats (PST mail stores, reader options and limits, three IO.Pst guides, in-place CFB editing, custom property sets); financial (provider and caching `appsettings` configuration, custom web provider, rewritten testing guide, money helpers, failure modes); globalization (five Recurrence guides, cache backends and options, custom cache backend, the CLI and MSBuild tooling, async and localization); core and numerics (string, date, calendar-shape, sequence, numeric/enum/stream extensions, `Tree<T>`, `RingBackedCollection<T>`, thread-safety table, `Complex<T>`); text (STJ migration, options lifetime, YAML callbacks and polymorphism, writer and DOM options, line-format errors); hashing and cryptography (choosing a primitive, BCL interop, mode support matrix, Serpent, stream AEAD, streams and async, value types, security posture, extending, resumable hashing, alphanumeric check digits). Every writer compiled every fenced sample in a scratch project against the libraries and produced the quoted output by running them. Defects fixed in existing pages on the way: `WeekPattern.Parse` samples used an abbreviation form that throws (the grammar is a seven-character mask); AEAD `Encrypt(x, aad)` binds to the span-output overload and does not compile (named `associatedData:` now); the Threefish CTR sample threw on unaligned input; the caching guide's backend composition; the provider guide's `AllowSynchronousNetworkAccess` default and disk-cache claims. Library defects found and **not** fixed here: `CronExpression.ToString()` expands a star-leading step into a list that re-parses as a restricted field (documented as a warning in the cron guide). Two further claims from this audit were re-checked in Phase 4 and resolved: the `HashingStream` finding was **wrong** — its XML-doc example's `Crc32` is `System.IO.Hashing.Crc32`, which the file imports and the project references, and a probe compiled and ran it; the `Gumm` finding was **right** and is now fixed (see Phase 4). |
| 4 — Navigation parity | **Done** (2026-09-21) | All six items applied. **4.1** home page: `Bodu.IO.Pst` / Outlook / Recurrence cards were already present and correct, the Outlook card now links its own landing page instead of the PST one, the drifted partial install block (22 of 60 packages) is replaced by a link to `package-matrix.md#install-commands`, and a **Companion packages** strip maps each family to its opt-in companions. **4.2** `introduction.md` gains Recurrence, `Bodu.IO.Pst`, and `Bodu.Formats.Outlook` table rows plus matching cards, and the dependency paragraph now states the Outlook chain and Recurrence's independence; `getting-started.md` gains a Recurrence install line and five new subsections (Recurrence, `Bodu.IO.Biff`, `Bodu.Formats.Excel.Binary`, `Bodu.IO.Pst`, `Bodu.Formats.Outlook`). Every new snippet was compiled against the libraries in a probe project; the Recurrence one was run and its quoted output is the measured output. **4.3** the docs-side hub package tables were already complete; the real defect was the reverse — seven package guide indexes claimed the topic hub carries “every guide in this topic”, which was false for five of the seven topics (146 guides were unlinked from their hub). Rather than turn each hub into a 200-line link dump, the hubs are now described accurately as topic maps and each carries a **Complete guide indexes** line, and the two per-package indexes with genuine gaps were filled (`financial/index.md` × 4, `formats/index.md` × 1). **4.4** the four “Core documentation” footers on collection guides (`bit-set`, `bi-dictionary`, `table`, `layered-and-defaulting-dictionaries`) now point at the `Bodu.Collections` introduction, matching `graphs.md` / `multiset.md`; the folder was left as `guides/core/` — splitting it would break every inbound link for no reader benefit. The `Bodu.Sequences` / `Bodu.Collections.(Generic.)Extensions` and `Bodu.Text` nodes the audit asked for are already in the Core Foundations guide TOC; `Bodu.Xml.Linq` gets none, because it is one helper type (`XmlNamespaceResolver`) with no guide to link — it stays documented in the namespace map. **4.5** the Financial node in `guides/toc.yml` is restructured into per-package sub-nodes mirroring the Calendar node, with all 14 hrefs preserved. **4.6** the Tool `dotnet tool install` command, the `.Build` row, the Generators row, and the Testing package's `IsPackable=false` note were already in place; the combined “File formats and exchange-rate data” section is now split into **File formats** and **Exchange-rate providers and caching**, so the FX rows no longer sit under a file-format heading. Also fixed on the way: `Gumm`'s XML docs (the false `aa ↔ bb` twin-error claim — the sweep measures 400 of 900 missed, the weakest of the four decimal schemes) and the sentence in `alphanumeric-check-digits.md` that quoted it. Verified: `docfx --warningsAsErrors` clean, `check-docs.py all` ok, 5,098 relative links resolve with 0 missing, all 60 packages carry an install command with no phantoms, no HTML entities in hand-authored TOCs, folder/namespace alignment clean, and `dotnet build bodu.slnx` green. |
| 5 — Sustainment | **Mostly done** (2026-09-21) | **5.1** the snippet-compile guard now covers 24 test projects, up from 13. Thirteen were added — core/collections, cryptography, numerics, financial, the two JSON companions, recurrence, IO.Pst, text filtering, YAML, and Delimited / DotEnv / INI — holding **451** opted-in guide snippets to compiling against the public API they document. Marking is opt-in (`<!-- compile -->`), so an illustrative fragment stays out: an ambient identifier the prose supplies, elided arguments (`AddRange(/* observations */)`), a block mixing type declarations with statements, a snippet needing a package the test project does not reference, and — in `financial/money.md` — one that demonstrates a compile error deliberately. The JSON guides are compiled by their own companion's test project, which has the references. **Three real defects were found in guides that had never been compiled:** `financial/money.md` omitted `new` on five cash-rounding lines across two snippets, and used `runtime` two lines before declaring it; `core/sequence-operators.md`'s `b.Index()` is ambiguous against `System.Linq.Enumerable.Index<TSource>`, which .NET 9 added — harmless for a `net8.0` consumer, `CS0121` for a `net9.0`+ one, so the guide now shows the qualified call and says why. That collision is a **library-level** issue left unfixed: renaming or removing `IEnumerableExtensions.Index` is a breaking change. Two harness defects were fixed on the way rather than worked around, both cases where the guide was right and the tool was wrong: it dropped every `using` directive (so an alias such as `using SysComplex = System.Numerics.Complex;` could never compile) and now hoists them into the wrapper, leaving `using` *statements* in the body; and it compiled a snippet only as a method body, so any example introducing a type failed on syntax — it now tries the statement and declaration readings and reports whichever fits. **5.5** `bld/docs-checks/package-docs-map.txt` declares each package's landing page, guides, and samples page, and `check-docs.py packages` resolves all three; it fails on a packable package with no row, a row naming a page that does not exist, and a row outliving its package. Four reviewed absences are recorded with reasons rather than hidden (`Bodu.IO.Biff` has no guide folder; `Bodu.Text.Serialization` and the three Outlook packages have no samples page because no sample projects exist for them). **5.2–5.4** were already in CI. Remaining: the eleven test projects still without a snippet guard (the calendar companions, the data packs, the exchange-rate providers, `Bodu.Text.Serialization`, and Outlook — several of which have no guide folder to compile), and the samples pages the map now shows as missing. |
