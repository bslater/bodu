// ---------------------------------------------------------------------------------------------------------------
// <copyright file="TweakableSymmetricAlgorithmExtensionsTests.TryCreateEncryptor.cs" company="Bodu Pty. Ltd.">
...
public partial class TweakableSymmetricAlgorithmExtensionsTests
{
    ...
    public void TryCreateEncryptor_WhenTweakSizeIsInvalid_ShouldReturnFalseAndNullOutput()
    {
        ...
        byte[] key = new byte[algorithm.KeySize / 8];
        byte[] iv = new byte[algorithm.BlockSize / 8];
        byte[] tweak = new byte[(algorithm.TweakSize / 8) + 1]; // one bytes too long
