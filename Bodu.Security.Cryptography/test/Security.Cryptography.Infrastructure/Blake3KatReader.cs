...
/// records for the unkeyed hash. Each case specifies an <c>input_len</c>; the reference input is the repeating
/// 251-byte ramp <c>input[i] = i mod 251</c>, and the <c>hash</c> field is the extended (XOF) output. The reader
/// reconstructs the input and truncates the expected output to the requested length.
/// </summary>
public static class Blake3KatReader
{
    /// <summary>
    /// Reads the unkeyed-hash vectors from <paramref name="stream" />, yielding one <see cref="MessageDigestKnownAnswer" />
    /// per case with the expected digest truncated to <paramref name="outputBytes" />.
    /// </summary>
    /// <param name="stream">A readable stream over the BLAKE3 test_vectors.json source.</param>
    /// <param name="outputBytes">The number of leading output bytes to compare (BLAKE3's first 32 bytes are the standard 256-bit hash).</param>
