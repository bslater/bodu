// ---------------------------------------------------------------------------------------------------------------
// <copyright file="MLKem512Tests.KnownAnswerTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Locks <see cref="MLKem512" /> against the embedded NIST ACVP ML-KEM-512 known-answer vectors.
/// </summary>
public partial class MLKem512Tests
{
    /// <summary>
    /// Yields the ACVP key-generation vectors for ML-KEM-512.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> KeyGenVectors() =>
        MLKemAcvpVectors.KeyGen("ML-KEM-512");

    /// <summary>
    /// Yields the ACVP fixed-randomness encapsulation vectors for ML-KEM-512.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> EncapsulationVectors() =>
        MLKemAcvpVectors.EncapDecap("ML-KEM-512", "encapsulation");

    /// <summary>
    /// Yields the ACVP decapsulation vectors for ML-KEM-512, including modified-ciphertext implicit-rejection rows.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> DecapsulationVectors() =>
        MLKemAcvpVectors.EncapDecap("ML-KEM-512", "decapsulation");

    /// <summary>
    /// Yields the ACVP key-import validation vectors for ML-KEM-512.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> KeyCheckVectors() =>
        MLKemAcvpVectors.KeyCheck("ML-KEM-512");
    ...
    {
        MLKemAcvpVectors.AssertEncapsulation(MLKemAcvpVectors.ResolveParameters("ML-KEM-512"), vector);
    }
