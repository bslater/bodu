// ---------------------------------------------------------------------------------------------------------------
// <copyright file="HashAlgorithmKnownAnswers.cs" company="Bodu Pty. Ltd.">
...
/// in <see cref="HashAlgorithmSharedInputs" />. A <see langword="null" /> slot signals that no published value is
/// available for the algorithm under test and the harness skips that particular vector.
/// </para>
/// <para>
/// Vectors that fall outside the shared-input set — for example, Whirlpool's ISO test corpus or BLAKE2's keyed
/// reference suite — live on the <see cref="Additional" /> list and are driven by the same data-driven test harness,
/// avoiding bespoke <c>[DataRow]</c> definitions.
/// </para>
/// </remarks>
public sealed record HashAlgorithmKnownAnswers
{
    /// <summary>
    ...
    public IReadOnlyList<HashAlgorithmKnownAnswer> Additional { get; init; } =
        Array.Empty<HashAlgorithmKnownAnswer>();
}
