...
{
    /// <summary>
    /// Verifies that <see cref="CryptographyThrowHelper.ThrowIfOutputBufferTooSmall(Span{byte}, int, string)"/> does not throw
    ...
    public void ThrowIfOutputBufferTooSmall_WhenBufferIsExactSize_ShouldNotThrow()
    {
        ...
        CryptographyThrowHelper.ThrowIfOutputBufferTooSmall(buffer, 8);
    }
    ...
    /// <summary>
    /// Verifies that <see cref="CryptographyThrowHelper.ThrowIfOutputBufferTooSmall(Span{byte}, int, string)"/> throws an
    ...
    /// <summary>
    /// Verifies that <see cref="CryptographyThrowHelper.ThrowIfOutputBufferTooSmall(Span{byte}, int, string)"/> includes the
    ...
    public void ThrowIfOutputBufferTooSmall_WhenBufferTooSmall_ShouldIncludeRequiredSizeInMessage()
    {
        ...
        {
            Span<byte> buffer = stackalloc byte[4];
            CryptographyThrowHelper.ThrowIfOutputBufferTooSmall(buffer, 16);
        });
