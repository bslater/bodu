...
public partial class Rfc6962MerkleTreeTests
{
    ...
    public void Ctor_WhenFactoryDelegateIsNull_ShouldThrowArgumentNullException()
    {
        ...
        {
            _ = new Rfc6962MerkleTree((Func<HashAlgorithm>)null!);
        });
    }

    /// <summary>
    /// Verifies that a <see langword="null" /> typed factory is rejected with
    /// <see cref="ArgumentNullException" />.
    /// </summary>
    [TestMethod]
    public void Ctor_WhenTypedFactoryIsNull_ShouldThrowArgumentNullException()
    {
        _ = Assert.ThrowsExactly<ArgumentNullException>(() =>
        {
            _ = new Rfc6962MerkleTree((IHashAlgorithmFactory<HashAlgorithm>)null!);
        });
    }
