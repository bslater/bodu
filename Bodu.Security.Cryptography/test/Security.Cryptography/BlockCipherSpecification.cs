...
public sealed record BlockCipherSpecification
{
    ...
    /// <summary>
    /// Gets the expected key size in bytes.
    /// </summary>
    public required int KeySize { get; init; }

    /// <summary>
    /// Gets a deterministic test key of <see cref="KeySize" /> bytes used to construct cipher instances during tests.
    /// Defaults to <see langword="null" />, in which case the base test class generates an all-zero key of
    ...
    /// <summary>
    /// Gets a value indicating whether this cipher requires a tweak.
    /// </summary>
    public bool IsTweakable => TweakSize > 0;

    /// <summary>
    /// Gets a deterministic test tweak of <see cref="TweakSize" /> bytes, or all-zero when <see langword="null" />.
    /// Ignored for non-tweakable ciphers.
    /// </summary>
    ...
    /// <summary>
    /// Gets a value indicating whether the cipher supports in-place transformation where the input and output buffers
    /// overlap. Defaults to <see langword="false" />.
    /// </summary>
    public bool SupportsInPlaceTransform { get; init; } = false;

    /// <summary>
    /// Gets the input lengths used to exercise distinct internal algorithm paths during boundary tests. Defaults to a
    /// general set covering sub-block, exact-block, and multi-block lengths derived from <see cref="BlockSize" /> at
    /// runtime when <see langword="null" />.
    /// </summary>
    public IReadOnlyList<int>? BoundaryLengths { get; init; } = null;

    /// <summary>
    /// Gets the boundary lengths to use, falling back to a sensible default derived from <see cref="BlockSize" /> when
    /// <see cref="BoundaryLengths" /> is <see langword="null" />.
    /// </summary>
