...
{
    /// <summary>
    /// Verifies that the <see cref="Blake2b.Key" /> property returns an empty array when the instance is created
    /// without a key.
    /// </summary>
    [TestMethod]
    public void Key_WhenDefaultConstructed_ShouldReturnEmptyArray()
    {
        using var sut = new Blake2b(512);
    ...
    /// <summary>
    /// Verifies that assigning <see langword="null" /> to <see cref="Blake2b.Key" /> throws
    ...
    /// <summary>
    /// Verifies that assigning a key whose length exceeds <see cref="Blake2b.MaxKeySize" /> throws
    ...
    public void Key_WhenSetToKeyLongerThanMaxKeySize_ShouldThrowCryptographicException()
    {
        ...
        {
            sut.Key = new byte[Blake2b.MaxKeySize + 1];
        });
    }

    /// <summary>
    /// Verifies that assigning a key of exactly <see cref="Blake2b.MaxKeySize" /> bytes is accepted without
    /// throwing.
    /// </summary>
    [TestMethod]
    ...
    {
        using var sut = new Blake2b(512);
        byte[] key = new byte[Blake2b.MaxKeySize];
    ...
    /// <summary>
    /// Verifies that the <see cref="Blake2b.Key" /> getter returns a defensive copy so that mutations to the
    ...
    /// <summary>
    /// Verifies that assigning an empty array to <see cref="Blake2b.Key" /> clears the key and reverts the
    ...
    /// <summary>
    /// Verifies that assigning <see cref="Blake2b.Key" /> after hashing has begun throws
    ...
    /// <summary>
    /// Verifies that accessing <see cref="Blake2b.Key" /> on a disposed instance throws
    /// <see cref="ObjectDisposedException" />.
    /// </summary>
    [TestMethod]
    public void Key_WhenGetOnDisposedInstance_ShouldThrowObjectDisposedException()
    {
        var sut = new Blake2b(512);
        sut.Dispose();
    ...
    /// <summary>
    /// Verifies that assigning <see cref="Blake2b.Key" /> on a disposed instance throws
    /// <see cref="ObjectDisposedException" />.
    /// </summary>
    [TestMethod]
    ...
    public void ComputeHash_WhenKeyedAndUnkeyed_ShouldProduceDifferentDigests()
    {
        ...
        using (var unkeyed = new Blake2b(512))
            unkeyedHash = unkeyed.ComputeHash(message);

        using (var keyed = new Blake2b(512) { Key = key })
    ...
    {
        byte[] message = new byte[64];
        byte[] key1 = Enumerable.Range(0, 32).Select(i => (byte)i).ToArray();
        byte[] key2 = Enumerable.Range(0, 32).Select(i => (byte)(i ^ 0xFF)).ToArray();

        byte[] hash1;
        byte[] hash2;

        using (var sut1 = new Blake2b(512) { Key = key1 })
            hash1 = sut1.ComputeHash(message);

        using (var sut2 = new Blake2b(512) { Key = key2 })
    ...
    /// the BLAKE2b keyed MAC specification (RFC 7693 Section 2.8), where the key block alone constitutes the
    /// padded message.
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// Verifies that changing the key between successive <see cref="Blake2b.ComputeHash" /> calls updates the
    ...
    public void ComputeHash_WhenKeyChangedBetweenCalls_ShouldProduceDifferentDigests()
    {
        byte[] message = new byte[64];
        byte[] key1 = Enumerable.Range(0, 32).Select(i => (byte)i).ToArray();
        byte[] key2 = Enumerable.Range(0, 32).Select(i => (byte)(255 - i)).ToArray();
