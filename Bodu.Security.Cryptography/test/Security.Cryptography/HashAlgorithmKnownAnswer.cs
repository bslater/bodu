...
/// <summary>
/// Represents a single algorithm-specific known-answer test vector — a named input payload paired with the expected
/// hex-encoded digest produced by the algorithm under test.
/// </summary>
/// <remarks>
/// Instances populate the <see cref="HashAlgorithmKnownAnswers.Additional" /> collection and let concrete test classes
/// contribute algorithm-specific vectors (for example, the ISO test set for Whirlpool, or the Tiger designer's
/// <c>"Tiger"</c> input) without bespoke <c>[DataRow]</c> plumbing. Common shared inputs — empty, <c>"ABC"</c>, the
