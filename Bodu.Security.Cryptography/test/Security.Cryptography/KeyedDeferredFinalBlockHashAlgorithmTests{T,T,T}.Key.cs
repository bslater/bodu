// ---------------------------------------------------------------------------------------------------------------
// <copyright file="KeyedDeferredFinalBlockHashAlgorithmTests{T,T,T}.Key.cs" company="Bodu Pty. Ltd.">
...
public abstract partial class KeyedDeferredFinalBlockHashAlgorithmTests<TTest, TAlgorithm, TVariant>
{
    ...
    {
        using TAlgorithm sut = CreateAlgorithm();
        byte[] key = new byte[sut.MaximumKeySize / 8];
    ...
    {
        using TAlgorithm sut = CreateAlgorithm();
        int keySize = Math.Max(1, sut.MaximumKeySize / 16);
