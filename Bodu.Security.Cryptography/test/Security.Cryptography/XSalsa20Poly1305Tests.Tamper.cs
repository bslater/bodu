// ---------------------------------------------------------------------------------------------------------------
// <copyright file="XSalsa20Poly1305Tests.Tamper.cs" company="Bodu Pty. Ltd.">
...
public partial class XSalsa20Poly1305Tests
{
    ...
    {
        byte[] sealed_ = Seal(new byte[] { 1, 2, 3, 4 });
        sealed_[^1] ^= 0x80;
    ...
    /// <summary>
    /// Verifies that <see cref="IAeadBlockCipherModeTransform.Decrypt" /> throws <see cref="CryptographicException" />
    /// when a ciphertext byte has been altered.
    /// </summary>
    [TestMethod]
    ...
    {
        byte[] sealed_ = Seal(new byte[] { 9, 9, 9, 9, 9, 9, 9, 9 });
        sealed_[^1] ^= 0xff;
    ...
    private static byte[] Seal(ReadOnlySpan<byte> plaintext)
    {
