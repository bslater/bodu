...
/// <summary>
/// Drives <see cref="MessageDigestKnownAnswer" /> rows against the synchronous and asynchronous extension
...
public sealed class HashAlgorithmExtensionsKatTests
{
    ...
    [
        new()
        {
            Name = "SHA-256 empty",
            Message = [],
            Digest = Convert.FromHexString("E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855"),
        },
        new()
        {
            Name = "SHA-256 'abc'",
            Message = System.Text.Encoding.ASCII.GetBytes("abc"),
            Digest = Convert.FromHexString("BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD"),
        },
    ];

    /// <summary>
    /// Verifies that every <see cref="MessageDigestKnownAnswer" /> row passes
    ...
    {
        foreach (MessageDigestKnownAnswer kat in Kats)
        {
            using var algorithm = SHA256.Create();
            bool verified = algorithm.VerifyHash(kat.Message, kat.Digest);
    ...
    public async Task VerifyHash_Async_WhenInputMatches_ShouldReturnTrue()
    {
        ...
        {
            using var algorithm = SHA256.Create();
            using MemoryStream stream = new(kat.Message);

            bool verified = await algorithm.VerifyHashAsync(stream, kat.Digest).ConfigureAwait(false);
    ...
    public void VerifyHash_Sync_WhenDigestIsCorrupted_ShouldReturnFalse()
    {
        ...
        {
            using var algorithm = SHA256.Create();
            byte[] corruptedHash = (byte[])kat.Digest.Clone();
            corruptedHash[0] ^= 0x01;

            bool verified = algorithm.VerifyHash(kat.Message, corruptedHash);
