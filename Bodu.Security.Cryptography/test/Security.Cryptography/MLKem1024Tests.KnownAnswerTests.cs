// ---------------------------------------------------------------------------------------------------------------
// <copyright file="MLKem1024Tests.KnownAnswerTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Locks <see cref="MLKem1024" /> against the embedded NIST ACVP ML-KEM-1024 known-answer vectors.
/// </summary>
public partial class MLKem1024Tests
{
    /// <summary>
    /// Yields the ACVP key-generation vectors for ML-KEM-1024.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> KeyGenVectors() =>
        MLKemAcvpVectors.KeyGen("ML-KEM-1024");

    /// <summary>
    /// Yields the ACVP fixed-randomness encapsulation vectors for ML-KEM-1024.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> EncapsulationVectors() =>
        MLKemAcvpVectors.EncapDecap("ML-KEM-1024", "encapsulation");

    /// <summary>
    /// Yields the ACVP decapsulation vectors for ML-KEM-1024, including modified-ciphertext implicit-rejection rows.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> DecapsulationVectors() =>
        MLKemAcvpVectors.EncapDecap("ML-KEM-1024", "decapsulation");

    /// <summary>
    /// Yields the ACVP key-import validation vectors for ML-KEM-1024.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> KeyCheckVectors() =>
        MLKemAcvpVectors.KeyCheck("ML-KEM-1024");
