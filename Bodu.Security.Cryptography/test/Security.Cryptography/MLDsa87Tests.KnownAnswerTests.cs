// ---------------------------------------------------------------------------------------------------------------
// <copyright file="MLDsa87Tests.KnownAnswerTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Locks <see cref="MLDsa87" /> against the embedded NIST ACVP ML-DSA-87 known-answer vectors.
/// </summary>
public partial class MLDsa87Tests
{
    /// <summary>
    /// Yields the ACVP key-generation vectors for ML-DSA-87.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> KeyGenVectors() =>
        MLDsaAcvpVectors.KeyGen("ML-DSA-87");

    /// <summary>
    /// Yields the ACVP deterministic and hedged signature-generation vectors for ML-DSA-87.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> SigGenVectors() =>
        MLDsaAcvpVectors.SigGen("ML-DSA-87");

    /// <summary>
    /// Yields the ACVP signature-verification vectors for ML-DSA-87, including tampered rows.
    /// </summary>
    /// <returns>One row per vector.</returns>
    private static IEnumerable<object[]> SigVerVectors() =>
        MLDsaAcvpVectors.SigVer("ML-DSA-87");
