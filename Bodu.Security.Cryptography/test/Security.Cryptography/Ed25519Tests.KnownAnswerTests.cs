...
/// verification corpus embedded in the test assembly.
/// </summary>
public partial class Ed25519Tests
{
    ...
    /// <summary>
    /// Verifies that <see cref="Ed25519.SignData(ReadOnlySpan{byte})" /> reproduces the published RFC 8032 §7.1
    /// signatures and that each signature verifies under the published public key.
    /// </summary>
    [TestMethod]
    [DataRow(
    ...
    public void SignData_WhenGivenRfc8032Vector_ShouldProducePublishedSignature(
        string testName, string seedHex, string publicHex, string messageHex, string signatureHex)
    {
        _ = testName;
        ...
        signer.ImportPrivateKey(Convert.FromHexString(seedHex));

        CollectionAssert.AreEqual(Convert.FromHexString(publicHex), signer.ExportPublicKey());
        CollectionAssert.AreEqual(expectedSignature, signer.SignData(message));

        using var verifier = new Ed25519();
        verifier.ImportPublicKey(Convert.FromHexString(publicHex));
        Assert.IsTrue(verifier.VerifyData(message, expectedSignature));
    }
