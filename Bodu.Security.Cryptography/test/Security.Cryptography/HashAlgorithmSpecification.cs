...
public record HashAlgorithmSpecification
{
    ...
    /// <summary>
    /// Gets the expected <see cref="HashAlgorithm.InputBlockSize" /> in bytes.
    /// </summary>
    public int InputBlockSize { get; init; } = 1;

    /// <summary>
    /// Gets the expected <see cref="HashAlgorithm.OutputBlockSize" /> in bytes.
    /// </summary>
    public int OutputBlockSize { get; init; } = 1;

    /// <summary>
    /// Gets the expected <see cref="HashAlgorithm.CanReuseTransform" /> value. Defaults to <see langword="true" />.
    /// </summary>
    public bool CanReuseTransform { get; init; } = true;

    /// <summary>
    /// Gets the expected <see cref="HashAlgorithm.CanTransformMultipleBlocks" /> value. Defaults to
    /// <see langword="true" />.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// This value represents an algorithm-specific hashing boundary, such as a compression block, rate block, chunk, or
    /// leaf size. It is used by tests and metadata consumers to identify meaningful input-size boundaries for the
    /// algorithm.
    /// </para>
    /// <para>
    /// This value is intentionally separate from
    /// <see cref="System.Security.Cryptography.ICryptoTransform.InputBlockSize" />. For <see cref="HashAlgorithm" />
    /// implementations, <see cref="System.Security.Cryptography.ICryptoTransform.InputBlockSize" /> normally describes
    /// the streaming transform contract and is typically <c>1</c>, regardless of the algorithm's internal block size.
    /// </para>
    /// </remarks>
    ...
    /// well-defined internal path boundaries such as CityHash.
    /// </summary>
    ...
    /// Defaults to half of <c>HashSize / 8</c>, but may be set higher for algorithms with strong avalanche properties
    /// where full-byte entropy is expected.
    /// </summary>
    public int? MinNonZeroBytesForLongInput { get; init; } = null;

    /// <summary>
    /// Gets a value indicating whether the algorithm maintains state across transform operations. Stateless algorithms
    /// produce identical output for identical input regardless of prior hashing operations. Stateful algorithms must
