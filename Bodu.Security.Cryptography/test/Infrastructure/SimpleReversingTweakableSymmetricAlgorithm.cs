...
namespace Bodu.Security.Cryptography
{
        ...
        : TweakableSymmetricAlgorithm
    {
        ...
        {
            this.BlockSizeValue = DefaultBlockSizeBits;
            this.LegalBlockSizesValue = BlockSizesValue;

            this.KeySizeValue = DefaultKeySizeBits;
            this.LegalKeySizesValue = KeySizesValue;

            this.TweakSizeValue = DefaultTweakSizeBits;
            this.LegalTweakSizesValue = TweakSizesValue;

            this.FeedbackSizeValue = DefaultBlockSizeBits;
        ...
        public new static SimpleReversingTweakableSymmetricAlgorithm Create() => new();
        ...
        public override ICryptoTransform CreateEncryptor(byte[] rgbKey, byte[] rgbIV, byte[] tweak)
        {
            ...
            return new SimpleReversingCryptoTransform(
                CreateCipher(rgbKey, this.BlockSizeBytes, tweak),
                this.blockMode, this.PaddingValue, rgbIV, encrypt: true);
        }
        ...
        public override ICryptoTransform CreateDecryptor(byte[] rgbKey, byte[] rgbIV, byte[] tweak)
        {
            ...
            return new SimpleReversingCryptoTransform(
                CreateCipher(rgbKey, this.BlockSizeBytes, tweak),
                this.blockMode, this.PaddingValue, rgbIV, encrypt: false);
        }
        ...
        public override void GenerateTweak()
        {
            ...
            if (!this.ValidTweakSize(this.TweakSizeValue))
                throw new CryptographicException(
                    $"TweakSize ({this.TweakSizeValue} bits) is not a valid tweak size for this algorithm.");

            this.TweakValue = CryptoHelpers.GetRandomNonZeroBytes(this.TweakSizeBytes);
        }
        ...
        private int TweakSizeBytes => this.TweakSizeValue / 8;
        ...
        private void Validate(byte[] key, byte[] iv, byte[] tweak)
        {
            ...
            if (tweak.Length != this.TweakSizeBytes)
                throw new CryptographicException(
                    string.Format(ResourceStrings.CryptographicException_InvalidTweakSize,
                                  tweak.Length * 8,
                                  CryptoHelpers.FormatLegalSizes(this.LegalTweakSizesValue)),
                    nameof(tweak));
        }
