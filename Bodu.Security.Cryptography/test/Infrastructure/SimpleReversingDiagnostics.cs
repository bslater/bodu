// ---------------------------------------------------------------------------------------------------------------
// <copyright file="SimpleReversingDiagnostics.cs" company="PlaceholderCompany">
...
namespace Bodu.Security.Cryptography
{
    ...
    public sealed class SimpleReversingDiagnostics
    {
        ...
        /// </summary>
        public IReadOnlyList<BlockOperation> EncryptLog => this.encryptLog;

        /// <summary>
        /// Gets the sequence of block-level decrypt operations recorded by the cipher engine.
        /// </summary>
        public IReadOnlyList<BlockOperation> DecryptLog => this.decryptLog;

        /// <summary>
        /// Gets the sequence of <see cref="SimpleReversingCryptoTransform.TransformBlock" /> calls.
        /// </summary>
        public IReadOnlyList<BlockOperation> TransformBlockLog => this.transformBlockLog;

        /// <summary>
        /// Gets the sequence of <see cref="SimpleReversingCryptoTransform.TransformFinalBlock" /> calls.
        /// </summary>
        public IReadOnlyList<BlockOperation> TransformFinalBlockLog => this.transformFinalBlockLog;
        ...
        {
            this.encryptLog.Add(new BlockOperation(input, output, this.encryptLog.Count));
            this.TotalEncryptedBytes += input.Length;
        }
        ...
        {
            this.decryptLog.Add(new BlockOperation(input, output, this.decryptLog.Count));
            this.TotalDecryptedBytes += input.Length;
        }
        ...
        {
            this.transformBlockLog.Add(new BlockOperation(input, output, this.transformBlockLog.Count));
            this.TotalTransformBlockBytes += input.Length;
        }
        ...
        {
            this.transformFinalBlockLog.Add(new BlockOperation(input, output, this.transformFinalBlockLog.Count));
            this.TotalTransformFinalBlockBytes += input.Length;
        }
        ...
        public void Reset()
        {
            this.encryptLog.Clear();
            this.decryptLog.Clear();
            this.transformBlockLog.Clear();
            this.transformFinalBlockLog.Clear();
            this.TotalEncryptedBytes = 0;
            this.TotalDecryptedBytes = 0;
            this.TotalTransformBlockBytes = 0;
            this.TotalTransformFinalBlockBytes = 0;
        }
        ...
        public sealed class BlockOperation
        {
            ...
            public int InputLength => this.Input.Length;

            /// <summary>Gets the number of output bytes in this operation.</summary>
            public int OutputLength => this.Output.Length;

            /// <inheritdoc />
            public override string ToString() =>
                $"[{this.SequenceNumber}] in={Convert.ToHexString(this.Input)} → out={Convert.ToHexString(this.Output)}";
        }
    }
}