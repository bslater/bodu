...
namespace Bodu.Security.Cryptography
{
        ...
        : ICryptoTransform, IAsyncDisposable
    {
        ...
        /// </summary>
        public SimpleReversingDiagnostics Diagnostics => this.cipher.Diagnostics;
            ...
            int outputOffset)
        {
            ...
            if (this.encrypt)
            {
                bytesWritten = this.mode.Transform(input, output, true);
            }
            else
            {
                ...
                else
                {
                    ...
                    bytesWritten = this.mode.Transform(input.Slice(0, bytesToProcess), output.Slice(0, bytesToProcess), false);
                }
            }

            this.Diagnostics.RecordTransformBlock(input, outputBuffer.AsSpan(outputOffset, bytesWritten));
            return bytesWritten;
        }
        ...
        public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            else
            {
                ...
                if (combined.Length == 0)
                {
                    ...
                    if (this.padding is not NoPadding and not ZeroPadding)
                ...
                result = this.padding.Unpad(decrypted, this.cipher.BlockSize);
            }

            this.Diagnostics.RecordTransformFinalBlock(input, result);
            return result;
        }
