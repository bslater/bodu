...
namespace Bodu.Security.Cryptography
{
        ...
        : IBlockCipher
    {
        ...
        public SimpleReversingBlockCipher(byte[] key, int blockSizeBytes)
        {
            ...
            this.tweak = null;
            this.Diagnostics = new SimpleReversingDiagnostics();
        }
        ...
        /// </summary>
        public byte[]? Tweak => this.tweak is null ? null : (byte[])this.tweak.Clone();
        ...
        public void Encrypt(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            this.ValidateSpans(input, output);

            ReadOnlySpan<byte> block = input.Slice(0, this.BlockSize);
            Span<byte> dest = output.Slice(0, this.BlockSize);

            block.CopyTo(dest);

            // XOR with tweak before reversing (encrypt direction).
            if (this.tweak is { Length: > 0 })
                ApplyTweak(dest, this.tweak);

            dest.Reverse();

            this.Diagnostics.RecordEncrypt(block, dest);
        }
        ...
        public void Decrypt(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            this.Diagnostics.RecordDecrypt(block, dest);
        }
        ...
        public void Dispose()
        {
            ...
            {
                CryptographicOperations.ZeroMemory(this.tweak);
                this.tweak = null;
            }
        ...
        private void ValidateSpans(ReadOnlySpan<byte> input, Span<byte> output)
        {
            if (input.Length != this.BlockSize)
                throw new ArgumentException(
                    $"Input span must be exactly {this.BlockSize} bytes but was {input.Length}.", nameof(input));
            if (output.Length != this.BlockSize)
                throw new ArgumentException(
                    $"Output span must be exactly {this.BlockSize} bytes but was {output.Length}.", nameof(output));
        }
    }
}