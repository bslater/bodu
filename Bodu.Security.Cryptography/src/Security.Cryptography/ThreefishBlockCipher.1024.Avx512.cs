// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ThreefishBlockCipher.1024.Avx512.cs" company="Bodu Pty. Ltd.">
...
/// <see cref="Vector512{T}" /> rather than <see cref="Vector256{T}" />, so the Vector Length extensions are not
/// required.
/// </para>
/// </remarks>
public sealed partial class Threefish1024Cipher
{
    /// <summary>The per-round rotation vector for the first round of each four-round group.</summary>
    /// <remarks>
    ...
    /// <summary>The per-round rotation vector for the second round of each four-round group.</summary>
    private static readonly Vector512<ulong> s_rotVec1 = Vector512.Create((ulong)R8, R9, R10, R11, R12, R13, R14, R15);

    /// <summary>The per-round rotation vector for the third round of each four-round group.</summary>
    private static readonly Vector512<ulong> s_rotVec2 = Vector512.Create((ulong)R16, R17, R18, R19, R20, R21, R22, R23);

    /// <summary>The per-round rotation vector for the fourth round of each four-round group.</summary>
    private static readonly Vector512<ulong> s_rotVec3 = Vector512.Create((ulong)R24, R25, R26, R27, R28, R29, R30, R31);

    /// <summary>The per-round rotation vector for the fifth round of each eight-round cycle.</summary>
    private static readonly Vector512<ulong> s_rotVec4 = Vector512.Create((ulong)R32, R33, R34, R35, R36, R37, R38, R39);

    /// <summary>The per-round rotation vector for the sixth round of each eight-round cycle.</summary>
    private static readonly Vector512<ulong> s_rotVec5 = Vector512.Create((ulong)R40, R41, R42, R43, R44, R45, R46, R47);

    /// <summary>The per-round rotation vector for the seventh round of each eight-round cycle.</summary>
    private static readonly Vector512<ulong> s_rotVec6 = Vector512.Create((ulong)R48, R49, R50, R51, R52, R53, R54, R55);

    /// <summary>The per-round rotation vector for the eighth round of each eight-round cycle.</summary>
    private static readonly Vector512<ulong> s_rotVec7 = Vector512.Create((ulong)R56, R57, R58, R59, R60, R61, R62, R63);

    /// <summary>The forward lane-shuffle index vector applied to the even-position register between rounds.</summary>
    /// <remarks>
    ...
    /// <summary>The forward lane-shuffle index vector applied to the odd-position register between rounds.</summary>
    private static readonly Vector512<ulong> s_hiShuffleForward = Vector512.Create(4UL, 6, 5, 7, 3, 1, 2, 0);

    /// <summary>The inverse lane-shuffle index vector applied to the even-position register between inverse rounds.</summary>
    private static readonly Vector512<ulong> s_loShuffleInverse = Vector512.Create(0UL, 1, 3, 2, 7, 4, 5, 6);

    /// <summary>The inverse lane-shuffle index vector applied to the odd-position register between inverse rounds.</summary>
    private static readonly Vector512<ulong> s_hiShuffleInverse = Vector512.Create(7UL, 5, 6, 4, 0, 2, 1, 3);

    /// <summary>The deinterleave index vector that selects the even-position words from a loaded block.</summary>
    /// <remarks>
    ...
    /// <summary>The deinterleave index vector that selects the odd-position words from a loaded block.</summary>
    private static readonly Vector512<ulong> s_oddWordIndices = Vector512.Create(1UL, 3, 5, 7, 9, 11, 13, 15);

    /// <summary>The reinterleave index vector that assembles the first canonical half of the output block.</summary>
    /// <remarks>
    ...
    /// <summary>The reinterleave index vector that assembles the second canonical half of the output block.</summary>
    ...
    /// <summary>
    /// Executes one forward Threefish-1024 round on the vectorised state and applies the inter-round shuffle that
    ...
    /// <summary>
    /// Executes one inverse Threefish-1024 round on the vectorised state. The inverse shuffle runs before the UNMIX so
    /// it undoes the previous forward shuffle, restoring the layout the original MIX operated on.
    /// </summary>
    /// <param name="loVec">The even-position state register, updated in place.</param>
    /// <param name="hiVec">The odd-position state register, updated in place.</param>
    ...
    /// <summary>
    /// Reinterleaves the <c>(loVec, hiVec)</c> vectorised state back into the canonical sixteen-word byte order and
    /// writes it to the destination buffer as two unaligned 512-bit stores.
    /// </summary>
