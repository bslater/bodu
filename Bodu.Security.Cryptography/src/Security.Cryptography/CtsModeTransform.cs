...
/// <remarks>
/// <para>
/// CTS produces ciphertext of exactly the same length as the plaintext by "stealing" bytes from the
/// penultimate ciphertext block to complete the final partial block. For block-aligned inputs (where the
/// total length is a multiple of the block size), CTS behaves identically to CBC with no stealing.
/// </para>
/// <para>
...
/// </description></item>
/// <item><description>
/// Encrypt: the penultimate block is CBC-encrypted to E; the final partial block P_n is padded with
/// E[m:] and raw-encrypted to produce C_n (full block); C_{n-1} = E[0:m]. Output: C_n then C_{n-1}.
/// </description></item>
/// <item><description>
/// Decrypt: C_n is raw-decrypted to recover the padded P_n and E[m:]; C_{n-1} is reconstructed as
/// E[0:m] || reconstructed; then the full C_{n-1} is CBC-decrypted to P_{n-1}.
/// </description></item>
/// </list>
/// </para>
/// <para>
...
public sealed class CtsModeTransform : IBlockCipherModeTransform
{
    ...
    private readonly byte[] currentIv; // running CBC chaining vector
    ...
    private int EncryptCbc(ReadOnlySpan<byte> input, Span<byte> output)
    {
        ...
        for (int offset = 0; offset < input.Length; offset += blockSize)
        {
            ...
            for (int i = 0; i < blockSize; i++)
                block[i] = (byte)(inBlock[i] ^ this.currentIv[i]);

            this.cipher.Encrypt(block, outBlock);
    ...
    private int DecryptCbc(ReadOnlySpan<byte> input, Span<byte> output)
    {
        ...
        for (int offset = 0; offset < input.Length; offset += blockSize)
        {
            ...
            this.cipher.Decrypt(inBlock, block);
            for (int i = 0; i < blockSize; i++)
                outBlock[i] = (byte)(block[i] ^ this.currentIv[i]);
    ...
    private int EncryptCts(ReadOnlySpan<byte> input, Span<byte> output, int fullBlocks, int tailBytes, int blockSize)
    {
        ...
        for (int i = 0; i < blockSize; i++)
            xored[i] = (byte)(penultimate[i] ^ this.currentIv[i]);
        this.cipher.Encrypt(xored, e);
    ...
    private int DecryptCts(ReadOnlySpan<byte> input, Span<byte> output, int fullBlocks, int tailBytes, int blockSize)
    {
        ...
        Span<byte> block = stackalloc byte[blockSize];
        this.cipher.Decrypt(e, block);
        Span<byte> penultimateOut = output.Slice(bodyLength, blockSize);
        for (int i = 0; i < blockSize; i++)
            penultimateOut[i] = (byte)(block[i] ^ this.currentIv[i]);
