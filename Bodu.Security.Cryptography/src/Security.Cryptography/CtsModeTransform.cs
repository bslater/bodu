...
public sealed class CtsModeTransform : IBlockCipherModeTransform
{
    ...
    public CtsModeTransform(IBlockCipher _cipher, byte[] _iv)
    {
        ...
        if (_iv.Length != _cipher.BlockSize)
            throw new ArgumentException(
                $"IV length ({_iv.Length}) must equal the _cipher block size ({_cipher.BlockSize}).",
                nameof(_iv));

        this._iv = (byte[])_iv.Clone();
        this._currentIv = (byte[])_iv.Clone();
    }
