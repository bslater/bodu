...
/// <remarks>
/// <para>
/// HC-128 binds a 128-bit key and a 128-bit IV at construction and produces 32-bit keystream words from two
/// 512-word secret tables, <c>P</c> and <c>Q</c>. Like Rabbit it has no seekable block counter: the tables are large
/// evolving state and each step both updates one table entry and emits one keystream word, so the keystream must be
/// produced strictly in sequence — the contract the engine-owns-advancement <see cref="IStreamCipher" /> interface
/// expresses.
/// </para>
/// <para>
    ...
    : IStreamCipher
{
    ...
    /// <summary>The first SHA-256-style message-schedule function used during initialization.</summary>
    ...
    /// <summary>The second SHA-256-style message-schedule function used during initialization.</summary>
    /// <param name="x">The input word.</param>
    ...
    /// <summary>The <c>g1</c> mixing function used when updating the <c>P</c> table.</summary>
    ...
    /// <summary>The <c>g2</c> mixing function used when updating the <c>Q</c> table.</summary>
    /// <param name="x">The first input word.</param>
    /// <param name="y">The second input word.</param>
    /// <param name="z">The third input word.</param>
    /// <returns><c>((x &lt;&lt;&lt; 10) ^ (z &lt;&lt;&lt; 23)) + (y &lt;&lt;&lt; 8)</c>.</returns>
    ...
    /// <summary>The <c>h1</c> table-lookup function, indexing the <c>Q</c> table by the low and high-middle bytes of <paramref name="x" />.</summary>
    ...
    /// <summary>The <c>h2</c> table-lookup function, indexing the <c>P</c> table by the low and high-middle bytes of <paramref name="x" />.</summary>
    /// <param name="x">The word selecting the table indices.</param>
