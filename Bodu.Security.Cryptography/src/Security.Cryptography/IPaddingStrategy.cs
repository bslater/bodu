// ---------------------------------------------------------------------------------------------------------------
// <copyright file="IPaddingStrategy.cs" company="Bodu Pty. Ltd.">
...
/// <remarks>
/// <para>
/// Block ciphers typically require input to be a multiple of a fixed block size. This interface provides a strategy
/// pattern to apply or remove padding schemes such as PKCS#7, zero-padding, or no padding.
/// </para>
/// <para>
/// Implementations must ensure that padding is applied correctly and that unpadding validates or strips padding bytes
...
/// </para>
/// <para>
/// Most callers go through <see cref="System.Security.Cryptography.SymmetricAlgorithm.Padding" /> (which dispatches via
/// <see cref="PaddingFactory" />) rather than constructing implementations directly. Direct use is appropriate when
/// wiring custom transforms that do not extend <see cref="System.Security.Cryptography.SymmetricAlgorithm" />.
...
{
    /// <summary>
    /// Gets a value indicating whether <see cref="Unpad" /> inspects the final block and may return fewer bytes than it
    /// received. Self-describing schemes such as PKCS#7, ANSI X.923, ISO 10126 and ISO/IEC 7816-4 return
    /// <see langword="true" />; pass-through schemes such as zero padding and no padding return
    /// <see langword="false" />.
    /// </summary>
    /// <remarks>
    /// Streaming block-cipher transforms use this flag to decide whether the final ciphertext block must be deferred
    /// during decryption so that padding validation and removal can happen at the stream boundary.
    /// </remarks>
    ...
    /// </param>
    /// <returns>
    /// A new byte array containing the padded data. The byte length of the result will be a multiple of
    /// <paramref name="blockSize" /> / 8.
    /// </returns>
    ...
    /// </summary>
    /// <param name="input">
    /// The padded input data to unpad. Its byte length must be a multiple of <paramref name="blockSize" /> / 8.
    /// </param>
    /// <param name="blockSize">
    ...
    /// Thrown if the input does not conform to the expected padding scheme or its byte length is not a multiple of
    /// <paramref name="blockSize" /> / 8.
    /// </exception>
