...
/// <remarks>
/// <para>
/// This type exists so that a caller which has just streamed a large input past itself receives both the root
/// <em>and</em> the leaf hashes from the same pass. Generating an authentication path needs the leaf hashes; without
/// them the caller would have to stream the input a second time.
/// </para>
/// <para>
/// The leaf hashes cost <c>leafCount × hashLength</c> bytes to retain — for a 512 MiB input at one-mebibyte blocks
/// that is 512 hashes, or 16 KiB. A caller that needs only the root and not a path should use
...
public sealed class MerkleComputation
{
    ...
    /// <summary>Gets the tree's root hash — the unbound Merkle Tree Hash, not a length-bound root.</summary>
    /// <value>
    /// The root, which is the hash of zero bytes when <see cref="InputLength" /> is zero, and the sole leaf's hash
    /// when the input produced exactly one block.
    /// </value>
    /// <remarks>
    /// To publish a commitment that also pins the input's length, pass this value and
    /// <see cref="InputLength" /> to <see cref="Rfc6962MerkleTree.BindRoot(ReadOnlySpan{byte}, long)" />.
    /// </remarks>
    public byte[] Root { get; }

    /// <summary>Gets the total number of input bytes the tree was computed over.</summary>
    /// <value>The input length in bytes; zero for an empty input.</value>
    public long InputLength { get; }

    /// <summary>Gets the block size the input was divided into leaves by.</summary>
    /// <value>The block size in bytes.</value>
    public int BlockSize { get; }

    /// <summary>Gets the ordered leaf hashes, one per block.</summary>
    /// <value>
