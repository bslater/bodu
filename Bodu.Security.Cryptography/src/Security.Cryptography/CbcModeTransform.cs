// ---------------------------------------------------------------------------------------------------------------
// <copyright file="CbcModeTransform.cs" company="Bodu Pty. Ltd.">
...
/// <c>Pᵢ = D(Cᵢ) ⊕ Cᵢ₋₁</c>. See <b>panel 2</b> of the diagram above: the dashed feedback line feeds each ciphertext
/// block forward into the XOR that precedes the next encryption, and the IV supplies that feedback for the very first
/// block.
/// </para>
/// <para>
/// The initialization vector must equal the cipher block size in length and should be unpredictable for each message;
/// repeating an IV under the same key weakens confidentiality. The instance retains the most recent ciphertext block as
    ...
    : IBlockCipherModeTransform
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CbcModeTransform" /> class with the specified cipher and
