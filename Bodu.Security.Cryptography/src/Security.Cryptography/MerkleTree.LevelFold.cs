// ---------------------------------------------------------------------------------------------------------------
// <copyright file="MerkleTree.LevelFold.cs" company="Bodu Pty. Ltd.">
...
{
    /// <summary>
    /// Folds a stream of leaf hashes into a root level by level with a configurable fan-out, holding only the nodes
    /// still waiting for a full group at each level.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// A fan-out above two is a sound commitment of its own but is not RFC 6962's tree, which is binary by definition.
    /// The fold holds at most <c>fanOut − 1</c> nodes per level and one level per <c>log<sub>fanOut</sub></c> of the
    /// leaf count, so memory does not grow with the input.
    /// </para>
    /// <para>
    /// The fold owns neither the hash algorithm nor the recorder; the caller supplies both and this type is used from
    /// one thread at a time. Every root computation on the tree, and the <see cref="MerkleBlockAccumulator" />, reduce
    /// through it.
    /// </para>
    /// </remarks>
    internal sealed class LevelFold
    {
        ...
        /// <summary>The index the next hashed node at each level receives, reported to the recorder.</summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LevelFold" /> class.
        /// </summary>
        ...
        /// </param>
        internal LevelFold(HashAlgorithm hasher, int hashLength, int fanOut, MerkleTreeDiagnostics? diagnostics = null)
        {
            _hasher = hasher;
        ...
        {
            _diagnostics?.RecordLeaf(_leafCount, leafHash);
            _leafCount++;
        ...
        {
            if (_leafCount == 0)
                return HashEmpty(_hasher, _hashLength);
        ...
        /// <summary>
        /// Hashes a group of nodes at a level into their parent and records it.
        /// </summary>
        ...
        {
            byte[][] children = [.. nodes];
            byte[] parent = HashChildren(_hasher, _hashLength, children);
            ...
            if (_diagnostics is not null)
            {
                ...
                _diagnostics.RecordInternal(level + 1, _nextIndex[level + 1]++, children, parent);
            }
