...
/// <summary>
/// Implements the labeled extract-and-expand primitives <c>LabeledExtract</c> and <c>LabeledExpand</c> of RFC 9180 §4
/// and §5.1, which prefix every HKDF input with the protocol version label <c>"HPKE-v1"</c> and a suite identifier so
/// that derivations for different suites and purposes never collide.
/// </summary>
internal static class HpkeLabeledKdf
{
    ...
    /// <summary>
    /// Computes
    /// <c>LabeledExpand(prk, label, info, L) = Expand(prk, I2OSP(L,2) ‖ "HPKE-v1" ‖ suite_id ‖ label ‖ info, L)</c> ,
    ...
    /// <param name="output">
    /// The span that receives the output keying material; its length is the requested <c>L</c>.
    /// </param>
