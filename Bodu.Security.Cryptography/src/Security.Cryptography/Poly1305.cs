    ...
    : KeyedBlockHashAlgorithm<Poly1305>
{
    ...
    private readonly uint[] _r = new uint[5];    // Polynomial _key

    // Encrypted nonce
    private readonly uint[] _s = new uint[4];    // Precomputed 5 * _r[1..4]
    ...
    protected override void ProcessBlock(ReadOnlySpan<byte> block)
    {
        ...
        // Load _r and perform 130-bit polynomial multiplication: accumulator * _r
        ...
        ulong t00 = h0 * r0 + h1 * this._s[3] + h2 * this._s[2] + h3 * this._s[1] + h4 * this._s[0];
        ulong t01 = h0 * r1 + h1 * r0 + h2 * this._s[3] + h3 * this._s[2] + h4 * this._s[1];
        ulong t02 = h0 * r2 + h1 * r1 + h2 * r0 + h3 * this._s[3] + h4 * this._s[2];
        ulong t03 = h0 * r3 + h1 * r2 + h2 * r1 + h3 * r0 + h4 * this._s[3];
    ...
    protected override byte[] ProcessFinalBlock()
    {
        ...
        // Pack h into 128-bit output and add _s (the final _key part, RFC step 3)
        ...
        // reject subsequent streaming after the MAC has been produced. The _key itself
        // is retained until Dispose so that the framework'_s automatic re-initialisation
        // (invoked by ComputeHash via CaptureHashCodeAndReinitialize) can succeed without
        // tripping the _key-set check in Initialize.
        this.finalized = true;
        this.State = 2;
#endif
        // Key material is no longer needed — zero it immediately to enforce
        // Poly1305'_s one-time-use guarantee and limit _key exposure in memory.
    ...
    protected override void OnKeyChanged()
    {
        ...
        // the constructor'_s default-_key setup.
        Array.Clear(this._acc);

        ReadOnlySpan<byte> _key = this.KeyValue;

        // Load and clamp the first 128 bits of the _key as the polynomial '_r' _key Clamp '_r' by setting/clearing specific bits to avoid
        // vulnerabilities as per RFC 8439, Section 2.5.1
        uint t0 = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(0));
        uint t1 = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(4));
        uint t2 = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(8));
        uint t3 = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(12));

        // Split 128-bit _r into 5 x 26-bit limbs with clamping (see RFC for bitmask values)
        ...
        // Precompute 5*_r[i] values to optimize carry-reduction step later
        this._s[0] = this._r[1] * 5;
        this._s[1] = this._r[2] * 5;
        this._s[2] = this._r[3] * 5;
        this._s[3] = this._r[4] * 5;

        // Load the second half of the _key (_s), which will be added during final tag computation
        this._key[0] = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(16));
        this._key[1] = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(20));
        this._key[2] = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(24));
        this._key[3] = BinaryPrimitives.ReadUInt32LittleEndian(_key.Slice(28));
    }
}
