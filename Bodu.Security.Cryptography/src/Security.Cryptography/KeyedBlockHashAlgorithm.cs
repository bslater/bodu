    ...
    where T : KeyedBlockHashAlgorithm<T>, new()
{
    ...
    /// The fixed size of input blocks, in bits, that the algorithm processes. Must be a positive multiple of 8. This
    /// must match the internal block size used by the underlying hash structure.
    /// </param>
    /// <param name="keySize">
    ...
    /// <remarks>
    /// <para>
    /// The key must be set prior to calling any hashing methods such as
    /// <see cref="HashAlgorithm.ComputeHash(byte[])" />. The setter stores a private copy of the supplied array, then
    /// invokes <see cref="OnKeyChanged" /> so derived classes can rebuild any key-dependent internal state (for
    /// example, a polynomial key schedule or pre-computed state vectors).
    /// </para>
    /// <para>
    /// The getter returns a defensive copy of the stored key so external callers cannot mutate the internal
    /// representation.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Called after <see cref="KeyValue" /> has been assigned or the algorithm has been re-initialized. Derived classes
    /// override this to rebuild any key-dependent internal state such as a round-key schedule, precomputed vectors, or
    /// accumulator reset.
    /// </summary>
    /// <remarks>
    /// <para>
    /// By contract, when this method runs <see cref="KeyValue" /> is guaranteed to be non-<see langword="null" /> and
    /// of byte length <see cref="KeySizeValue" /> / 8. It is invoked from the <see cref="Key" /> setter, from
    /// <see cref="Initialize" />, and should be invoked by derived constructors after they have placed a default key
    /// into <see cref="KeyValue" />.
    /// </para>
    /// <para>
    /// The default implementation is a no-op so that derived classes with no key-dependent precomputation pay no
    /// overhead.
    /// </para>
    /// </remarks>
