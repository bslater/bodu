// ---------------------------------------------------------------------------------------------------------------
// <copyright file="BlockCipherModeFactory.cs" company="Bodu Pty. Ltd.">
...
public static class BlockCipherModeFactory
{
    ...
    private static void ValidateIv(string name, byte[]? iv, int requiredSize)
    {
        ...
        {
            throw new ArgumentException(
                string.Format(CultureInfo.CurrentCulture, CryptoResourceStrings.Arg_Invalid_IvLengthForMode, requiredSize, requiredSize / 8, iv.Length * 8, iv.Length),
                name);
        }
    }
}
