...
{
    private const int TotalWords = 16;                              // number of 32-bit words in the working _buffer.
    private const int Mask = TotalWords - 1;                        // bitmask to constrain index calculations to the _buffer length; inlined as an immediate by the JIT.
    ...
    private readonly uint[] _buffer = new uint[TotalWords];          // internal working _buffer used for permutation and round processing.
    private readonly uint[] _state;                                  // internal _state used to accumulate the hash output across input blocks.
