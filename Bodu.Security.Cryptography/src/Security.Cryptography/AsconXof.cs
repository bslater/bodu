    ...
    where T : AsconXof<T>, new()
{
    ...
    /// <summary>
    /// Initialises a new instance of the <see cref="AsconXof{T}" /> class with the specified algorithm parameters.
    /// </summary>
    ...
    public void Squeeze(Span<byte> output)
    {
        ...
        while (outOff < output.Length)
        {
            ...
            int toCopy = Math.Min(this._squeezeBufAvailable, output.Length - outOff);
    ...
    private void ProcessInputBlocks(ReadOnlySpan<byte> data)
    {
        ...
        {
            int needed = BlockSize - this._residualBytes;
