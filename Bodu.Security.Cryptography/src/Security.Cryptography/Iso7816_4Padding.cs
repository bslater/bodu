...
/// <c>0x80</c> and remaining pad bytes are <c>0x00</c>.
/// </summary>
/// <remarks>
/// <para>
/// A full block of padding is always added when the input is already block-aligned so that <see cref="Unpad" /> can
/// unambiguously recover the original length by locating the terminator. The scheme is widely used in smart-card
/// protocols, SHA-3/Keccak and CMAC. <see cref="Unpad" /> validates in constant time over the final block to resist
...
/// </remarks>
/// <example>
/// <code language="csharp"> using Bodu.Security.Cryptography; IPaddingStrategy padding = new Iso7816_4Padding(); byte[]
/// padded = padding.Pad(plaintext, blockSize: 128); // 128 bits = 16 bytes // padded ends with 0x80 followed by zero or
/// more 0x00 bytes. byte[] recovered = padding.Unpad(padded, blockSize: 128); </code>
/// </example>
