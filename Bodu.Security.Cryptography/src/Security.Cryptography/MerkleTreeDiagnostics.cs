// ---------------------------------------------------------------------------------------------------------------
// <copyright file="MerkleTreeDiagnostics.cs" company="PlaceholderCompany">
...
/// <summary>
/// Captures the complete node-by-node trace of a <see cref="ParallelMerkleTreeHash"/>
/// computation, and provides structural inspection and independent hash re-validation.
/// </summary>
/// <remarks>
/// <para>
...
/// to the number of internal nodes and the size of the hash output. This overhead is acceptable
...
public sealed class MerkleTreeDiagnostics
{
    ...
    /// <summary>Records an internal node produced by combining child hashes at the given source level.</summary>
    ...
    /// <summary>
    /// Independently re-computes every internal node's hash from its recorded child hashes and
    /// verifies the result matches the value stored in the diagnostic node.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The same <paramref name="algorithmFactory"/> used for the original computation should be
    /// supplied so that the hash algorithm, output size, and combination strategy all match.
    /// </para>
    /// <para>
    ...
    ///   Factory returning a fresh <see cref="HashAlgorithm"/> per call. Must not be
    ///   <see langword="null"/>.
    /// </param>
    /// <param name="errors">
    ///   On return, contains one entry per validation failure describing the node and the hash
    ///   mismatch. Empty when the method returns <see langword="true"/>.
    /// </param>
    /// <returns>
    ///   <see langword="true"/> if all internal node hashes are consistent with their recorded
    ///   children; <see langword="false"/> if any mismatch is found.
    /// </returns>
    ...
    /// <summary>
    /// Writes a formatted representation of the recorded tree to <paramref name="writer"/>,
    /// including per-node hashes, child references, and a validation summary.
    /// </summary>
    /// <remarks>
    /// The output is for diagnostic use only. Child references are resolved by matching recorded
    /// hash values, so an ambiguous match may occur if two nodes share the same hash.
    /// </remarks>
    /// <param name="writer">The destination writer. Must not be <see langword="null"/>.</param>
    /// <param name="algorithmFactory">
    ///   Optional factory used to run <see cref="Validate"/> and append a validation summary.
    ///   Pass <see langword="null"/> to skip validation output.
    /// </param>
    /// <exception cref="ArgumentNullException">
