        ...
        for (int index = 0; index < leafHashes.Count; index++)
        {
            byte[] leafHash = leafHashes[index]
                ?? throw new ArgumentException(
                    string.Format(
                        ...
                        CryptoResourceStrings.Arg_Invalid_MerkleLeafHashNullAtIndex,
                        index),
                    nameof(leafHashes));
            ...
            if (leafHash.Length != HashLength)
            {
                throw new ArgumentException(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        CryptoResourceStrings.Arg_Invalid_MerkleLeafHashLengthAtIndex,
                        index,
                        HashLength,
                        leafHash.Length),
                    nameof(leafHashes));
    ...
    private void ThrowIfNotHashLength(int length, string paramName)
    {
        if (length != HashLength)
        {
            throw new ArgumentException(
                string.Format(
                    CultureInfo.CurrentCulture,
                    CryptoResourceStrings.Arg_Invalid_MerkleHashLength,
                    HashLength,
                    length),
                paramName);
        }
