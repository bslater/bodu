// ---------------------------------------------------------------------------------------------------------------
// <copyright file="HashAlgorithmExtensions.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Extends <see cref="HashAlgorithm" /> with incremental input, async hashing of streams, and constant-time
/// verification against expected digests supplied as bytes, spans, memory, or hex strings.
/// </summary>
/// <remarks>
/// <para>
/// The <see cref="HashAlgorithm" /> base type exposes a low-level API: <c>TransformBlock</c> /
/// <c>TransformFinalBlock</c> for incremental input, and <see cref="HashAlgorithm.ComputeHash(byte[])" /> for one-shot
/// work. Production code that hashes streams, verifies downloads, or matches user-supplied digests ends up writing the
/// same boilerplate repeatedly: read the stream into a buffer, push it through the algorithm, and compare the result
/// with <see cref="CryptographicOperations.FixedTimeEquals(System.ReadOnlySpan{byte}, System.ReadOnlySpan{byte})" /> to
/// avoid timing leaks. This class collapses that boilerplate into a small, named API surface while keeping every
...
/// <c>NonCryptographicHashAlgorithmExtensions</c> companion in <c>Bodu.IO.Hashing</c>; both surfaces follow the same
