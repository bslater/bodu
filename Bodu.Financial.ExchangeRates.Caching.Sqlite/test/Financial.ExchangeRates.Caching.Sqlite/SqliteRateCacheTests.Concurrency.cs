// ---------------------------------------------------------------------------------------------------------------
// <copyright file="SqliteRateCacheTests.Concurrency.cs" company="Bodu Pty. Ltd.">
...
public sealed partial class SqliteRateCacheTests
{
    ...
    public async Task Store_WhenManyThreadsWriteDifferentPairsOnOneInstance_ShouldPersistEvery()
    {
        ...
        for (int i = 0; i < pairs.Length; i++)
        {
            ...
            writes.Add(Task.Run(() => cache.Store(pair, new[] { new CachedRate(date, rate, now) }, Duration, now)));
        }
        ...
        for (int i = 0; i < pairs.Length; i++)
        {
            IReadOnlyList<CachedRate> rows = cache.GetRates(pairs[i], Duration, now);
    ...
    public async Task GetRates_WhenReadConcurrentlyWithWrites_ShouldNeverThrowAndStayConsistent()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            work.Add(Task.Run(() =>
            {
                try
                {
                    cache.Store(Pair, new[] { new CachedRate(date, rate, now) }, Duration, now);
                }
                catch (Exception ex)
                {
                    errors.Add(ex);
