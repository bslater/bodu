// ---------------------------------------------------------------------------------------------------------------
// <copyright file="SqliteRateCacheTests.Logging.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies that <see cref="SqliteRateCache" /> reports its best-effort storage degradation through the supplied
...
public sealed partial class SqliteRateCacheTests
{
    ...
    public void Constructor_WhenDatabaseCorruptAndLoggerSupplied_ShouldLogWarningWithException()
    {
        ...
        var cache = new SqliteRateCache(
            new SqliteRateCacheOptions { Provider = Provider, DatabaseFilePath = path },
            timeProvider: null,
            logger: logger);
        _caches.Add(cache);
    ...
    public void Store_WhenDatabaseCorrupt_ShouldLogWarning()
    {
        ...
        clock.Advance(TimeSpan.FromMinutes(2));
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, clock.GetUtcNow()) }, Duration, clock.GetUtcNow());
    ...
    /// <summary>
    /// Verifies that, with <see cref="RateCacheOptions.ThrowOnStorageFailure" /> set, a storage failure
    ...
    public void Constructor_WhenDatabaseCorruptAndThrowOnStorageFailure_ShouldThrowAndNotLog()
    {
        ...
        {
            _ = new SqliteRateCache(
                new SqliteRateCacheOptions { Provider = Provider, DatabaseFilePath = path, ThrowOnStorageFailure = true },
                timeProvider: null,
                logger: logger);
        });
