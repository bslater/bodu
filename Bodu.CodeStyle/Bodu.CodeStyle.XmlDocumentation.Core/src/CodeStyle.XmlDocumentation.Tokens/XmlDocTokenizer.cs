...
internal static class XmlDocTokenizer
{
    ...
    /// <param name="end">When this method returns, the index immediately after the closing <c>]]&gt;</c>; otherwise zero.</param>
    /// <param name="text">When this method returns, the full CDATA section text including delimiters; otherwise an empty string.</param>
    /// <returns><see langword="true" /> if a complete CDATA section was read; otherwise <see langword="false" />.</returns>
    ...
    /// <param name="end">When this method returns, the index immediately after the closing <c>--&gt;</c>; otherwise zero.</param>
    /// <param name="text">When this method returns, the full comment text including delimiters; otherwise an empty string.</param>
    /// <returns><see langword="true" /> if a complete XML comment was read; otherwise <see langword="false" />.</returns>
    ...
    /// <summary>
    /// Attempts to read an XML tag beginning at the given position and classify it as a block start, block end,
    /// or inline atomic token.
    /// </summary>
    /// <param name="content">The content being tokenized.</param>
    /// <param name="start">The index of the candidate <c>&lt;</c> opener.</param>
    /// <param name="inlineTags">The set of element names treated as inline atomic tokens.</param>
    ...
    /// <summary>
    /// Normalizes whitespace within a raw tag, joining a multi-line tag onto a single line for the line-based
    /// composer unless attribute or cref preservation is requested.
    /// </summary>
    /// <param name="raw">The raw tag text as captured from the source.</param>
    /// <param name="preserveTagAttributes">When <see langword="true" />, the tag is returned verbatim including internal line breaks and alignment.</param>
    /// <param name="preserveCrefText">When <see langword="true" />, whitespace inside attribute values is preserved rather than collapsed.</param>
    ...
    /// <summary>
    /// Scans forward from the given position to the closing <c>&gt;</c> of the opening tag, skipping over
    ...
    /// <summary>
    /// Finds the start index of the last closing tag (<c>&lt;/</c>) in the raw text, skipping over quoted
    /// attribute values.
    /// </summary>
    /// <param name="raw">The raw tag text.</param>
    /// <param name="start">The index at which to begin scanning.</param>
    ...
    /// <summary>
    /// Appends the characters in the given range to the builder, dropping any carriage-return or line-feed
    /// characters.
    /// </summary>
    ...
    /// <summary>
    /// Collapses whitespace within a section of a tag, applying separate rules for whitespace inside and
    ...
    /// <param name="collapseOutsideQuotes">When <see langword="true" />, horizontal whitespace runs outside quotes collapse to a single space.</param>
    /// <param name="collapseInsideQuotes">When <see langword="true" />, horizontal whitespace inside quoted values collapses to a single space.</param>
    ...
    /// <returns><see langword="true" /> if the character may appear within a name; otherwise <see langword="false" />.</returns>
    ...
    /// </summary>
    /// <param name="ch">The character to test.</param>
    /// <returns><see langword="true" /> if the character may begin a name; otherwise <see langword="false" />.</returns>
    ...
    /// <summary>
    /// Determines whether the given character terminates an element name within a closing tag (that is, ends the
    /// name rather than continuing it).
    /// </summary>
    /// <param name="ch">The character to test.</param>
    /// <returns><see langword="true" /> if the character forms a close-tag name boundary; otherwise <see langword="false" />.</returns>
