...
{
    /// <summary>
    /// Strips a single physical documentation line down to its logical content: leading indentation, the <c>///</c>
    /// doc-comment prefix, and one following space are removed.
    /// </summary>
    ...
    /// Thrown when <paramref name="physicalLine" /> is <see langword="null" />.
    /// </exception>
    ...
    /// <summary>
    /// Splits the raw text between an element's start and end tags into its logical body lines: each physical line is
    /// stripped of its indentation and <c>///</c> prefix (and trailing whitespace), and leading and trailing blank
    /// lines are dropped so the caller alone controls the surrounding layout.
    /// </summary>
    /// <param name="rawBody">The raw inter-tag body text.</param>
    /// <returns>The logical body lines.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="rawBody" /> is <see langword="null" />.
    /// </exception>
    ...
    /// <summary>
    /// Canonicalizes the raw content of a documentation element to a single line: each physical line's indentation and
    /// <c>///</c> prefix is stripped, the lines are joined, and runs of whitespace are collapsed to a single space with
    /// the ends trimmed.
    /// </summary>
    ...
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="rawContent" /> is <see langword="null" />.
    /// </exception>
