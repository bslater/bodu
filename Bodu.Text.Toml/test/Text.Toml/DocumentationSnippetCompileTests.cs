...
/// <summary>
/// Compiles the TOML documentation guide examples that are explicitly opted in, so a code sample cannot
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    /// <summary>
    /// Verifies that every opted-in TOML guide snippet compiles against the current public API, with at
    /// least one snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void TomlGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
    ...
    /// TOML guides directory beneath it.
    /// </summary>
    ...
    private static string? FindGuidesDirectory()
    {
        for (DirectoryInfo? dir = new(AppContext.BaseDirectory); dir is not null; dir = dir.Parent)
        {
            ...
            {
                string guides = Path.Combine(dir.FullName, "docs", "guides", "serialization", "toml");
