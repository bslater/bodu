// ---------------------------------------------------------------------------------------------------------------
// <copyright file="TomlDocumentReaderTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies the behaviour of <see cref="TomlDocumentReader" />, the forward-only TOML token reader.
/// </summary>
[TestClass]
public sealed partial class TomlDocumentReaderTests
{
    ...
    {
        TomlDocumentReader reader = Create("i = 42\ns = \"x\"\nf = 1.5\nb = true\n");
    ...
    {
        TomlDocumentReader reader = Create("[owner]\nname = \"Tom\"\nage = 45\n");
    ...
    {
        TomlDocumentReader reader = Create("p = {x = 1}\n");
    ...
    {
        TomlDocumentReader reader = Create("[[products]]\nname = \"A\"\n\n[[products]]\nname = \"B\"\n");
    ...
    {
        TomlDocumentReader reader = Create("d = 1979-05-27T07:32:00Z\n");
    ...
    {
        TomlDocumentReader reader = Create("d = 1979-05-27T07:32:00\n");
    ...
    {
        TomlDocumentReader reader = Create("d = 1979-05-27\n");
    ...
    {
        TomlDocumentReader reader = Create("t = 07:32:10\n");
    ...
    /// <summary>
    /// Verifies that <see cref="TomlDocumentReader.Skip" /> over a table start advances to just past its matching end.
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// Verifies that <see cref="TomlDocumentReader.Skip" /> over an array start advances to just past its matching end.
    /// </summary>
    [TestMethod]
    public void Skip_WhenPositionedOnStartArray_ShouldAdvancePastArray()
    {
        TomlDocumentReader reader = Create("v = [1, [2, 3], 4]\nw = 5\n");
    ...
    /// <summary>
    /// Verifies that <see cref="TomlDocumentReader.Skip" /> over a property name advances past the property's value,
    ...
    /// <summary>
    /// Verifies that <see cref="TomlDocumentReader.Skip" /> is a no-op when the reader is on a scalar token.
    /// </summary>
    [TestMethod]
    ...
    public void GetInt64_WhenTokenIsString_ShouldThrowInvalidOperationException()
    {
        ...
        {
            TomlDocumentReader reader = Create("s = \"x\"\n");
            ExpectStartTable(ref reader);
            ExpectProperty(ref reader, "s");
            ExpectToken(ref reader, TomlTokenType.String);
            _ = reader.GetInt64();
    ...
    public void Constructor_WhenNestingExceedsMaxDepth_ShouldThrowTomlFormatException()
    {
        ...
        Assert.ThrowsExactly<TomlFormatException>(() =>
        {
            _ = new TomlDocumentReader(Encoding.UTF8.GetBytes("v = [[[1]]]\n"), options);
        });
    }
    ...
    {
        TomlDocumentReader reader = Create("d = 1979-05-27T00:32:00-07:00\n");
    ...
    {
        TomlDocumentReader reader = Create("v = [{x = 1}, {x = 2}]\n");
    ...
    private static TomlDocumentReader Create(string toml) =>
    ...
    /// <param name="toml">The TOML source text.</param>
    /// <returns>A reader positioned before the first token.</returns>
    private static TomlDocumentReader CreateV11(string toml) =>
    ...
    /// </remarks>
    private static void ExpectSingleValue(ref TomlDocumentReader reader, TomlTokenType expected)
    {
        ExpectStartTable(ref reader);
        ExpectProperty(ref reader, "v");
    ...
    /// </summary>
    /// <param name="reader">The reader to advance.</param>
    private static void ExpectStartTable(ref TomlDocumentReader reader) =>
        ExpectToken(ref reader, TomlTokenType.StartTable);
    ...
    /// </summary>
    /// <param name="reader">The reader to advance.</param>
    private static void ExpectEndTable(ref TomlDocumentReader reader) =>
        ExpectToken(ref reader, TomlTokenType.EndTable);
