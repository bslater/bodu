...
/// <see cref="char" /> — the key types whose text form round-trips cleanly through a TOML table key. The newer scalar
/// converters (<see cref="Version" />, <see cref="System.TimeSpan" />, <see cref="decimal" />, <see cref="Half" />, the
/// 128-bit integers) are a deliberate scope cut as key types: each would need its own key-conversion kind and parse
/// plumbing for marginal value. A dictionary with any other key type is not claimed by this factory and falls through
/// to the later converters.
/// </remarks>
