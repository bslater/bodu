...
/// <remarks>
/// TOML has no null token. A member whose value is <see langword="null" /> is omitted from the output by the enclosing
/// dictionary converter before the value converter is consulted, so this converter only ever reads or writes a present
/// value and forwards it to the underlying converter.
/// </remarks>
    ...
    where T : struct
{
    ...
    private readonly TomlConverter _inner;
    ...
    /// </exception>
    public NullableConverter(TomlConverter inner)
    {
