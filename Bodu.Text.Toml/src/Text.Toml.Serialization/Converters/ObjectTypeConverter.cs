...
/// runtime type selects the converter, and on read the value surfaces as a <see cref="TomlElement" />.
/// </summary>
/// <remarks>
/// <para>
/// A value whose runtime type is exactly <see cref="object" /> carries no data, so it writes as an empty inline table —
/// the TOML analogue of the empty JSON object <see cref="System.Text.Json.JsonSerializer" /> emits. Any other runtime
/// type dispatches to that type's resolved converter, so a boxed scalar, collection, dictionary, or plain object writes
/// exactly as it would when statically typed. A <see langword="null" /> value writes nothing, matching the omission of
/// null members TOML's missing null form imposes.
/// </para>
/// <para>
