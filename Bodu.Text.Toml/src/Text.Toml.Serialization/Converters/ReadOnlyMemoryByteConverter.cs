...
/// <summary>
/// Converts a <see cref="ReadOnlyMemory{T}" /> of <see cref="byte" /> to and from TOML using the byte-array
/// representation selected by <see cref="TomlSerializerOptions.ByteArrayHandling" />, mirroring the way
...
internal sealed class ReadOnlyMemoryByteConverter
    : TomlConverter<ReadOnlyMemory<byte>>
{
    /// <inheritdoc />
    public override ReadOnlyMemory<byte> Read(ref TomlDocumentReader reader, Type typeToConvert, TomlSerializerOptions options) =>
        new(ByteArrayConverter.ReadCore(ref reader));

    /// <inheritdoc />
    public override void Write(Utf8TomlWriter writer, ReadOnlyMemory<byte> value, TomlSerializerOptions options)
    {
