...
internal static class CookbookOverrideApplier
{
        ...
        foreach (CookbookOverride operation in overrides)
        {
            ...
            if (index < 0)
            {
                diagnostics.Add(new NotableDateValidationDiagnostic(
                    ...
                    "BODU-CAL2-OVERRIDE-ND",
                    string.Format(CultureInfo.InvariantCulture, Calendar2ResourceStrings.Validation_OverrideNotableDateNotFound, operation.NotableDateRef)));
                continue;
            }
    ...
    private static void AddRuleNotFound(ICollection<NotableDateValidationDiagnostic> diagnostics, string ruleRef, string notableDateId) =>
        diagnostics.Add(new NotableDateValidationDiagnostic(
            NotableDateValidationSeverity.Error,
            "BODU-CAL2-OVERRIDE-RULE",
            string.Format(CultureInfo.InvariantCulture, Calendar2ResourceStrings.Validation_OverrideRuleNotFound, ruleRef, notableDateId)));
}
