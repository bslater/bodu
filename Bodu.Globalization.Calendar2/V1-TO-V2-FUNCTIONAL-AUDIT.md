...
dates); the unverified Hindu festivals are documented as omitted in `region-in.xml` rather than
shipped with uncertain dates.
...
|---|---|
| `Date` (`DateTime`), `EndDate`, `DurationDays` | `Date` (`DateOnly`); multi-day spans deferred |
...
2a. ~~**Regional data packs**~~ — ✅ **done**: all three packs migrated for every v1 territory (38
    countries). Remaining polish: multi-day spans (`durationDays`), the twice-a-year Islamic case,
    and the unverified Hindu festivals.
