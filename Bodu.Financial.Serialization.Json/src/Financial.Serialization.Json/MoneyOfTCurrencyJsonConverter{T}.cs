    ...
    where TCurrency : ICurrency
{
    ...
    private static Money<TCurrency> ReadCompact(ref Utf8JsonReader reader)
    {
        if (reader.TokenType != JsonTokenType.String)
        {
            throw new JsonException(
                string.Format(
                    ...
                    FinancialJsonResourceStrings.Json_Invalid_ExpectedCompactString_MoneyOfTCurrency,
        ...
        return !Money<TCurrency>.TryParse(text.AsSpan(), CultureInfo.InvariantCulture, out Money<TCurrency> result)
            ? throw new JsonException(
                string.Format(
                    CultureInfo.CurrentCulture,
                    FinancialJsonResourceStrings.Json_Invalid_CompactMoneyOfTCurrencyForm,
                    text,
                    typeof(TCurrency).Name))
            : result;
    }
    ...
    private Money<TCurrency> ReadObject(ref Utf8JsonReader reader)
    {
        if (reader.TokenType != JsonTokenType.StartObject)
            throw new JsonException(FinancialJsonResourceStrings.Json_Invalid_ExpectedObject_MoneyOfTCurrency);
        ...
        return !string.Equals(currency, expectedIso, StringComparison.Ordinal)
            ? throw new JsonException(
                string.Format(
                    CultureInfo.CurrentCulture,
                    FinancialJsonResourceStrings.Json_Invalid_CurrencyMismatch,
                    currency,
                    expectedIso))
