...
public partial class ExchangeRateJsonConverterTests
{
    ...
    public void Strict_WhenUsingPairShorthandAndStringRate_ShouldResolve()
    {
        ...
        ExchangeRate restored = JsonSerializer.Deserialize<ExchangeRate>(json, OptionsFor(FinancialJsonPolicy.Strict));
    ...
    public void Strict_WhenPayloadMalformed_ShouldThrowJsonException(string json) =>
        Assert.ThrowsExactly<JsonException>(() => JsonSerializer.Deserialize<ExchangeRate>(json, OptionsFor(FinancialJsonPolicy.Strict)));
}
