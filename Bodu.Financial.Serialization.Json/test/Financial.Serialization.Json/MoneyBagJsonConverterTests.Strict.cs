...
public partial class MoneyBagJsonConverterTests
{
    ...
    {
        string json = JsonSerializer.Serialize(SampleBag(), OptionsFor(FinancialJsonPolicy.Strict));
        MoneyBag restored = JsonSerializer.Deserialize<MoneyBag>(json, OptionsFor(FinancialJsonPolicy.Strict))!;
    ...
    {
        string json = JsonSerializer.Serialize(new MoneyBag(), OptionsFor(FinancialJsonPolicy.Strict));
        MoneyBag restored = JsonSerializer.Deserialize<MoneyBag>(json, OptionsFor(FinancialJsonPolicy.Strict))!;
    ...
    {
        MoneyBag restored = JsonSerializer.Deserialize<MoneyBag>("{\"balances\":{\"USD\":\"10.50\"}}", OptionsFor(FinancialJsonPolicy.Strict))!;
    ...
    public void Strict_WhenPayloadMalformed_ShouldThrowJsonException(string json) =>
        Assert.ThrowsExactly<JsonException>(() => JsonSerializer.Deserialize<MoneyBag>(json, OptionsFor(FinancialJsonPolicy.Strict)));
}
