// ---------------------------------------------------------------------------------------------------------------
// <copyright file="WebExchangeRateProviderTests.GetLoadedSnapshot.cs" company="Bodu Pty. Ltd.">
...
public partial class WebExchangeRateProviderTests
{
    ...
    public void GetLoadedSnapshot_WhenCold_ShouldResolveNothing()
    {
        ...
        FixedDatedExchangeRateProvider snapshot = provider.GetLoadedSnapshot();

        Assert.IsFalse(snapshot.TryGetRate("AUD", "USD", Known, ExchangeRateLookupOptions.Exact, out _));
    }
    ...
    public async Task GetLoadedSnapshot_AfterLoad_ShouldServeLoadedRate()
    {
        ...
        FixedDatedExchangeRateProvider snapshot = provider.GetLoadedSnapshot();
        ExchangeRateLookupResult result = snapshot.GetRate("AUD", "USD", Known, ExchangeRateLookupOptions.Exact);
    ...
    public async Task GetLoadedSnapshot_WhenLoadedAfterAccess_ShouldNotMutateReturnedSnapshot()
    {
        ...
        FixedDatedExchangeRateProvider before = provider.GetLoadedSnapshot();
        await provider.LoadPairAsync("AUD", "USD", RangeStart, RangeEnd);

        Assert.IsFalse(before.TryGetRate("AUD", "USD", Known, ExchangeRateLookupOptions.Exact, out _), "the pre-load snapshot is immutable and unaffected by the fetch");
        Assert.AreNotSame(before, provider.GetLoadedSnapshot());
    }

    /// <summary>
    /// Verifies that the snapshot exposes the same book instance <see cref="WebExchangeRateProvider.GetLoadedBook" />
