...
public partial class MoneyTests
{
    ...
    public void Subtraction_WhenSameCurrency_ShouldReturnDifference() =>
        Assert.AreEqual(new Money(7m, "USD"), new Money(10m, "USD") - new Money(3m, "USD"));
    ...
    public void UnaryNegation_WhenHasCurrency_ShouldReverseSign() =>
        Assert.AreEqual(new Money(-5m, "USD"), -new Money(5m, "USD"));
    ...
    public void NamedMultiplyAndDivide_WhenInvoked_ShouldMatchOperatorsAndRounding()
    {
        ...
        Assert.AreEqual(new Money(0.12m, "USD"), new Money(1m, "USD").Divide(8m, MidpointRounding.ToEven));
        Assert.AreEqual(new Money(0.13m, "USD"), new Money(1m, "USD").Multiply(0.125m, MidpointRounding.AwayFromZero));
    }
    ...
    {
        var low = new Money(1m, "USD");
        var high = new Money(2m, "USD");
        var lowAgain = new Money(1m, "USD");
    ...
    {
        Assert.IsTrue(new Money(1m, "USD") != new Money(2m, "USD"));
        Assert.IsFalse(new Money(1m, "USD") != new Money(1m, "USD"));
    }
    ...
    {
        Assert.IsTrue(new Money(1m, "USD").Equals((object)new Money(1m, "USD")));
        Assert.IsFalse(new Money(1m, "USD").Equals((object)"USD"));
    }
    ...
    {
        var value = new Money(1m, "USD");

        Assert.AreEqual(1, value.CompareTo((object?)null));
        Assert.AreEqual(0, value.CompareTo((object)new Money(1m, "USD")));
        Assert.IsTrue(value.CompareTo((object)new Money(2m, "USD")) < 0);
    ...
    public void Sign_WhenInspected_ShouldReturnSignOfAmount(int amount, int expected) =>
        Assert.AreEqual(expected, new Money(amount, "USD").Sign);
}
