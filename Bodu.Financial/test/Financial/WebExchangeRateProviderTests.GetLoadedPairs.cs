...
public partial class WebExchangeRateProviderTests
{
    ...
    /// <see cref="IExchangeRatePairLoader" /> contract, so a caller can treat any provider uniformly without knowing the
    /// concrete type.
    /// </summary>
    [TestMethod]
    public async Task GetLoadedPairs_WhenAccessedThroughInterface_ShouldReportLoadedPair()
    {
        IExchangeRatePairLoader loader = CreateProvider();

        await loader.LoadPairAsync("AUD", "USD", RangeStart, RangeEnd);

        CollectionAssert.Contains(loader.GetLoadedPairs().ToList(), new ExchangeRatePair(CurrencyCode.AUD, CurrencyCode.USD));
    }
}
