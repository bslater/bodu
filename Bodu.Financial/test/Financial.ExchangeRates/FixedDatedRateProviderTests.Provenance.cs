// ---------------------------------------------------------------------------------------------------------------
// <copyright file="FixedDatedRateProviderTests.Provenance.cs" company="Bodu Pty. Ltd.">
...
public partial class FixedDatedRateProviderTests
{
    ...
    private static FixedDatedRateProvider StampedProvider()
    {
        RateTableBuilder builder = new();
        builder.Upsert(new CurrencyPair(CurrencyCode.USD, CurrencyCode.AUD), "RBA", s_d1, 1.50m, s_fetchedAt);
    ...
    {
        FixedDatedRateProvider table = StampedProvider();

        Assert.IsTrue(table.TryGetRate("USD", "AUD", s_d1, RateLookupOptions.Exact, out RateLookupResult result));
    ...
    public void TryGetRate_WhenInverseSeriesHasFetchInstant_ShouldStampServedRate()
    {
        ...
        Assert.IsTrue(table.TryGetRate("AUD", "USD", s_d1, RateLookupOptions.Exact, out RateLookupResult result));
    ...
    /// <summary>
    /// Verifies that <see cref="FixedDatedRateProvider.GetRatesAsync" /> stamps every rate in a resolved range
    ...
    {
        RateTableBuilder builder = new();
        builder.Upsert(new CurrencyPair(CurrencyCode.USD, CurrencyCode.AUD), "RBA", new DateOnly(2024, 1, 2), 1.49m, s_fetchedAt);
        builder.Upsert(new CurrencyPair(CurrencyCode.USD, CurrencyCode.AUD), "RBA", new DateOnly(2024, 1, 3), 1.50m, s_fetchedAt);
        FixedDatedRateProvider table = new(builder.ToBook());
    ...
    public void TryGetRate_WhenSameCurrencyIdentity_ShouldLeaveFetchInstantNull()
    {
        ...
        Assert.IsTrue(table.TryGetRate("USD", "USD", s_d1, RateLookupOptions.Exact, out RateLookupResult result));

        Assert.IsNull(result.Rate.FetchedAtUtc);
    }

    /// <summary>
    /// Verifies that a rate resolved directly from the table reports <see cref="RateOrigin.Live" /> provenance
    ...
    /// <summary>
    /// Verifies that the result returned by <see cref="FixedDatedRateProvider.GetRate" /> carries the same
    /// <see cref="RateOrigin.Live" /> provenance as the matching <c>TryGetRate</c> call.
    /// </summary>
    [TestMethod]
    public void GetRate_WhenResolvedFromTable_ShouldReportLiveProvenance()
    {
        ...
        RateLookupResult result = table.GetRate("USD", "AUD", s_d1, RateLookupOptions.Exact);
    ...
    /// <summary>
    /// Verifies that an inverse resolution reports <see cref="RateOrigin.Live" /> provenance attributed to the
    ...
    /// <summary>
    /// Verifies that a same-currency identity rate reports <see cref="RateOrigin.Live" /> provenance attributed
    /// to the synthetic <see cref="FixedDatedRateProvider.IdentityProviderName" /> source, with no cache fields
    /// populated.
    /// </summary>
    [TestMethod]
    public void TryGetRate_WhenSameCurrencyIdentity_ShouldReportLiveProvenanceWithIdentityProvider()
    {
        ...
        Assert.AreEqual(RateOrigin.Live, result.Provenance.Origin);
        Assert.AreEqual(FixedDatedRateProvider.IdentityProviderName, result.Provenance.Provider);
