// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateSeriesStorageTests.cs" company="Bodu Pty. Ltd.">
...
public partial class RateSeriesStorageTests
{
    ...
    private static RateObservation Obs(int dayNumber, decimal rate) =>
        new(DateOnly.FromDayNumber(dayNumber), rate);

    /// <summary>
    /// Verifies that <see cref="RateSeriesStorage.Create(IEnumerable{RateObservation}, string)" />
    ...
    public void Create_Observations_WhenEmpty_ShouldThrowArgumentException()
    {
        ExceptionAssert.ThrowsExactlyWithParamName<ArgumentException>(
            () =>
            {
                _ = RateSeriesStorage.Create(Array.Empty<RateObservation>(), ObservationsParam);
            },
            ObservationsParam);
    }
    ...
    /// <summary>
    /// Verifies that <see cref="RateSeriesStorage.Create(IEnumerable{ValueTuple{DateOnly, decimal}}, string)" />
