// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateSeriesBufferTests.ToStorage.cs" company="Bodu Pty. Ltd.">
...
{
    /// <summary>
    /// Verifies that <see cref="RateSeriesBuffer.ToStorage" /> on an empty buffer throws
    ...
    public void ToStorage_WhenBufferMutatedAfterCall_ShouldNotAffectSnapshot()
    {
        ...
        RateSeriesStorage snapshot = buffer.ToStorage();

        buffer.Upsert(1000, 99m, RateParam);
        buffer.Add(1010, 1.5m, RateParam, DateParam);

        RateObservation[] observations = snapshot.Enumerate().ToArray();
        Assert.HasCount(1, observations);
        Assert.AreEqual(1.4m, observations[0].Rate);
    }

    /// <summary>
    /// Verifies that <see cref="RateSeriesBuffer.FromStorage" /> copies every observation from the source
    /// storage into a new buffer.
    /// </summary>
    [TestMethod]
    public void FromStorage_WhenSeeded_ShouldCopyAllObservations()
    {
        var source = RateSeriesStorage.Create(
            [
                new RateObservation(DateOnly.FromDayNumber(1000), 1.4m),
                new RateObservation(DateOnly.FromDayNumber(1010), 1.5m),
                new RateObservation(DateOnly.FromDayNumber(1020), 1.6m),
            ],
            ObservationsParam);

        var buffer = RateSeriesBuffer.FromStorage(source);
    ...
    /// <summary>
    /// Verifies that a buffer seeded by <see cref="RateSeriesBuffer.FromStorage" /> can be mutated without
    ...
    public void FromStorage_WhenBufferMutated_ShouldNotAffectSourceStorage()
    {
        ...
        RateObservation[] sourceObservations = source.Enumerate().ToArray();
        Assert.AreEqual(1.4m, sourceObservations[0].Rate);
    }

    /// <summary>
    /// Verifies that <see cref="RateSeriesBuffer.Enumerate" /> emits observations in strictly ascending
    /// date order.
    /// </summary>
    [TestMethod]
    ...
    public void Enumerate_WhenInvokedTwice_ShouldYieldEquivalentSequences()
    {
        ...
        RateObservation[] first = buffer.Enumerate().ToArray();
        RateObservation[] second = buffer.Enumerate().ToArray();
