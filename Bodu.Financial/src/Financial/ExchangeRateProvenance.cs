...
/// <remarks>
/// <para>
/// Every <see cref="ExchangeRateLookupResult" /> carries a populated <see cref="ExchangeRateProvenance" />. A rate
/// resolved directly by a provider reports <see cref="ExchangeRateOrigin.Live" /> with a <see langword="null" />
/// <see cref="Backend" />, <see cref="CachedAtUtc" />, and <see cref="Age" />; a rate served from a cache reports
/// <see cref="ExchangeRateOrigin.Cache" /> with the serving backend, the instant the data was cached, and the age it
...
/// </para>
/// </remarks>
public readonly record struct ExchangeRateProvenance(
    string Provider,
    ...
    TimeSpan? Age)
{
    ...
    /// <see cref="ExchangeRateOrigin.Live" /> and no backend, cache instant, or age.
    /// </returns>
    public static ExchangeRateProvenance Live(string provider) =>
        new(provider, ExchangeRateOrigin.Live, Backend: null, CachedAtUtc: null, Age: null);
    ...
    /// <returns>
    /// An <see cref="ExchangeRateProvenance" /> with <see cref="Origin" /> set to
    /// <see cref="ExchangeRateOrigin.Live" />, carrying <paramref name="backend" /> but no cache instant or age.
    /// </returns>
