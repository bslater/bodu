...
public readonly partial struct Money<TCurrency>
{
    ...
    /// <exception cref="OverflowException">
    /// The product falls outside the range of <see cref="decimal" />.
    /// </exception>
    /// <remarks>
    ...
    /// <see cref="MultiplyExact(Bodu.Numerics.Fraction{System.Numerics.BigInteger}, MidpointRounding)" /> shortcut. To control
    /// the rounding rule on a single scalar multiplication, use
    ...
    /// <summary>
    /// Divides a monetary value by a scalar, rounding the result to the currency's minor-unit precision using
    /// banker's rounding (<see cref="MidpointRounding.ToEven" />).
    /// </summary>
    /// <param name="left">The amount.</param>
    ...
    /// </exception>
    /// <exception cref="OverflowException">
    /// The quotient falls outside the range of <see cref="decimal" />.
    /// </exception>
    /// <remarks>
    /// <para>
    ...
    /// <see cref="FromFraction(Bodu.Numerics.Fraction{System.Numerics.BigInteger}, MidpointRounding)" />. To control the
    /// rounding rule on a single scalar division, use <see cref="Divide(decimal, MidpointRounding)" /> in place
    /// of the operator.
    /// </para>
    /// </remarks>
    public static Money<TCurrency> operator /(Money<TCurrency> left, decimal right) =>
    ...
    /// <exception cref="DivideByZeroException">
    /// <paramref name="right" /> has an amount of zero.
    /// </exception>
    /// <remarks>
    /// Equivalent to <see cref="RatioTo(Money{TCurrency})" />; use the named method when the operator is
    /// ambiguous in domain code or when the ratio is the primary computation being expressed.
    /// </remarks>
    public static decimal operator /(Money<TCurrency> left, Money<TCurrency> right) =>
        left.RatioTo(right);

    /// <summary>
    /// Multiplies this amount by <paramref name="multiplier" /> using banker's rounding, identical to the
    /// <c>*</c> operator.
    /// </summary>
    ...
    /// </exception>
    /// <remarks>
    /// Use the <see cref="Multiply(decimal, MidpointRounding)" /> overload when a rounding rule other than
    /// banker's rounding is required.
    /// </remarks>
    public Money<TCurrency> Multiply(decimal multiplier) =>
        new Money<TCurrency>(_amount * multiplier);

    /// <summary>
    /// Multiplies this amount by <paramref name="multiplier" /> and rounds the result to the currency's
    /// minor-unit precision using the supplied rule.
    /// </summary>
    /// <param name="multiplier">The scalar multiplier.</param>
    ...
    /// <summary>
    /// Divides this amount by <paramref name="divisor" /> using banker's rounding, identical to the <c>/</c>
    /// operator.
    /// </summary>
    ...
    /// <summary>
    /// Divides this amount by <paramref name="divisor" /> and rounds the result to the currency's minor-unit
    ...
    /// <exception cref="DivideByZeroException">
    /// <paramref name="other" /> has an amount of zero.
    /// </exception>
    /// <remarks>
    /// Provides a readable named alternative to the <c>Money&lt;TCurrency&gt; / Money&lt;TCurrency&gt;</c>
    /// operator, which returns the same value. Both forms are equivalent.
    /// </remarks>
