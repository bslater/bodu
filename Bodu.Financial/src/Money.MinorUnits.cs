// ---------------------------------------------------------------------------------------------------------------
// <copyright file="Money.MinorUnits.cs" company="Bodu Pty. Ltd.">
...
{
    /// <summary>
    /// Creates a <see cref="Money{TCurrency}" /> from an exact integer count of minor units (cents, yen,
    /// fils, etc.).
    /// </summary>
    ...
    /// is the canonical bridge from that representation. For example, <c>Money&lt;USD&gt;.FromMinorUnits(1999)</c>
    /// is <c>$19.99</c>; <c>Money&lt;JPY&gt;.FromMinorUnits(1234)</c> is <c>¥1,234</c>;
    ...
    /// The scaled value falls outside the range of <see cref="long" />. For practical money amounts (up to
    /// trillions of major units) this is unreachable; the exception covers pathological values built from
    /// near-<see cref="decimal.MaxValue" /> inputs.
    /// </exception>
    public long ToMinorUnits() =>
        decimal.ToInt64(_amount * CurrencyMetadata<TCurrency>.Value.MinorUnitFactor);

    /// <summary>
    /// Attempts to express this amount as an integer count of minor units, returning <see langword="false" />
    /// instead of throwing when the value would overflow <see cref="long" />.
    /// </summary>
