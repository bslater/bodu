    ...
    : JsonConverter<CurrencyPair>
{
    ...
    private CurrencyPair ReadObject(ref Utf8JsonReader reader)
    {
        if (reader.TokenType != JsonTokenType.StartObject)
            throw new JsonException(FinancialResourceStrings.Json_Invalid_ExpectedObject_CurrencyPair);
    ...
    private static JsonException CompactPairFormatException(string text) =>
        new(string.Format(CultureInfo.CurrentCulture, FinancialResourceStrings.Json_Invalid_CompactCurrencyPairForm, text));
}
