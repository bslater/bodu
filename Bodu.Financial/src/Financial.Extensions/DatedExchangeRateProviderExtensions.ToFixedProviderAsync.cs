// ---------------------------------------------------------------------------------------------------------------
// <copyright file="DatedExchangeRateProviderExtensions.ToFixedProviderAsync.cs" company="Bodu Pty. Ltd.">
...
public static partial class DatedExchangeRateProviderExtensions
{
    /// <summary>
    ...
    /// <see cref="IDatedExchangeRateProvider.GetRatesAsync(string, string, DateOnly, DateOnly, CancellationToken)" /> —
    ...
    /// </para>
    /// </remarks>
    public static async Task<FixedDatedExchangeRateProvider> ToFixedProviderAsync(
        this IDatedExchangeRateProvider provider,
        IEnumerable<ExchangeRatePair> pairs,
        DateOnly startDate,
        DateOnly endDate,
        CancellationToken cancellationToken = default)
    {
        ...
        List<ExchangeRatePair> distinctPairs = new();
        HashSet<ExchangeRatePair> seenPairs = new();
        foreach (ExchangeRatePair pair in pairs)
        {
            FinancialThrowHelper.ThrowIfInvalidExchangeRatePair(pair, nameof(pairs));
            if (seenPairs.Add(pair))
                distinctPairs.Add(pair);
        }

        if (distinctPairs.Count == 0)
            throw new ArgumentException(FinancialResourceStrings.Arg_Invalid_ExchangeRatePairsEmpty, nameof(pairs));

        List<ExchangeRate> rows = new();
        List<string> providerOrder = new();
        HashSet<string> seenProviders = new(StringComparer.Ordinal);

        foreach (ExchangeRatePair pair in distinctPairs)
        {
            ExchangeRateRangeResult range = await provider
        ...
        ExchangeRateBook book = rows.ToBook();
        ...
        return providerOrder.Count > 0
            ? new FixedDatedExchangeRateProvider(book, providerOrder)
            : new FixedDatedExchangeRateProvider(book);
    }
}
