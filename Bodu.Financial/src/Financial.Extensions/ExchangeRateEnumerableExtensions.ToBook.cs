...
{
    /// <summary>
    /// Materializes a sequence of exchange-rate observations into an immutable <see cref="RateBook" />, one series per
    /// (pair, provider) combination.
    /// </summary>
    ...
    /// <see cref="RateSeries.FetchedAtUtc" /> is set to the latest fetch instant seen among its rows, so provenance
    /// survives the round trip deterministically regardless of input order.
    /// </para>
    /// </remarks>
