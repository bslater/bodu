...
{
    /// <summary>
    /// Materializes a sequence of exchange-rate observations into an immutable <see cref="RateBook" />, one
    ...
    /// <remarks>
    /// <para>
    /// Unlike <see cref="FixedDatedRateProvider(IEnumerable{ExchangeRate})" />, this materializer accepts rates
    ...
    /// <see cref="RateTableBuilder.Upsert" />), so re-materializing overlapping fetches is resilient rather
    /// than throwing.
    /// </para>
    /// <para>
    ...
    /// <see cref="RateSeries.FetchedAtUtc" /> is set to the latest fetch instant seen among its rows, so
    ...
    public static RateBook ToBook(this IEnumerable<ExchangeRate> rates)
    {
        ...
        foreach (ExchangeRate rate in rates)
        {
            ...
            CurrencyPair pair = rate.IsInverted ? rate.Pair.Inverse() : rate.Pair;
            var value = rate.IsInverted ? rate.ObservedRate : rate.Rate;

            RateSeriesBuilder series = builder.GetOrAddSeries(pair, rate.Provider);
