// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateSeriesBuilder.cs" company="Bodu Pty. Ltd.">
...
public sealed class RateSeriesBuilder
{
    ...
    private readonly RateSeriesBuffer _buffer;

    /// <summary>
    /// Initializes a new instance of the <see cref="RateSeriesBuilder" /> class with no observations.
    /// </summary>
    ...
    public RateSeriesBuilder(RateSeries series)
    {
        ...
        _buffer = RateSeriesBuffer.FromStorage(series.GetStorage());
    }
    ...
    public void AddRange(IEnumerable<(DateOnly Date, decimal Rate)> rates)
    {
        ...
        _buffer.AddRange(rates.Select(static r => new RateObservation(r.Date, r.Rate)), nameof(rates));
    }
    ...
    {
        ThrowHelper.ThrowIfNull(rates);
        _buffer.UpsertRange(rates.Select(static r => new RateObservation(r.Date, r.Rate)), nameof(rates));
    }
    ...
    public IEnumerable<RateObservation> GetObservations() => _buffer.Enumerate();

    /// <summary>
    /// Produces an immutable <see cref="RateSeries" /> snapshot of the current observations.
    /// </summary>
    /// <returns>A new <see cref="RateSeries" /> reflecting the builder's current state.</returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown if the builder is empty; an immutable series must contain at least one observation.
    /// </exception>
    public RateSeries ToSeries() => new(Pair, Provider, _buffer.ToStorage(), FetchedAtUtc);
}
