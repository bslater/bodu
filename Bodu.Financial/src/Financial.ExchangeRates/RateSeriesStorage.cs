// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateSeriesStorage.cs" company="Bodu Pty. Ltd.">
...
/// </para>
/// </remarks>
internal sealed class RateSeriesStorage
{
    ...
    private RateSeriesStorage(int[] dayNumbers, decimal[] rates)
    {
        ...
        out decimal rate)
    {
        ...
        if (!RateDateSearch.TrySelectCandidate(dayNumbers, requestedDayNumber, options.DateResolution, previous, next, out int candidate))
        {
            resolvedDate = default;
            rate = default;
            return false;
    ...
    /// <returns>A new <see cref="RateSeriesStorage" /> wrapping the materialised arrays.</returns>
    private static RateSeriesStorage MaterialiseFromNormalised(List<(int DayNumber, decimal Rate)> normalised)
    {
