// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateSeriesBuffer.cs" company="Bodu Pty. Ltd.">
...
/// </para>
/// </remarks>
internal sealed class RateSeriesBuffer
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RateSeriesBuffer" /> class with zero capacity.
    /// </summary>
    public RateSeriesBuffer()
    {
        _dayNumbers = [];
        _rates = [];
        Count = 0;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RateSeriesBuffer" /> class with at least
    ...
    public RateSeriesBuffer(int capacity)
    {
        if (capacity <= 0)
        {
            _dayNumbers = [];
    ...
    /// </exception>
    public void AddRange(IEnumerable<RateObservation> observations, string observationsParamName)
    {
    ...
    public void UpsertRange(IEnumerable<RateObservation> observations, string observationsParamName)
    {
        List<(int DayNumber, decimal Rate)> incoming = PrepareIncoming(observations, observationsParamName);
        if (incoming.Count == 0)
            return;
    ...
    public IEnumerable<RateObservation> Enumerate()
    {
        for (int i = 0; i < Count; i++)
        {
            yield return new RateObservation(DateOnly.FromDayNumber(_dayNumbers[i]), _rates[i]);
        }
    }
    ...
    public RateSeriesStorage ToStorage()
    {
        ...
        return RateSeriesStorage.AdoptSortedUniqueArrays(dayNumbers, rates);
    }
    ...
    /// <returns>A new <see cref="RateSeriesBuffer" /> with the same observations.</returns>
    public static RateSeriesBuffer FromStorage(RateSeriesStorage storage)
    {
        int count = storage.Count;
        var buffer = new RateSeriesBuffer(count);
    ...
    private static List<(int DayNumber, decimal Rate)> PrepareIncoming(
        ...
        RateObservationNormalizer.ToSortedUniqueList(observations, observationsParamName, allowEmpty: true);
