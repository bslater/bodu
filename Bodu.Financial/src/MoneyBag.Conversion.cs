...
public sealed partial class MoneyBag
{
    ...
    /// non-target balance and aggregating into the destination precision per
    ...
    /// <exception cref="InvalidOperationException">A rate returned by <paramref name="rates" /> is zero or negative.</exception>
        ...
        where TTarget : ICurrency
    {
        ...
        return ConvertCore<TTarget>((from, to) => rates.GetRate(from, to), policy);
    }
    ...
    /// <exception cref="InvalidOperationException">A rate returned by <paramref name="rateLookup" /> is zero or negative.</exception>
    ...
    /// <summary>
    /// Converts the entire bag to a single target currency using a delegate-based rate lookup and the
    /// supplied rounding policy.
    /// </summary>
    /// <typeparam name="TTarget">The destination currency type.</typeparam>
    /// <param name="rateLookup">A delegate that returns the rate for a (from, to) pair.</param>
    /// <param name="policy">The rounding policy applied during aggregation.</param>
    ...
    /// <summary>
    /// Shared core of the conversion paths. Validates each rate is strictly positive and applies the supplied
    /// rounding policy.
    /// </summary>
    /// <typeparam name="TTarget">The destination currency type.</typeparam>
    /// <param name="rateLookup">A delegate that returns the rate for a (from, to) pair.</param>
