...
/// <summary>
/// Compiles the YAML documentation guide examples that are explicitly opted in, so a code sample cannot silently
...
public sealed class DocumentationSnippetCompileTests
{
    ...
    [
        "docs/guides/serialization/yaml",
    ];
    ...
    /// <summary>
    /// Verifies that every opted-in YAML guide snippet compiles against the current public API, with at least one
    /// snippet marked so the guard is demonstrably wired.
    /// </summary>
    [TestMethod]
    [TestCategory("Regression")]
    public void YamlGuideSnippets_WhenMarkedForCompile_ShouldCompile()
    {
