...
/// </para>
/// <para>
/// Sharing one declaration is what makes prefix drift structurally impossible rather than merely test-detected. The
/// two tree shapes are <em>not</em> shared and deliberately so — see below — but a leaf hashed under one prefix in one
/// library and a different prefix in the other would be a silent interoperability break, and that can no longer
/// happen.
/// </para>
/// <para>
...
/// <c>H(0x00 || data)</c> and an internal node as <c>H(0x01 || children)</c>. Without the leaf prefix a one-leaf
/// tree's root would be the entry's bare digest, and an internal node's preimage — a concatenation of child hashes —
/// could be presented as leaf data. That is how a second tree is constructed to produce a root somebody has already
/// signed. The third prefix keeps a length-bound root from being confused with either.
/// </para>
/// <para>
...
/// must not be cross-checked against a transparency log. For RFC 6962's actual tree, and for inclusion and
/// consistency proofs, use <c>Rfc6962MerkleTree</c> in the <c>Bodu.Collections.Merkle</c> package.
/// </para>
/// <para>
...
internal static class MerkleTreeFormat
{
    ...
    /// <summary>
    /// The domain-separation prefix byte prepended to a length-bound root's big-endian value and tree head, as
    /// <c>H(0x02 || u64_be(boundValue) || treeHead)</c>.
    /// </summary>
    /// <remarks>
