// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateCacheContractTests{T}.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Validates the <see cref="IRateCache" /> contract shared by every implementation: read-time freshness
...
public abstract class RateCacheContractTests<TCache>
    where TCache : IRateCache
{
    /// <summary>
    ...
    public void GetRates_WhenNothingStored_ShouldReturnEmpty()
    {
        ...
        IReadOnlyList<CachedRate> result = cache.GetRates(Pair, Duration, DateTimeOffset.UtcNow);
    ...
    public void GetRates_WhenStoredAndFresh_ShouldReturnRow()
    {
        ...
        IReadOnlyList<CachedRate> result = cache.GetRates(Pair, Duration, now);
    ...
    public void GetRates_WhenAgeEqualsDuration_ShouldFilterOut()
    {
        ...
        IReadOnlyList<CachedRate> result = cache.GetRates(Pair, Duration, asOf);
    ...
    public void Store_WhenSameDateStoredTwice_ShouldKeepLatestCached()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(date, 0.5000m, older) }, Duration, newer);
        cache.Store(Pair, new[] { new CachedRate(date, 0.6000m, newer) }, Duration, newer);

        IReadOnlyList<CachedRate> result = cache.GetRates(Pair, Duration, newer);
    ...
    public void Store_WhenDifferentDates_ShouldMergeAndOrderByDate()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 6), 0.5100m, now) }, Duration, now);
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now) }, Duration, now);
    ...
    public void Store_WhenExistingRowsStale_ShouldPruneOnWrite()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, stale) }, Duration, stale);
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 6), 0.5100m, now) }, Duration, now);
    ...
    public void Store_WhenEmpty_ShouldBeNoOp()
    {
        ...
        cache.Store(Pair, Array.Empty<CachedRate>(), Duration, now);
    ...
    public void GetRates_WhenStoredUnderDifferentPair_ShouldNotLeak()
    {
        ...
        IReadOnlyList<CachedRate> other = cache.GetRates(new CurrencyPair(CurrencyCode.EUR, CurrencyCode.USD), Duration, now);
    ...
    /// <summary>
    /// Verifies that <see cref="IRateCache.RecordCoverage" /> rejects an inverted window.
    /// </summary>
    [TestMethod]
    ...
    public void Store_WhenRowHasNonPositiveRate_ShouldNotReturnRow()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0m, now) }, Duration, now);
    ...
    public void Store_WhenRowHasDefaultDate_ShouldNotReturnRow()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(default, 0.5000m, now) }, Duration, now);
    ...
    public void Store_WhenMixOfValidAndInvalidRows_ShouldRetainOnlyValid()
    {
        ...
        cache.Store(
            ...
            new[]
            {
                new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now),
                new CachedRate(new DateOnly(2023, 1, 6), -0.1m, now),
            },
            Duration,
            now);
    ...
    public void StoreFetchedRange_WhenRowsAndWindowSupplied_ShouldStoreBothHalves()
    {
        ...
        RateCacheWriteStatus status = cache.StoreFetchedRange(
            ...
            {
                new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now),
                new CachedRate(new DateOnly(2023, 1, 6), 0.5100m, now),
            },
    ...
    public void StoreFetchedRange_WhenPriorStatePresent_ShouldPreserveOtherWindow()
    {
        ...
        cache.StoreFetchedRange(
            Pair,
            new[] { new CachedRate(new DateOnly(2023, 1, 10), 0.5200m, now) },
    ...
    public void StoreFetchedRange_WhenRowInvalid_ShouldDropRowButRecordCoverage()
    {
        ...
        RateCacheWriteStatus status = cache.StoreFetchedRange(
            ...
            {
                new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now),
                new CachedRate(new DateOnly(2023, 1, 4), -0.1m, now),
            },
            new DateOnly(2023, 1, 3),
    ...
    public async Task StoreFetchedRange_WhenConcurrentForSamePair_ShouldNotLoseRowsOrCoverage()
    {
        ...
        IEnumerable<Task> writes = Enumerable.Range(0, Windows).Select(i => Task.Run(() =>
        {
            ...
            cache.StoreFetchedRange(
                Pair,
                new[] { new CachedRate(date, 0.5000m + (0.0001m * i), now) },
                date,
                date,
                Duration,
                now);
    ...
    public void Store_WhenRowHasObservedAtUtc_ShouldRoundTripObservedAtUtc()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(new DateOnly(2023, 1, 3), 0.5000m, now, observedAt) }, Duration, now);
    ...
    /// <see cref="CachedRate.ObservedAtUtc" />, across every backend.
    /// </summary>
    [TestMethod]
    ...
    /// instant, proving <see cref="CachedRate.ObservedAtUtc" /> rides along through a merge-and-prune cycle.
    /// </summary>
    [TestMethod]
    public void Store_WhenSameDateRestoredWithNewerCachedAt_ShouldKeepLatestObservedAt()
    {
        ...
        cache.Store(Pair, new[] { new CachedRate(date, 0.5000m, older, observedOld) }, Duration, newer);
        cache.Store(Pair, new[] { new CachedRate(date, 0.6000m, newer, observedNew) }, Duration, newer);

        IReadOnlyList<CachedRate> rows = cache.GetRates(Pair, Duration, newer);
        Assert.HasCount(1, rows);
