// ---------------------------------------------------------------------------------------------------------------
// <copyright file="TomlFileRateCacheTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies <see cref="TomlFileRateCache" /> construction, directory selection, and the round-trip of cached
...
public sealed partial class TomlFileRateCacheTests
{
    ...
    private TomlFileRateCache CreateCache() =>
    ...
    /// <returns>A new cache instance.</returns>
    private TomlFileRateCache CreateCache(RateCacheFileLayout layout) =>
        new(new FileRateCacheOptions { Provider = Provider, CacheDirectory = _directory, Layout = layout });
    ...
    {
        TomlFileRateCache cache = new(new FileRateCacheOptions { Provider = Provider });
    ...
    public void Constructor_WhenProviderIsBlank_ShouldThrowArgumentException()
    {
        ...
        {
            _ = new TomlFileRateCache(new FileRateCacheOptions { Provider = "  ", CacheDirectory = _directory });
        });
    ...
    /// <see cref="RateCacheWriteStatus.Failed" /> rather than falsely reporting the rows and coverage as stored.
    /// </summary>
    [TestMethod]
    public void StoreFetchedRange_WhenFileWriteFails_ShouldReturnFailed()
    {
        ...
        Assert.AreEqual(RateCacheWriteStatus.Failed, status);
    }
}
