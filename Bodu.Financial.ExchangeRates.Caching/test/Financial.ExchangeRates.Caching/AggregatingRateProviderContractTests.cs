// ---------------------------------------------------------------------------------------------------------------
// <copyright file="AggregatingRateProviderContractTests.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies that <see cref="AggregatingRateProvider" /> satisfies the shared dated-provider contract when
    ...
    : DatedRateProviderContractTests<AggregatingRateProvider>
{
    ...
    /// <inheritdoc />
    protected override AggregatingRateProvider CreateProvider()
    {
        FixedDatedRateProvider child = new(new[] { new ExchangeRate(CurrencyCode.AUD, CurrencyCode.USD, s_seeded, 0.6828m, ProviderName) });

        return new AggregatingRateProvider(new[] { new NamedDatedRateProvider(ProviderName, child) });
    }
}
