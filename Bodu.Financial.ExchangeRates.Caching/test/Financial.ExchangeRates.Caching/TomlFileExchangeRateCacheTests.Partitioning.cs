...
/// <summary>
/// Verifies that a partitioned <see cref="ExchangeRateCacheFileLayout" /> splits a pair's rows and coverage across one
...
public sealed partial class TomlFileExchangeRateCacheTests
{
    ...
    public void Store_WhenLayoutIsMonthly_ShouldWriteOneFilePerMonth()
    {
        ...
        cache.Store(
            ...
            new[]
            {
                new CachedExchangeRate(new DateOnly(2023, 1, 3), 0.50m, now),
                new CachedExchangeRate(new DateOnly(2023, 2, 6), 0.51m, now),
            },
            Duration,
            now);
    ...
    {
        DateTimeOffset now = new(2023, 3, 1, 0, 0, 0, TimeSpan.Zero);
        CreateCache(ExchangeRateCacheFileLayout.Monthly).Store(
            Pair,
            new[]
            {
                new CachedExchangeRate(new DateOnly(2023, 1, 3), 0.50m, now),
        ...
        TomlFileExchangeRateCache reopened = CreateCache(ExchangeRateCacheFileLayout.Monthly);
    ...
    public void Store_WhenPartitionPrunedToEmpty_ShouldDeleteItsFile()
    {
        ...
        TomlFileExchangeRateCache cache = CreateCache(ExchangeRateCacheFileLayout.Monthly);
        cache.Store(Pair, new[] { new CachedExchangeRate(new DateOnly(2023, 1, 3), 0.50m, januaryCachedAt) }, Duration, januaryCachedAt);
    ...
    {
        TomlFileExchangeRateCache cache = CreateCache(ExchangeRateCacheFileLayout.Yearly);
        ...
        cache.Store(
            ...
            new[]
            {
                new CachedExchangeRate(new DateOnly(2022, 6, 1), 0.50m, now),
                new CachedExchangeRate(new DateOnly(2023, 6, 1), 0.51m, now),
            },
            Duration,
            now);
