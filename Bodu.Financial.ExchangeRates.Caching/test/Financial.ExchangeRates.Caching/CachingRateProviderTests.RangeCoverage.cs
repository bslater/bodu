// ---------------------------------------------------------------------------------------------------------------
// <copyright file="CachingRateProviderTests.RangeCoverage.cs" company="Bodu Pty. Ltd.">
...
public sealed partial class CachingRateProviderTests
{
    ...
    public async Task GetRatesAsync_WhenStoreFetchedRangeFails_ShouldRefetchRatherThanFalseHit()
    {
        ...
        var failingCache = new FailingStoreRateCache(Provider);
        var sut = new CachingRateProvider(inner, failingCache, _options, _clock);
    ...
    /// <summary>
    /// Verifies that a backend modelling a swallowed storage write reports <see cref="RateCacheWriteStatus.Failed" />
    /// from <see cref="IRateCache.StoreFetchedRange" />, the contract signal the decorator relies on to refetch
    ...
    public void StoreFetchedRange_WhenBackendSwallowsWrite_ShouldReportFailed()
    {
        ...
        RateCacheWriteStatus status = failingCache.StoreFetchedRange(
