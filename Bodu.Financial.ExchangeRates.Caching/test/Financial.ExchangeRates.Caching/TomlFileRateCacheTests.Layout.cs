// ---------------------------------------------------------------------------------------------------------------
// <copyright file="TomlFileRateCacheTests.Layout.cs" company="Bodu Pty. Ltd.">
...
/// <summary>
/// Verifies how <see cref="TomlFileRateCache" /> resolves a pair's directory and file paths under the configured
...
public sealed partial class TomlFileRateCacheTests
{
    ...
    {
        var layout = RateCacheFileLayout.Create(
        ...
        TomlFileRateCache cache = CreateCache(layout);
    ...
    public void Constructor_WhenLayoutIsNull_ShouldThrowArgumentException()
    {
        ...
        {
            _ = new TomlFileRateCache(new FileRateCacheOptions { Provider = Provider, CacheDirectory = _directory, Layout = null! });
        });
