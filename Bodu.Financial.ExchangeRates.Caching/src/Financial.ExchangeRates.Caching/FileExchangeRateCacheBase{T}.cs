...
/// <summary>
/// Provides the file-storage mechanism for an <see cref="IExchangeRateCache" />: layout-driven directory and file-name
...
/// <remarks>
/// <para>
/// A cache instance is bound to one provider, and the <see cref="FileExchangeRateCacheOptions.Layout" /> decides where
    ...
    where TOptions : FileExchangeRateCacheOptions
{
    ...
    private readonly ExchangeRateCacheFileLayout _layout;
    ...
    private CachePairState ReadPartitioned(ExchangeRatePair pair)
    {
        ...
        List<CachedExchangeRate> entries = new();
        ...
        // sufficient here; sorting and de-duplication are the caller's responsibility through ExchangeRateCacheRules.
    ...
    private void WritePartitioned(ExchangeRatePair pair, CachePairState state)
    {
        ...
        Dictionary<string, (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage)> buckets = BucketByPartition(state);
        ...
        foreach (KeyValuePair<string, (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage)> bucket in buckets)
        ...
        {
            (List<CachedExchangeRate> entries, List<CoverageWindow> coverage) = buckets[target.Value];
    ...
    private Dictionary<string, (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage)> BucketByPartition(CachePairState state)
    {
        ExchangeRateCachePartitionStrategy strategy = _layout.PartitionStrategy;
        Dictionary<string, (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage)> buckets = new(StringComparer.Ordinal);
    ...
    private static (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage) Bucket(
        Dictionary<string, (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage)> buckets,
        string key)
    {
        if (!buckets.TryGetValue(key, out (List<CachedExchangeRate> Entries, List<CoverageWindow> Coverage) bucket))
        {
            bucket = (new List<CachedExchangeRate>(), new List<CoverageWindow>());
            buckets[key] = bucket;
        }
