// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RateCacheFileLayout.cs" company="Bodu Pty. Ltd.">
...
/// </para>
/// </remarks>
public sealed class RateCacheFileLayout
{
    ...
    private readonly Func<RateCacheDirectoryContext, string> _directory;

    /// <summary>The delegate resolving the name of a single cache file.</summary>
    private readonly Func<RateCacheFileContext, string> _fileName;

    /// <summary>
    /// Initializes a new instance of the <see cref="RateCacheFileLayout" /> class.
    /// </summary>
    ...
    private RateCacheFileLayout(
        RateCachePartitionStrategy partitionStrategy,
        Func<RateCacheDirectoryContext, string> directory,
        Func<RateCacheFileContext, string> fileName)
    {
    ...
    public static RateCacheFileLayout SingleFile { get; } =
        new(
            RateCachePartitionStrategy.Single,
    ...
    public static RateCacheFileLayout Yearly { get; } =
        Partitioned(RateCachePartitionStrategy.Yearly);
    ...
    public static RateCacheFileLayout Monthly { get; } =
        Partitioned(RateCachePartitionStrategy.Monthly);
    ...
    public static RateCacheFileLayout Daily { get; } =
        Partitioned(RateCachePartitionStrategy.Daily);

    /// <summary>
    /// Gets the strategy deciding how a pair's rows are split across files by date.
    /// </summary>
    /// <value>
    /// The partition strategy; <see cref="RateCachePartitionStrategy.Single" /> for a single-file layout.
    /// </value>
    public RateCachePartitionStrategy PartitionStrategy { get; }
    ...
    /// </exception>
    public static RateCacheFileLayout Create(
        RateCachePartitionStrategy partitionStrategy,
        Func<RateCacheDirectoryContext, string>? directory = null,
        Func<RateCacheFileContext, string>? fileName = null)
    {
        ThrowHelper.ThrowIfNull(partitionStrategy);

        return new RateCacheFileLayout(
            partitionStrategy,
    ...
    public string ResolveDirectory(string root, string provider, CurrencyPair pair) =>
        _directory(new RateCacheDirectoryContext(root, provider, pair));
    ...
    public string ResolveFileName(string provider, CurrencyPair pair, string partitionKey, string fileExtension) =>
        _fileName(new RateCacheFileContext(provider, pair, partitionKey, fileExtension));
    ...
    private static RateCacheFileLayout Partitioned(RateCachePartitionStrategy partitionStrategy) =>
    ...
    private static Func<RateCacheDirectoryContext, string> DefaultDirectory(bool isPartitioned) =>
        isPartitioned
    ...
    private static Func<RateCacheFileContext, string> DefaultFileName(bool isPartitioned) =>
        isPartitioned
