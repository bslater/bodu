// ---------------------------------------------------------------------------------------------------------------
// <copyright file="AsiaPacificCalendarData.cs" company="PlaceholderCompany">
...
/// <summary>
/// Provides factory entry points for the Asia-Pacific region calendar data pack — embedded notable-date rules for Australia, China,
/// India, Japan, South Korea, Malaysia, New Zealand, and Singapore — so consumers can compose a <see cref="NotableDateService" />
/// without knowing the underlying resource layout.
/// </summary>
/// <remarks>
/// <para>
/// Each region XML is embedded under the historical <c>Bodu.Globalization.Calendar.Resources.*</c> path so that
/// <c>&lt;UseFrom resource="./global-all.xml"&gt;</c> directives resolve against the main library's globals via the provider's
/// assembly chain.
/// </para>
/// <para>
/// Several rules in the Asia-Pacific pack (lunar, Hindu, Islamic, Buddhist) require custom <see cref="INotableDateAlgorithm" />
/// implementations registered with the <see cref="INotableDateAlgorithmRegistry" />. Without those registrations the affected dates
/// resolve to no occurrences without error; the remaining Western/Gregorian rules behave unaffected.
/// </para>
/// </remarks>
/// <example>
/// <code>
...
public static class AsiaPacificCalendarData
{
    ...
    /// <summary>
    /// Creates an <see cref="INotableDateRuleProvider" /> rooted at <paramref name="resourceName" /> using the standard pack →
    /// main-library assembly chain. Exposed to support consumers loading a specific resource by name.
    /// </summary>
    /// <param name="resourceName">The logical resource name of the root XML payload. Must not be <see langword="null" />.</param>
    /// <returns>A configured rule provider.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="resourceName" /> is <see langword="null" />.</exception>
