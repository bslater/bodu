// --------------------------------------------------------------------------------------------------------------- //
// <copyright file="FiscalWeekQuarterProviderTests.GetQuarterEndDate.cs" company="Bodu Pty. Ltd.">
...
public partial class FiscalWeekQuarterProviderTests
{
    ...
    /// <summary>
    /// Verifies that <see cref="FiscalWeekQuarterProvider.GetQuarterEndDate(DateOnly)" /> resolves a
    /// date prior to the 53-week fiscal year (<see cref="s_sunday53" />) into the preceding fiscal year.
    /// </summary>
    [TestMethod]
    public void GetQuarterEndDate_WhenDateOnlyIsBeforeAnchorFiscalYearAcrossCalendarYear_ShouldResolveToPriorFiscalYear() =>
        ...
        Assert.AreEqual(new DateOnly(2019, 12, 28), s_sunday53.GetQuarterEndDate(new DateOnly(2019, 12, 28)));

    /// <summary>
    /// Verifies that <see cref="FiscalWeekQuarterProvider.GetQuarterEndDate(DateOnly)" /> maps the
    /// first day of the next fiscal year following the 53-week <see cref="s_sunday53" /> provider into
    ...
    [DataRow(0)]
    [DataRow(5)]
    public void GetQuarterEndDate_WhenQuarterIsOutOfRange_ShouldThrowExactly(int quarter)
    {
