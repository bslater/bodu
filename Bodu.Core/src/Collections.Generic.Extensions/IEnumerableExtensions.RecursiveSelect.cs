...
public static partial class IEnumerableExtensions
{
    ...
    /// </para>
    /// <para>
    /// The traversal uses a depth-first strategy. For each element, in order:
    /// <list type="bullet">
    /// <item>
    ...
    /// <item>
    /// <description>
    /// If <c>Break</c> is set, remaining siblings are skipped at the current level. Because this is evaluated after the
    /// recursion step, <c>Break</c> and <c>Recurse</c> compose: the current element's children are visited before the
    /// remaining siblings are skipped.
    /// </description>
    /// </item>
    /// </list>
    /// </para>
        ...
        RecursionState? state = null)
    {
        ...
        foreach (TSource? element in source)
        {
            ...
            bool shouldYield = (control & RecursiveSelectControl.Yield) != 0 &&
                               (control & RecursiveSelectControl.Skip) == 0;
            ...
            if ((control & RecursiveSelectControl.Exit) != 0)
            {
                state.ExitRequested = true;
                yield break;
            }

            // Recurse into children if requested and available. This runs before the sibling-level Break check so that
            // Break and Recurse compose: a combined value descends into the current element's children first, then the
            // Break stops the remaining siblings.
            if ((control & RecursiveSelectControl.Recurse) != 0)
            {
            ...
            if ((control & RecursiveSelectControl.Break) != 0)
                break;
        }
    }
