...
/// set-membership predicates (<c>ContainsAll</c>, <c>ContainsAny</c>), tree traversal (<c>RecursiveSelect</c>), shuffling
/// (<c>Randomize</c>), and multi-aggregate fan-out (<c>Aggregate</c> overloads that compute up to several seeds in a single
/// enumeration). Each method is offered with the overload set required to keep the call site clean — a default form, a form that
/// accepts a projection or comparer, and where useful a form that hands the caller a pooled buffer to drive throughput-sensitive
/// work.
/// </para>
/// <para>
...
///     .ContainsAny(new[] { "foo", "baz" }, StringComparer.OrdinalIgnoreCase);
/// // => true
/// </code>
/// </example>
/// </remarks>
