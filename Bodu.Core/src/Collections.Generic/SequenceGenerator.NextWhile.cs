...
public static partial class SequenceGenerator
{
    ...
    /// <param name="conditionHandler">A predicate evaluated against the current value before each emission. Must not be <see langword="null"/>.</param>
    /// <param name="resultSelector">A function that derives the next value from the current value. Must not be <see langword="null"/>.</param>
    /// <returns>A lazily evaluated sequence terminating on the first iteration where <paramref name="conditionHandler"/> returns <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="conditionHandler"/> or <paramref name="resultSelector"/> is <see langword="null"/>.
    /// </exception>
    /// <remarks>
    /// <para>
    /// Use this overload when the next value depends only on the current one — classic recurrence relations such as
    /// <c>x ↦ 2·x</c> or <c>x ↦ x / 2</c>. Reach for the indexed overload when the position matters, and the state-based overload
    /// when more than one variable must be tracked across iterations.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The sequence may be infinite if the predicate never fails; bound it with <c>Take</c> or <c>TakeWhile</c> when an upper limit
    /// is required. Iteration is deferred and allocates only the iterator state and a single boxed state slot.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code language="csharp">
    /// // Halving sequence — terminates when the value drops to zero.
    /// var halves = SequenceGenerator.NextWhile(64, v =&gt; v &gt; 0, v =&gt; v / 2);
    /// // => 64, 32, 16, 8, 4, 2, 1
    ///
    /// // Empty result when the seed already fails the predicate.
    /// var none = SequenceGenerator.NextWhile(0, v =&gt; v &gt; 0, v =&gt; v - 1);
    /// // => (empty)
    /// </code>
    /// </example>
    ...
    /// A function that derives the next value from the current value and its zero-based index. The index passed in for the very
    /// first call to the selector is <c>0</c>. Must not be <see langword="null"/>.
    /// </param>
    /// <returns>A lazily evaluated sequence terminating on the first iteration where <paramref name="conditionHandler"/> returns <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="conditionHandler"/> or <paramref name="resultSelector"/> is <see langword="null"/>.
    ...
    /// <remarks>
    /// <para>
    /// Choose this overload over <see cref="NextWhile{TResult}(TResult, Func{TResult, bool}, Func{TResult, TResult})"/> when the
    /// transformation depends on its position — for example, accumulating sums of <c>i</c>, generating polynomial terms, or scaling
    /// by a power of the current index.
    /// </para>
    /// <para>
    /// The index increments after the selector returns, so the seed is index <c>0</c>, the value passed to the first selector call
    /// is index <c>0</c>, and the value emitted next sits at index <c>1</c>. Iteration is deferred and the index is held internally;
    /// it cannot overflow a single iteration but will wrap to <see cref="int.MinValue"/> after <see cref="int.MaxValue"/> elements.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code language="csharp">
    ...
    /// <param name="conditionHandler">A predicate evaluated against the current state before each emission. Must not be <see langword="null"/>.</param>
    /// <param name="iterateFunction">A function that produces the next state from the current state. Must not be <see langword="null"/>.</param>
    /// <param name="resultSelector">A function that projects the current state into the emitted element. Must not be <see langword="null"/>.</param>
    /// <returns>A lazily evaluated sequence whose elements are produced by projecting each state visited by the iteration.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="conditionHandler"/>, <paramref name="iterateFunction"/>, or <paramref name="resultSelector"/> is <see langword="null"/>.
    /// </exception>
    /// <remarks>
    /// <para>
    /// This is the most general <c>NextWhile</c> form. Use it when each iteration needs to track several variables — for example, a
