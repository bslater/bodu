...
{
    /// <summary>
    /// Yields consecutive integers from <paramref name="start" /> through <paramref name="stop" />, walking in
    /// whichever direction is implied by the two endpoints.
    /// </summary>
    /// <param name="start">The first value emitted by the sequence.</param>
    /// <param name="stop">
    /// The final value emitted by the sequence; this value is included in the output when reached exactly.
    /// </param>
    ...
    /// <paramref name="stop" />, descending otherwise. When <paramref name="start" /> equals <paramref name="stop" /> a
    ...
    /// </remarks>
    /// <example>
    /// <code language="csharp"><![CDATA[ foreach (int n in SequenceGenerator.Range(10, 14)) Console.Write($"{n} "); // => 10 11
    /// 12 13 14 foreach (int n in SequenceGenerator.Range(3, -2)) Console.Write($"{n} "); // => 3 2 1 0 -1 -2 ]]></code>
    /// </example>
    public static IEnumerable<int> Range(int start, int stop) => SequenceGenerator.Range(start, stop, start < stop ? 1 : -1);

    /// <summary>
    /// Yields integers between <paramref name="start" /> and <paramref name="stop" /> inclusive, advancing by
    /// <paramref name="step" /> at each iteration.
    /// </summary>
    /// <param name="start">The first value emitted by the sequence.</param>
    /// <param name="stop">
    /// The endpoint at which iteration terminates; included in the output when reached exactly.
    /// </param>
    /// <param name="step">
    /// The signed delta applied between successive values. Positive values produce an ascending sequence, negative
    /// values produce a descending sequence, and zero produces an unbounded sequence that yields
    /// <paramref name="start" /> forever.
    /// </param>
    /// <returns>
    /// A lazily evaluated sequence of <see cref="int" /> values; finite when <paramref name="step" /> is non-zero,
    /// otherwise infinite.
    /// </returns>
    /// <remarks>
    /// <para>
    /// Use this overload when the caller needs explicit control over the stride, including descending ranges and
    /// non-unit steps that <see cref="System.Linq.Enumerable.Range(int, int)" /> cannot express. The two-argument
    /// <see cref="Range(int, int)" /> is more convenient when a step of <c>+1</c> or <c>-1</c> is sufficient.
    /// </para>
    /// <para>
    /// Iteration is bounded by <paramref name="stop" />: the inclusive endpoint is yielded only when
    /// <paramref name="step" /> divides the interval evenly. If the next value would overflow
    ...
    /// </remarks>
    /// <example>
    /// <code language="csharp"><![CDATA[ foreach (int n in SequenceGenerator.Range(0, 20, 5)) Console.Write($"{n} "); // => 0 5
    ...
    /// <returns>
    /// A lazily evaluated, finite sequence of <see cref="long" /> values containing exactly <paramref name="count" />
    /// elements.
    /// </returns>
    ...
    /// <remarks>
    /// <para>
    /// Prefer this overload when the desired output is a count-bounded run of contiguous values, particularly for
    /// ranges that exceed the addressable region of <see cref="System.Linq.Enumerable.Range(int, int)" /> (which is
    /// restricted to <see cref="int" />).
    /// </para>
    /// <para>
    /// A <paramref name="count" /> of <c>0</c> produces an empty sequence. The endpoint is exclusive in the sense that
    /// the final value emitted is <c><paramref name="start" /> + <paramref name="count" /> - 1</c>.
    /// </para>
    /// <para>
    /// Overflow is rejected up-front by the argument validation, so the iterator never throws mid-enumeration.
    /// Iteration is deferred, deterministic, and allocation-free apart from the iterator state.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code language="csharp"><![CDATA[ foreach (long n in SequenceGenerator.Range(1_000_000_000_000L, 4)) Console.Write($"{n}
    /// "); // => 1000000000000 1000000000001 1000000000002 1000000000003 var empty = SequenceGenerator.Range(0L,
    /// 0).ToArray(); // => empty.Length == 0 ]]></code>
    /// </example>
