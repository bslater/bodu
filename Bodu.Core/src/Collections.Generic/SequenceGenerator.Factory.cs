...
{
    /// <summary>
    /// Wraps a user-supplied enumerator factory in an <see cref="IEnumerable{TResult}"/> so it can participate in LINQ pipelines.
    /// </summary>
    /// <typeparam name="TResult">The element type produced by the supplied enumerator.</typeparam>
    /// <param name="enumeratorFactory">
    /// A delegate that returns a fresh <see cref="IEnumerator{T}"/> on each call. Must not be <see langword="null"/>. The delegate
    /// is invoked once per <c>foreach</c> / <c>GetEnumerator</c> call, so it must produce an independent enumerator each time to
    /// allow re-enumeration.
    /// </param>
    /// <returns>An <see cref="IEnumerable{TResult}"/> that defers all work to <paramref name="enumeratorFactory"/> at iteration time.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="enumeratorFactory"/> is <see langword="null"/>.</exception>
    /// <remarks>
    /// <para>
    ...
    /// supplied enumerator and is re-enumerable only if <paramref name="enumeratorFactory"/> returns a new, independent enumerator
    /// on every call.
    /// </para>
    /// <para>Allocations are limited to a single wrapper instance plus whatever the supplied enumerator allocates per iteration.</para>
    /// </remarks>
    /// <example>
    ...
    ///     Console.Write($"{b} ");
    /// // => 79 235 64 230   (a deterministic run with seed 42)
    /// </code>
    /// </example>
