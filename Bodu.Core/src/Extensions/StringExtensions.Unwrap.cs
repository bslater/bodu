...
{
    /// <summary>
    /// Returns <paramref name="value" /> with <paramref name="prefix" /> and <paramref name="suffix" />
    /// removed if both are present at their respective ends.
    /// </summary>
    ...
    /// <returns>
    /// The unwrapped string when both ends match; otherwise <paramref name="value" /> unchanged. Unwrapping
    /// requires both ends to match — partial matches are left intact so the operation is reversible against
    /// <see cref="Wrap(string, string, string)" />.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when any string argument is <see langword="null" />.
    /// </exception>
