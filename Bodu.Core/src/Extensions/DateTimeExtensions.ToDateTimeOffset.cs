// ---------------------------------------------------------------------------------------------------------------
// <copyright file="DateTimeExtensions.ToDateTimeOffset.cs" company="Bodu Pty. Ltd.">
...
{
    /// <summary>
    /// Returns a new <see cref="DateTimeOffset" /> representing the same moment as the specified
    /// <paramref name="dateTime" />, with the offset inferred from its <see cref="DateTime.Kind" />.
    /// </summary>
    /// <param name="dateTime">The date and time value to convert.</param>
    /// <returns>
    /// A <see cref="DateTimeOffset" /> representing the same point in time as <paramref name="dateTime" />, with the
    /// offset derived from its <see cref="DateTime.Kind" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// The behavior depends on the <see cref="DateTime.Kind" /> of the input:
    /// </para>
    /// <list type="bullet">
    /// <item>
    ...
    /// <paramref name="dateTime" />, with the supplied <paramref name="offset" /> applied.
    /// </summary>
    /// <param name="dateTime">The date and time value to convert.</param>
    /// <param name="offset">
    /// The UTC <see cref="TimeSpan" /> offset to associate with the resulting <see cref="DateTimeOffset" />. Must be
    /// within the range ±14 hours.
    /// </param>
    /// <returns>
    /// A <see cref="DateTimeOffset" /> with the same local time as <paramref name="dateTime" /> and the supplied
    ...
    /// Thrown if <paramref name="offset" /> is not within the range ±14 hours, -or- <paramref name="offset" /> is
    /// incompatible with the <see cref="DateTime.Kind" /> of <paramref name="dateTime" />.
    /// </exception>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown if the resulting UTC time is outside the supported range of <see cref="DateTimeOffset" />.
    /// </exception>
