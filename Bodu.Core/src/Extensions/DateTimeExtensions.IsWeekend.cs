...
{
    /// <summary>
    /// Determines whether the specified <see cref="DateTime"/> falls on a weekend, using the default <see cref="WorkingDaysOfWeek.MondayToFriday"/> rule.
    /// </summary>
    ...
    /// <summary>
    /// Determines whether the specified <see cref="DateTime"/> falls on a weekend, using the supplied <see cref="WorkingDaysOfWeek"/> and an optional custom <paramref name="provider"/>.
    /// </summary>
    /// <param name="dateTime">The date and time value to evaluate.</param>
    ...
    /// <returns><see langword="true"/> if <paramref name="dateTime"/> falls on a weekend day as defined by the supplied working-week or provider; otherwise, <see langword="false"/>.</returns>
    /// <remarks>
    ...
    /// <summary>
    /// Determines whether the specified <see cref="DayOfWeek"/> is considered a weekend day, using the supplied <see cref="WorkingDaysOfWeek"/> and an optional custom <paramref name="provider"/>.
    /// </summary>
    ...
    /// <remarks>
    /// <para>This overload supports custom weekend evaluation logic via <paramref name="provider"/> when <paramref name="workingWeek"/> is <see cref="WorkingDaysOfWeek.Custom"/>. For all other values the result is derived from the canonical <see cref="WeekPattern"/> implied by <paramref name="workingWeek"/>.</para>
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown if <paramref name="dayOfWeek"/> is not a defined value of the <see cref="DayOfWeek"/> enumeration,
    /// -or- <paramref name="workingWeek"/> is not a defined value of the <see cref="WorkingDaysOfWeek"/> enumeration,
    /// -or- <paramref name="workingWeek"/> is <see cref="WorkingDaysOfWeek.Custom"/> and <paramref name="provider"/> is <see langword="null"/>.
    /// </exception>
