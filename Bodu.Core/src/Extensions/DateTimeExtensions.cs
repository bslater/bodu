...
/// <c>LastDateOfYear</c> and the matching <c>LastDateOf…</c> set), day-boundary helpers (<c>StartOfDay</c>,
/// <c>EndOfDay</c>, <c>Midday</c>, <c>Midnight</c>, <c>Truncate</c>), weekday navigation (<c>NextDateOfWeek</c>,
/// <c>PreviousDateOfWeek</c>, <c>NearestDateOfWeek</c>, <c>NextWeekday</c>, <c>NthDateOfWeekInMonth</c>), period
/// predicates and numbering (<c>IsLeapYear</c>, <c>IsWeekend</c>, <c>IsInRange</c>, <c>WeekOfMonth</c>,
/// <c>WeekOfYear</c>, <c>IsoWeekOfYear</c>, <c>IsoYear</c>, <c>Quarter</c>), and conversions (<c>ToDateOnly</c>,
/// <c>ToDateTimeOffset</c>, <c>ToIsoString</c>, <c>ToTimeSpan</c>, the <c>UnixTime</c>/epoch family).
/// </para>
/// <para>
/// Many helpers operate directly on <see cref="DateTime.Ticks" /> to avoid intermediate <see cref="DateTime" />
/// allocations and favor method-impl <c>AggressiveInlining</c> for hot-path calendar code. Operations that read culture
/// data (<c>DayName</c>, <c>MonthName</c>, ISO week calculations against a
/// <see cref="System.Globalization.CultureInfo" />) accept the culture explicitly or fall back to
/// <see cref="System.Globalization.CultureInfo.CurrentCulture" />; pure date arithmetic is culture-neutral and
/// deterministic. The <see cref="DateTime.Kind" /> of the input is preserved unless an explicit conversion is
...
/// </para>
/// <example>
/// <code language="csharp"><![CDATA[ var dt = new DateTime(2025, 4, 30, 14, 35, 0, DateTimeKind.Utc); // Snap to the start and
...
/// this date. DateTime nextMonday = dt.NextDateOfWeek(DayOfWeek.Monday); // 2025-05-05T14:35:00Z ]]></code>
/// </example>
/// </remarks>
public static partial class DateTimeExtensions
{
    ...
    /// <summary>
    /// Computes the day number corresponding to the specified <see cref="DateTime" />, representing the number of days
    /// since 0001-01-01.
    /// </summary>
    ...
    /// <remarks>
    /// This method performs a fast, allocation-free conversion by dividing the <see cref="DateTime.Ticks" /> value by
    ...
    /// <summary>
    /// Computes the day number for the specified year, month, and day, representing the number of days elapsed since
    /// 0001-01-01.
    /// </summary>
    ...
    /// <returns>
    /// An <see cref="int" /> representing the number of days since 0001-01-01, where that date is treated as day 0.
    /// </returns>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown if <paramref name="year" />, <paramref name="month" />, or <paramref name="day" /> is outside the valid
    /// range of the Gregorian calendar, or if the combination does not form a valid date.
    /// </exception>
    /// <remarks>
    /// <para>
    /// This method performs full validation of the input parameters to ensure that the specified date is valid in the
    /// proleptic Gregorian calendar.
    /// </para>
    /// <para>
    /// It provides functionality equivalent to computing <c>new DateOnly(year, month, day).DayNumber</c>, but avoids
    ...
    /// </summary>
    /// <param name="ticks">
    /// A tick count representing a date and time, where one tick equals 100 nanoseconds since 0001-01-01 00:00:00.000
    /// in the Gregorian calendar.
    /// </param>
    /// <returns>
    /// The number of ticks corresponding to the start of the day (midnight) for the given <paramref name="ticks" />.
    /// This value is a multiple of <see cref="DateTimeExtensions.TicksPerDay" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method discards any time-of-day component by rounding down to the nearest whole day. It is equivalent to
    /// calling <c>DateTime.Date.Ticks</c> but avoids object allocations and is optimized for internal calendar or
    /// performance-sensitive usage.
    /// </para>
    /// <para>
    /// No validation is performed. The caller is responsible for ensuring that <paramref name="ticks" /> falls within
    /// the valid <see cref="DateTime" /> range.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </summary>
    /// <param name="ticks">
    /// The number of ticks representing the date, where one tick equals 100 nanoseconds since 0001-01-01T00:00:00.000
    /// (Gregorian calendar).
    /// </param>
    /// <param name="year">
    ...
    /// <remarks>
    /// <para>
    /// This method performs a tick-based decomposition of a date using Gregorian calendar rules, without instantiating
    /// a <see cref="DateTime" /> object. It is intended for performance-critical scenarios where object allocations
    /// must be avoided, or where calendar field extraction is required from raw tick values.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// This method uses the standard proleptic Gregorian calendar and assumes 400-year cycles with leap years every 4
    /// years, except years divisible by 100 unless divisible by 400.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Extracts the year, month, and day components from the specified <paramref name="dateTime" /> using tick-based
    /// computation.
    /// </summary>
    /// <param name="dateTime">
    /// The <see cref="System.DateTime" /> instance whose date components are to be extracted.
    /// </param>
    /// <param name="year">
    ...
    /// <remarks>
    /// <para>
    /// This method computes the date parts directly from the <see cref="DateTime.Ticks" /> value, using Gregorian
    /// calendar math. It avoids calling the standard <see cref="DateTime.Year" />, <see cref="DateTime.Month" />, or
    /// <see cref="DateTime.Day" /> properties to reduce overhead in high-performance scenarios.
    /// </para>
    /// <para>
    /// The result is equivalent to the values returned by the standard <see cref="DateTime" /> accessors, but the
    /// method is optimized for scenarios where multiple components are needed and maximum efficiency is desired.
    /// </para>
    /// <para>
    /// This method assumes the input is within the valid <see cref="DateTime" /> range and uses no internal validation.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <param name="dateTime">
    /// The <see cref="System.DateTime" /> instance from which to extract the date portion as a tick count.
    /// </param>
    /// <returns>
    /// A <see cref="long" /> representing the number of ticks at midnight (00:00:00.000) on the day of
    /// <paramref name="dateTime" />. This value is a multiple of <see cref="DateTimeExtensions.TicksPerDay" /> and
    /// falls within the valid <see cref="DateTime" /> range.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method truncates the time component of the input <paramref name="dateTime" /> by rounding down to the
    /// nearest whole day boundary. It is equivalent to <c>dateTime.Date.Ticks</c> but avoids allocation of a new
    /// <see cref="DateTime" /> instance and is optimized for internal use.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Computes the day number for the specified year, month, and day, representing the number of days since
    /// 0001-01-01.
    /// </summary>
    ...
    /// <returns>
    /// An integer representing the number of days elapsed since 0001-01-01, with that date treated as day 0.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method uses Gregorian calendar rules and a precomputed day-of-year lookup table to account for leap years.
    /// It performs no validation on the input parameters and assumes that <paramref name="year" />,
    /// <paramref name="month" />, and <paramref name="day" /> form a valid calendar date.
    /// </para>
    /// <para>
    /// This method is intended for trusted, performance-critical internal use where input correctness is guaranteed by
    /// the caller. It avoids allocating <see cref="DateTime" /> instances and is suitable for inlining.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </summary>
    /// <param name="ticks">
    /// A date and time expressed as the number of 100-nanosecond intervals (ticks) that have elapsed since January 1,
    /// 0001 at 00:00:00.000, based on the proleptic Gregorian calendar.
    /// </param>
    /// <returns>
    /// A <see cref="DayOfWeek" /> value indicating the day of the week corresponding to the specified
    /// <paramref name="ticks" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method computes the day of the week using modulo arithmetic based on the number of days since 0001-01-01.
    /// It is equivalent in result to <see cref="DateTime.DayOfWeek" /> but avoids object instantiation and is optimized
    /// for tick-level operations.
    /// </para>
    /// <para>
    /// No argument validation is performed. The caller is responsible for ensuring that the input
    /// <paramref name="ticks" /> value is valid within the supported <see cref="DateTime" /> range.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </summary>
    /// <param name="days">
    /// A value representing whole and/or fractional days. This value may be positive or negative and supports sub-day
    /// precision.
    /// </param>
    /// <returns>
    /// A <see cref="long" /> representing the number of ticks that correspond to the specified number of days. One day
    /// equals 864,000,000,000 ticks.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method multiplies the input <paramref name="days" /> by the number of ticks per day, rounding to the
    /// nearest whole tick using symmetric arithmetic rounding (midpoint rounding away from zero).
    /// </para>
    /// <para>
    /// It is equivalent in purpose to <see cref="TimeSpan.FromDays(double)" /> but optimized for internal use and
    /// avoids allocations.
    /// </para>
    /// <para>
    /// No argument validation is performed; it is the caller's responsibility to ensure that <paramref name="days" />
    ...
    /// <summary>
    /// Returns the tick count that represents the date nearest to the specified <paramref name="dayOfWeek" /> relative
    /// to the provided <paramref name="ticks" /> value.
    /// </summary>
    ...
    /// <summary>
    /// Calculates the number of ticks between the specified <paramref name="dateTime" /> and the next occurrence of
    /// <paramref name="dayOfWeek" />, including the current day when it already matches.
    /// </summary>
    /// <param name="dateTime">The <see cref="DateTime" /> from which to calculate the forward offset.</param>
    ...
    /// <paramref name="dateTime" /> and the next occurrence of <paramref name="dayOfWeek" />, including the current day
    /// when it already matches. This value is a non-negative multiple of <see cref="DateTimeExtensions.TicksPerDay" />.
    /// </returns>
    /// <remarks>
    /// <para>
    ...
    /// <summary>
    /// Calculates the number of ticks between the specified <paramref name="ticks" /> value and the next occurrence of
    /// the given <paramref name="dayOfWeek" />, including the current day when it already matches.
    /// </summary>
    /// <param name="ticks">
    /// A date and time expressed as the number of 100-nanosecond intervals (ticks) since 0001-01-01T00:00:00.000 in the
    /// Gregorian calendar.
    /// </param>
    ...
    /// </param>
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks between the specified <paramref name="ticks" />
    /// value and the next occurrence of <paramref name="dayOfWeek" />, including the current day when it already
    /// matches. This value is a non-negative multiple of <see cref="DateTimeExtensions.TicksPerDay" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// If the date represented by <paramref name="ticks" /> already falls on <paramref name="dayOfWeek" />, this method
    /// returns <c>0</c>.
    /// </para>
    /// <para>
    /// This method performs no argument validation. The caller must ensure that <paramref name="ticks" /> falls within
    /// the valid range for <see cref="DateTime" />, and that <paramref name="dayOfWeek" /> is a valid
    /// <see cref="DayOfWeek" /> enum value.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Calculates the number of ticks between the specified <paramref name="ticks" /> value and the most recent
    /// occurrence of the given <paramref name="dayOfWeek" />, including the current day when it already matches.
    /// </summary>
    /// <param name="ticks">
    /// A date and time expressed as the number of 100-nanosecond intervals (ticks) since 0001-01-01T00:00:00.000 in the
    /// Gregorian calendar.
    ...
    /// </param>
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks between the specified <paramref name="ticks" /> and
    /// the most recent occurrence of <paramref name="dayOfWeek" />. This value is a non-negative multiple of
    ...
    /// <see cref="DateTime" /> range, and that <paramref name="dayOfWeek" /> is a valid <see cref="DayOfWeek" /> enum
    /// value.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <param name="day">The day component, which must be valid for the specified year and month.</param>
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks since 0001-01-01T00:00:00.000 that correspond to
    /// the specified date.
    /// </returns>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="year" />, <paramref name="month" />, or <paramref name="day" /> is outside the valid
    /// range of the Gregorian calendar or does not form a valid date.
    /// </exception>
    /// <remarks>
    /// <para>
    /// This method performs full validation of the input parameters and throws an exception if the combination does not
    /// represent a valid calendar date.
    /// </para>
    /// <para>
    /// It is equivalent in result to <c>new DateTime(year, month, day).Ticks</c> but avoids object allocation and is
    /// optimized for high-performance internal date calculations.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Returns the <see cref="DayOfWeek" /> corresponding to January 1st of the specified year, using Gregorian
    /// calendar rules.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// This method uses a fast arithmetic formula to determine the weekday of January 1st without allocating a
    /// <see cref="DateTime" /> instance. It is equivalent in result to: <c>new DateTime(year, 1, 1).DayOfWeek</c>.
    /// </para>
    /// <para>
    /// The returned value corresponds to the <see cref="DayOfWeek" /> enumeration, where <c>0 = Sunday</c>,
    /// <c>1 = Monday</c>, ..., <c>6 = Saturday</c>.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Calculates the calendar week number of the year for the specified tick value, using the provided week rule and
    /// week start day.
    /// </summary>
    /// <param name="ticks">
    /// The number of ticks representing the target date (100ns intervals since 0001-01-01T00:00:00.000).
    /// </param>
    /// <param name="rule">
    /// A <see cref="CalendarWeekRule" /> value that determines how the first week of the year is defined (FirstDay,
    /// FirstFullWeek, FirstFourDayWeek).
    /// </param>
    /// <param name="firstDayOfWeek">
    /// A <see cref="DayOfWeek" /> value indicating the first day of the week (e.g., Sunday, Monday).
    /// </param>
    /// <returns>
    /// The 1-based week number of the year that contains the specified tick value, based on the specified rule.
    /// </returns>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown if <paramref name="rule" /> is not a valid <see cref="CalendarWeekRule" />.
    /// </exception>
    ...
    /// <summary>
    /// Returns the day of the week considered the start of the week for a given <see cref="WorkingDaysOfWeek" />
    /// pattern.
    /// </summary>
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown if the provided <paramref name="workingWeek" /> is not supported.
    /// </exception>
    ...
    /// </param>
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks at 00:00:00.000 on the first day of the month of
    /// <paramref name="dateTime" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method is equivalent in result to <c>new DateTime(dateTime.Year, dateTime.Month, 1).Ticks</c> but avoids
    /// object allocation and is optimized for internal date calculations.
    /// </para>
    /// </remarks>
    ...
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks at 00:00:00.000 on the first
    /// <paramref name="dayOfWeek" /> occurring in the same month as <paramref name="dateTime" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// The calculation starts at the first day of the month and advances forward to the first matching
    /// <paramref name="dayOfWeek" />.
    /// </para>
    /// <para>
    /// This method performs no validation on <paramref name="dayOfWeek" /> and assumes it is a valid
    /// <see cref="DayOfWeek" /> enumeration value.
    /// </para>
    /// <para>
    /// It avoids allocation by returning a raw tick count and is suitable for high-performance date logic.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// The method starts from the first day of the given month and advances to the first matching
    /// <paramref name="dayOfWeek" /> using modulo arithmetic.
    /// </para>
    /// <para>
    /// The returned tick count corresponds to 00:00:00.000 of the computed date.
    /// </para>
    /// <para>
    /// This method performs no validation. It assumes the specified <paramref name="year" />, <paramref name="month" />
    /// , and <paramref name="dayOfWeek" /> are within valid ranges and form a valid calendar month.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </summary>
    /// <param name="dateTime">
    /// The <see cref="System.DateTime" /> value from which to determine the year and month.
    /// </param>
    /// <returns>
    /// A <see cref="long" /> value representing the number of ticks at 00:00:00.000 on the last day of the month for
    /// the given <paramref name="dateTime" />.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method uses the number of days in the specified month and year (via <c>dateTime.DaysInMonth()</c>) to
    /// identify the last calendar day.
    /// </para>
    /// <para>
    /// It is equivalent in result to <c>new DateTime(year, month, DaysInMonth()).Ticks</c>, but avoids object
    ...
    /// <returns>
    /// The tick count at midnight of the last <paramref name="dayOfWeek" /> in the month containing
    /// <paramref name="ticks" />.
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <param name="dayOfWeek">The <see cref="DayOfWeek" /> to locate.</param>
    /// <returns>
    /// The tick count at midnight of the last <paramref name="dayOfWeek" /> in <paramref name="dateTime" />'s month.
    /// </returns>
    ...
    /// <remarks>
    /// <para>
    /// This method starts from the last day of the given month and steps backward to find the final occurrence of the
    /// specified <paramref name="dayOfWeek" />.
    /// </para>
    /// <para>
    /// The result is a tick value normalized to midnight and avoids object allocation.
    /// </para>
    /// <para>
    /// No validation is performed; the caller is responsible for ensuring all parameters are within valid calendar
    /// ranges.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </summary>
    /// <param name="dateTime">
    /// The <see cref="System.DateTime" /> instance from which to extract the time portion.
    /// </param>
    /// <returns>
    ...
    /// <remarks>
    /// <para>
    /// This method removes the date component from the <paramref name="dateTime" /> and returns only the fractional
    /// portion corresponding to the time of day. It is equivalent to <c>dateTime.TimeOfDay.Ticks</c> but avoids
    /// allocations.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Computes the calendar week number of the year for a given date using either
    /// <see cref="CalendarWeekRule.FirstFullWeek" /> or <see cref="CalendarWeekRule.FirstFourDayWeek" />, depending on
    /// the <paramref name="fullDays" /> parameter.
    /// </summary>
