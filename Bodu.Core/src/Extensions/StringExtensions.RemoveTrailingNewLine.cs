...
public static partial class StringExtensions
{
    ...
    /// <returns>
    /// <paramref name="value" /> with the last <c>"\r\n"</c>, <c>"\n"</c>, or <c>"\r"</c> sequence removed.
    /// Returns the original instance unchanged when <paramref name="value" /> does not end with a line
    /// terminator.
    /// </returns>
    ...
    /// </exception>
    /// <remarks>
    /// Only one trailing line terminator is removed. Repeated trailing newlines are intentionally preserved —
    /// use <see cref="string.TrimEnd(char[])" /> with <c>'\r','\n'</c> when greedy stripping is required.
    /// </remarks>
