...
{
    /// <summary>
    /// Returns a new <see cref="DateOnly"/> representing the previous calendar weekday before the specified <paramref name="date"/>, based on the supplied <paramref name="workingWeek"/> pattern.
    /// </summary>
    ...
    /// <returns>A <see cref="DateOnly"/> value set to the first calendar day before <paramref name="date"/> that is a working day under the specified <paramref name="workingWeek"/> rule.</returns>
    /// <remarks>
    /// <para>The method evaluates each preceding day until it finds one that is selected as a working day by the specified rule. The original <paramref name="date"/> is never returned, even if it already falls on a working day.</para>
    /// </remarks>
    ...
    /// <summary>
    /// Returns a new <see cref="DateOnly"/> representing the previous calendar weekday before the specified <paramref name="date"/>, using the supplied <paramref name="workingWeek"/> pattern and an optional custom <paramref name="provider"/>.
    /// </summary>
    /// <param name="date">The starting date value from which to search backward.</param>
    /// <param name="workingWeek">The <see cref="WorkingDaysOfWeek"/> that determines which days are treated as working days.</param>
    ...
    /// <returns>A <see cref="DateOnly"/> value set to the first calendar day before <paramref name="date"/> that is a working day under the specified <paramref name="workingWeek"/> rule and the logic of <paramref name="provider"/>.</returns>
    /// <remarks>
    /// <para>The method evaluates each preceding day prior to <paramref name="date"/> until it finds one that is a working day, either by the supplied <paramref name="workingWeek"/> pattern or by the custom logic of <paramref name="provider"/>.</para>
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="workingWeek"/> is not a defined value of the <see cref="WorkingDaysOfWeek"/> enumeration.</exception>
