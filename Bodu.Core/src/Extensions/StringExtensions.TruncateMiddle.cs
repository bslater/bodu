...
{
    /// <summary>
    /// Returns <paramref name="value" /> truncated to at most <paramref name="maxLength" /> characters by
    /// keeping the beginning and end intact and inserting <paramref name="separator" /> in the middle.
    /// </summary>
    ...
    /// The marker inserted in place of the omitted middle. Defaults to the ellipsis character <c>"…"</c>.
    /// Must not be <see langword="null" />.
    /// </param>
    /// <returns>
    /// <paramref name="value" /> when its length is at most <paramref name="maxLength" />; otherwise a new
    /// string composed of the leading portion, <paramref name="separator" />, and the trailing portion of
    /// <paramref name="value" /> such that the total length equals <paramref name="maxLength" />. When the
    /// remaining budget cannot be evenly split, the extra character goes to the prefix.
    /// </returns>
