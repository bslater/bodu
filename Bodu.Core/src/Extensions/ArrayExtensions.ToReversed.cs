// ---------------------------------------------------------------------------------------------------------------
// <copyright file="ArrayExtensions.ToReversed.cs" company="Bodu Pty. Ltd.">
...
public static partial class ArrayExtensions
{
    ...
    /// <returns>
    /// A new <typeparamref name="T" />[] containing all elements of <paramref name="source" /> in reverse order.
    /// </returns>
    ...
    /// the SIMD-accelerated copy and reversal. See <see cref="ToReversed{T}(T[], int, int)" /> for full performance and
    /// allocation details.
    /// </remarks>
    ...
    /// to <c>source.Length</c>.
    /// </param>
    /// <param name="count">
    /// The number of elements to reverse. Must be non-negative and, together with <paramref name="index" />, must not
    /// exceed <c>source.Length</c>. A value of zero or one results in a straight copy with no reversal performed.
    /// </param>
    /// <returns>
    /// A new <typeparamref name="T" />[] of the same length as <paramref name="source" />, where elements in [
    ...
    /// Thrown when <paramref name="index" /> is negative or greater than <c>source.Length</c>, or when
    ...
    /// , which uses unsigned comparison to catch both negative values and out-of-bounds values in a single branch.
    /// Validated arguments are then forwarded to <see cref="ReverseCore{T}" />, which performs the SIMD-accelerated
    /// copy and in-place reversal of the nominated slice.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// </param>
    /// <returns>
    /// A new <typeparamref name="T" />[] of the same length as <paramref name="source" />, where elements within
    ...
    /// <c>source.Length</c>. Resolution is performed by <see cref="Range.GetOffsetAndLength" />; the resulting values
    /// are forwarded directly to <see cref="ReverseCore{T}" /> without a second validation pass.
    /// </exception>
    /// <remarks>
    /// Resolves <paramref name="range" /> via <see cref="Range.GetOffsetAndLength" /> and delegates to
    /// <see cref="ReverseCore{T}" />. Prefer <see cref="ToReversed{T}(T[], int, int)" /> when the start and count are
    ...
    /// <summary>
    /// Creates a new single-dimensional array containing all elements of <paramref name="source" /> in reverse order.
    /// </summary>
    /// <param name="source">The array whose elements will be reversed.</param>
    /// <returns>
    /// A new <see cref="Array" /> of the same element type and length as <paramref name="source" />, with all elements
    /// in reverse order.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source" /> is <see langword="null" />.
    ...
    /// <see cref="Array.Reverse(Array, int, int)" />, which is used internally, does not support multidimensional
    /// arrays.
    /// </exception>
    /// <remarks>
    ...
    /// <summary>
    /// Creates a new single-dimensional array that is a copy of <paramref name="source" /> with elements in the
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="Array" /> of the same element type and length as <paramref name="source" />, where elements in
    /// [<paramref name="index" />, <paramref name="index" /> + <paramref name="count" />) appear in reverse order and
    /// all other elements are copied unchanged.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    ...
    /// </exception>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="source" /> is not a single-dimensional array, or when <c>index + count</c> exceeds
    /// <c>source.Length</c>.
    /// </exception>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="index" /> or <paramref name="count" /> is negative or exceeds <c>source.Length</c>.
    /// </exception>
    /// <remarks>
    /// <para>
    ...
    /// to reverse the nominated slice in-place on the copy. These BCL methods are used internally via
    /// <see cref="ReverseArrayCore" />.
    /// </para>
    /// <para>
    /// Bounds validation is delegated to <see cref="ThrowHelper.ThrowIfArrayOffsetOrCountInvalid" />, which also guards
    ...
    /// <summary>
    /// Creates a new single-dimensional array that is a copy of <paramref name="source" /> with elements in the
    /// specified <see cref="Range" /> reversed, leaving all other elements in their original positions.
    /// </summary>
    /// <param name="source">The array to copy and partially reverse.</param>
    /// <param name="range">
    /// The range of elements to reverse. Both start-relative (e.g. <c>1..4</c>) and end-relative (e.g. <c>^4..^1</c>)
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="Array" /> of the same element type and length as <paramref name="source" />, where elements
    /// within <paramref name="range" /> appear in reverse order and all elements outside <paramref name="range" /> are
    /// copied unchanged.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source" /> is <see langword="null" />.
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="range" /> resolves to a start index that is negative or to an end index that exceeds
    /// <c>source.Length</c>. Resolution is performed by <see cref="Range.GetOffsetAndLength" />.
    /// </exception>
    /// <remarks>
    /// Resolves <paramref name="range" /> via <see cref="Range.GetOffsetAndLength" /> and delegates to
    /// <see cref="ReverseArrayCore" />. Prefer <see cref="ToReversed(Array, int, int)" /> when the start and count are
    /// already available as integers to avoid constructing an intermediate <see cref="Range" /> value.
    /// </remarks>
    ...
    /// </summary>
    /// <param name="source">
    /// The single-dimensional array to copy. Must not be <see langword="null" />, must have <see cref="Array.Rank" />
    /// of 1, and must have been validated by the caller.
    /// </param>
    /// <param name="index">The zero-based index of the first element to reverse. Must be pre-validated.</param>
    /// <param name="count">The number of elements to reverse. Must be pre-validated.</param>
    /// <returns>
    /// A new <see cref="Array" /> of the same element type and length as <paramref name="source" />, with the slice [
    /// <paramref name="index" />, <paramref name="index" /> + <paramref name="count" />) reversed.
    /// </returns>
    /// <remarks>
    /// <para>
    /// This method is the non-generic counterpart to <see cref="ReverseCore{T}" />. Because the element type is not
    /// known at compile time, it cannot use <see cref="GC.AllocateUninitializedArray{T}" /> or
    ...
    /// </para>
    /// <para>
    /// This method is <see langword="internal" /> to allow direct testing via assemblies granted
    /// <see cref="System.Runtime.CompilerServices.InternalsVisibleToAttribute" /> access. All public entry points
    ...
    /// </param>
    /// <param name="count">
    /// The number of elements to reverse. Must be a valid, pre-validated count relative to <paramref name="index" />
    /// and <paramref name="source" />.
    /// </param>
    /// <returns>
    /// A new <typeparamref name="T" />[] containing all elements of <paramref name="source" />, with the slice [
    /// <paramref name="index" />, <paramref name="index" /> + <paramref name="count" />) reversed. The caller receives
    /// a <see cref="Span{T}" /> or <typeparamref name="T" />[] depending on which public overload initiated the call,
    /// both backed by this same allocation.
    /// </returns>
    /// <remarks>
    /// <para>
    /// Returns <typeparamref name="T" />[] rather than <see cref="Span{T}" /> so that both the span-based and
    /// array-based public overloads can share this single allocation without a second heap allocation.
    /// <typeparamref name="T" />[] implicitly converts to <see cref="Span{T}" />, so span callers pay no extra cost.
    /// </para>
    /// <para>
    /// This method is <see langword="internal" /> so that it can be exercised directly by unit tests in assemblies
    /// granted access via <see cref="System.Runtime.CompilerServices.InternalsVisibleToAttribute" />, without exposing
    /// it as part of the public API surface. All public entry points validate their arguments before calling this
    /// method; no bounds checking is performed here.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The <c>count &gt; 1</c> guard avoids entering the SIMD dispatch machinery for degenerate slices (empty or single
    /// element), which would be wasted overhead.
    /// </para>
    /// </remarks>
