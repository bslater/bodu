...
{
    /// <summary>
    /// Returns a new <see cref="DateTime"/> representing the next calendar weekday after the specified <paramref name="dateTime"/>, based on the supplied <paramref name="workingWeek"/> pattern.
    /// </summary>
    ...
    /// <returns>An object whose value is set to the first calendar day after <paramref name="dateTime"/> that is a working day under the specified <paramref name="workingWeek"/> rule, with the original time-of-day and <see cref="DateTime.Kind"/> preserved.</returns>
    /// <remarks>
    /// <para>The method evaluates each successive day until it finds one that is selected as a working day by the specified rule. The original <paramref name="dateTime"/> is never returned, even if it already falls on a working day.</para>
    /// </remarks>
    ...
    /// <summary>
    /// Returns a new <see cref="DateTime"/> representing the next calendar weekday after the specified <paramref name="dateTime"/>, using the supplied <paramref name="workingWeek"/> pattern and an optional custom <paramref name="provider"/>.
    /// </summary>
    /// <param name="dateTime">The starting date and time value from which to search forward.</param>
    /// <param name="workingWeek">The <see cref="WorkingDaysOfWeek"/> that determines which days are treated as working days.</param>
    ...
    /// <returns>An object whose value is set to the first calendar day after <paramref name="dateTime"/> that is a working day under the specified <paramref name="workingWeek"/> rule and the logic of <paramref name="provider"/>, with the original time-of-day and <see cref="DateTime.Kind"/> preserved.</returns>
    /// <remarks>
    /// <para>The method evaluates each successive day following <paramref name="dateTime"/> until it finds one that is a working day, either by the supplied <paramref name="workingWeek"/> pattern or by the custom logic of <paramref name="provider"/>.</para>
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="workingWeek"/> is not a defined value of the <see cref="WorkingDaysOfWeek"/> enumeration.</exception>
