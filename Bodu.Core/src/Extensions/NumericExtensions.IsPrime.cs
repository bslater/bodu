// ---------------------------------------------------------------------------------------------------------------
// <copyright file="NumericExtensions.IsPrime.cs" company="PlaceholderCompany">
...
public static partial class NumericExtensions
{
    ...
    /// <returns>
    /// <see langword="true"/> when <paramref name="value"/> is greater than <c>1</c> and has no positive
    /// divisors other than <c>1</c> and itself; otherwise <see langword="false"/>. Returns
    /// <see langword="false"/> for negative values and for <c>0</c> and <c>1</c>.
    /// </returns>
    ...
    /// <returns>
    /// <see langword="true"/> when <paramref name="value"/> is greater than <c>1</c> and has no positive
    /// divisors other than <c>1</c> and itself; otherwise <see langword="false"/>.
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Tests primality of an unsigned integer that is known to be greater than or equal to two,
    /// using a trial-division loop over odd divisors up to <c>sqrt(value)</c>.
    /// </summary>
    ...
    /// <remarks>
    /// Returns <see langword="true"/> immediately for <c>2</c>; rejects all other even values via a
    /// single bit-test; iterates odd candidate divisors from <c>3</c> up to the integer square root
    /// of <paramref name="value"/> inclusive.
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
