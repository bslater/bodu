...
public static partial class SpanExtensions
{
    ...
    /// <returns>
    /// A new <see cref="Span{T}"/> backed by a heap-allocated array containing all
    /// elements of <paramref name="source"/> in reverse order.
    /// </returns>
    /// <remarks>
    /// Delegates to <see cref="Reverse{T}(ReadOnlySpan{T}, int, int)"/> with
    /// <c>start = 0</c> and <c>length = source.Length</c>. See that overload for
    /// full performance and allocation details.
    /// </remarks>
    ...
    /// Must be non-negative and less than or equal to <see cref="ReadOnlySpan{T}.Length"/>.
    /// </param>
    /// <param name="count">
    /// The number of elements to reverse. Must be non-negative and, together with
    /// <paramref name="index"/>, must not exceed <see cref="ReadOnlySpan{T}.Length"/>.
    /// A value of zero or one results in a straight copy with no reversal performed.
    /// </param>
    /// <returns>
    /// A new <see cref="Span{T}"/> of the same length as <paramref name="source"/>, where
    /// elements in the range [<paramref name="index"/>, <paramref name="index"/> +
    ...
    /// <see cref="ReadOnlySpan{T}.Length"/>, or when <paramref name="count"/> is negative
    /// or would extend the range beyond the end of <paramref name="source"/>.
    /// </exception>
    /// <remarks>
    /// <para>
    /// This is the canonical public implementation. The full-span overload
    /// (<see cref="Reverse{T}(ReadOnlySpan{T})"/>) and the <see cref="Range"/>-based
    /// overload (<see cref="Reverse{T}(ReadOnlySpan{T}, Range)"/>) both resolve their
    /// arguments and delegate here after validation.
    /// </para>
    /// <para>
    /// Bounds validation uses unsigned comparison so that a single branch catches both
    /// negative values and values that exceed the span length, matching the pattern used
    /// throughout <see cref="MemoryExtensions"/>.
    /// </para>
    /// <para>
    /// Once validated, execution is forwarded to <see cref="ReverseCore{T}"/>, which
    /// performs the allocation and the SIMD-accelerated copy and reversal.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// <returns>
    /// A new <see cref="Span{T}"/> of the same length as <paramref name="source"/>, where
    /// elements within <paramref name="range"/> appear in reverse order and all elements
    /// outside <paramref name="range"/> are copied unchanged.
    /// </returns>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="range"/> resolves to a start index that is negative or
    /// to an end index that exceeds <see cref="ReadOnlySpan{T}.Length"/>. Resolution is
    /// performed by <see cref="Range.GetOffsetAndLength"/>; the resulting offset and length
    /// are then validated and forwarded to
    /// <see cref="Reverse{T}(ReadOnlySpan{T}, int, int)"/>.
    /// </exception>
    /// <remarks>
    /// <para>
    /// Resolves <paramref name="range"/> to an offset and length via
    /// <see cref="Range.GetOffsetAndLength"/> and delegates to
    /// <see cref="Reverse{T}(ReadOnlySpan{T}, int, int)"/>. Prefer that overload when
    /// the start and length are already available as integers to avoid constructing an
    /// intermediate <see cref="Range"/> value.
    /// </para>
    /// <para>
    /// This overload is most ergonomic when the range is expressed as a C# 8+ index
    /// expression at the call site (e.g. <c>1..4</c> or <c>^3..^1</c>), where the
    /// compiler handles <see cref="Range"/> construction automatically.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// <remarks>
    /// Convenience overload for mutable spans. Implicitly converts <paramref name="source"/>
    /// to <see cref="ReadOnlySpan{T}"/> and delegates to
    /// <see cref="Reverse{T}(ReadOnlySpan{T})"/>.
    /// </remarks>
    ...
    /// Convenience overload for mutable spans. Implicitly converts <paramref name="source"/>
    /// to <see cref="ReadOnlySpan{T}"/> and delegates to
    /// <see cref="Reverse{T}(ReadOnlySpan{T}, Range)"/>.
    /// </remarks>
    ...
    /// </param>
    /// <param name="count">
    /// The number of elements to reverse. Must be a valid, pre-validated length relative
    /// to <paramref name="index"/> and <paramref name="source"/>.
    /// </param>
    /// <returns>
    /// A new <see cref="Span{T}"/> backed by a heap-allocated array containing all elements
    /// of <paramref name="source"/>, with the slice [<paramref name="index"/>,
    ...
    /// </para>
    /// <para>
    /// The <c>length &gt; 1</c> guard avoids entering the SIMD dispatch machinery for
