...
{
    /// <summary>
    /// Returns <paramref name="value" /> guaranteed to end with <paramref name="suffix" />, appending the
    /// suffix only when it is not already present.
    /// </summary>
    ...
    /// The comparison rule used to test whether <paramref name="value" /> already ends with
    /// <paramref name="suffix" />. Defaults to <see cref="StringComparison.Ordinal" />.
    /// </param>
    /// <returns>
    /// <paramref name="value" /> when it already ends with <paramref name="suffix" /> under
    /// <paramref name="comparison" />; otherwise a new string equal to <paramref name="value" /> concatenated
