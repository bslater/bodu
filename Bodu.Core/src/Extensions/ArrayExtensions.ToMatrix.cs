...
public static partial class ArrayExtensions
{
    ...
    /// <param name="source">
    /// The jagged source array. All inner arrays must be non-<see langword="null"/> and have the same
    /// length.
    /// </param>
    /// <param name="transpose">
    /// When <see langword="false"/>, the result has shape <c>[rows, cols]</c> matching
    /// <paramref name="source"/>; when <see langword="true"/>, rows and columns are swapped to produce a
    /// shape of <c>[cols, rows]</c>.
    /// </param>
    /// <returns>
    /// A new rectangular array containing the values from <paramref name="source"/>.
    /// </returns>
    ...
    public static T[,] ToMatrix<T>(this T[][] source, bool transpose)
    {
        ...
        for (int i = 1; i < rows; i++)
        {
            ...
            if (row.Length != cols)
                throw new ArgumentException("All inner arrays must have the same length.", nameof(source));
        }
