...
public static partial class SequenceGenerator
{
    ...
    /// <returns>
    /// A lazily evaluated, finite sequence of <see cref="string" /> values where the first element is <c>"1"</c> and
    /// each subsequent element is the run-length-encoded description of the digits of the previous element.
    /// </returns>
    ...
    /// <c>"1"</c>; if a different seed is required, take the first term as a starting string and apply the run-length
    /// transform manually.
    /// </para>
    /// <para>
    ...
    /// </remarks>
    /// <example>
    /// <code language="csharp"><![CDATA[ foreach (string term in SequenceGenerator.LookAndSay(6)) Console.WriteLine(term); // =>
    /// 1 // => 11 // => 21 // => 1211 // => 111221 // => 312211 ]]></code>
    /// </example>
