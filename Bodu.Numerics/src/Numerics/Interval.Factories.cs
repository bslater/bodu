...
public readonly partial struct Interval<T>
{
    ...
    public static Interval<T> Empty =>
        new Interval<T>(T.Zero, T.Zero, lowerInclusive: false, upperInclusive: false);
    ...
    /// <remarks>
    /// When <paramref name="lower" /> is greater than <paramref name="upper" />, the returned interval is empty.
    /// When the two endpoints are equal, the returned interval is a degenerate single-point interval (see
    /// <see cref="Singleton(T)" />).
    /// </remarks>
    public static Interval<T> Closed(T lower, T upper) =>
        new Interval<T>(lower, upper, lowerInclusive: true, upperInclusive: true);
    ...
    /// <remarks>
    /// When <paramref name="lower" /> is greater than or equal to <paramref name="upper" />, the returned
    /// interval is empty.
    /// </remarks>
    public static Interval<T> Open(T lower, T upper) =>
        new Interval<T>(lower, upper, lowerInclusive: false, upperInclusive: false);

    /// <summary>
    /// Creates a closed-open interval — <c>[lower, upper)</c> — that includes the lower endpoint and excludes
    /// the upper endpoint.
    /// </summary>
    ...
    /// bound and ending before an exclusive upper bound matches the conventions of <c>System.Range</c>, LINQ's
    /// <c>Enumerable.Range</c>, and most iterator protocols.
    /// </remarks>
    public static Interval<T> ClosedOpen(T lower, T upper) =>
        new Interval<T>(lower, upper, lowerInclusive: true, upperInclusive: false);

    /// <summary>
    /// Creates an open-closed interval — <c>(lower, upper]</c> — that excludes the lower endpoint and includes
    /// the upper endpoint.
    /// </summary>
    ...
    public static Interval<T> OpenClosed(T lower, T upper) =>
        new Interval<T>(lower, upper, lowerInclusive: false, upperInclusive: true);
    ...
    /// <returns>A closed-closed interval whose lower and upper endpoints both equal <paramref name="value" />.</returns>
    public static Interval<T> Singleton(T value) =>
        new Interval<T>(value, value, lowerInclusive: true, upperInclusive: true);
}
