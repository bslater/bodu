...
/// loop, index them via <see cref="this[int]" /> (0-based, bounded by <see cref="Count" />), or read <see cref="First" />
/// and <see cref="Second" /> directly.
/// </para>
/// </remarks>
/// <example>
    ...
    where T : INumber<T>
{
    ...
    /// <summary>
    /// Creates a result from a lower and an upper candidate piece, dropping any that are empty and packing the remainder
    /// lowest-first. The caller must supply the pieces in ascending order.
    /// </summary>
    ...
    /// <summary>
    /// Returns a set-notation string representation of the result — the empty-set glyph when empty, otherwise the pieces
    /// joined by the union symbol.
    /// </summary>
