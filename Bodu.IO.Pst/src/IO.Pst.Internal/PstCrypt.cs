// ---------------------------------------------------------------------------------------------------------------
// <copyright file="PstCrypt.cs" company="Bodu Pty. Ltd.">
// Copyright (c) Bodu Pty. Ltd. All rights reserved.
// </copyright>
// ---------------------------------------------------------------------------------------------------------------

namespace Bodu.IO.Pst.Internal;

/// <summary>
/// Decodes the two "compressible encryption" byte-substitution schemes MS-PST applies to external block data: the fixed
/// permutation (§5.1) and the block-keyed cyclic substitution (§5.2).
/// </summary>
/// <remarks>
/// <para>
/// The three 256-byte tables are the published <c>mpbbR</c> / <c>mpbbS</c> / <c>mpbbI</c> data of MS-PST §5.1:
/// <see cref="s_tableR" /> and <see cref="s_tableI" /> are mutually inverse permutations and <see cref="s_tableS" /> is
/// an involution — which makes <see cref="Cyclic" /> self-inverse, so the same routine encodes and decodes.
/// </para>
/// <para>
/// These schemes are obfuscation, not cryptography; they apply only to external (leaf data) blocks, never to pages or
/// internal tree blocks.
/// </para>
/// </remarks>
internal static class PstCrypt
{
    /// <summary>The §5.1 <c>mpbbR</c> substitution table (the encode permutation).</summary>
    private static readonly byte[] s_tableR =
    {
        ,
    };

    /// <summary>The §5.1 <c>mpbbS</c> substitution table (an involution, used by the cyclic scheme).</summary>
    private static readonly byte[] s_tableS =
    {
        ,
    };

    /// <summary>The §5.1 <c>mpbbI</c> substitution table (the inverse of <see cref="s_tableR" />).</summary>
    private static readonly byte[] s_tableI =
    {
        ,
    };

    /// <summary>
    /// Decodes a permuted block in place (§5.1, <c>NDB_CRYPT_PERMUTE</c>).
    /// </summary>
    /// <param name="data">The block data to decode.</param>
    internal static void PermuteDecode(Span<byte> data)
    {
        for (int i = 0; i < data.Length; i++)
            data[i] = s_tableI[data[i]];
    }

    /// <summary>
    /// Applies the cyclic substitution in place (§5.2, <c>NDB_CRYPT_CYCLIC</c>). The routine is its own inverse, so it
    /// both encodes and decodes.
    /// </summary>
    /// <param name="data">The block data to transform.</param>
    /// <param name="key">The low 32 bits of the block identifier.</param>
    internal static void Cyclic(Span<byte> data, uint key)
    {
        var w = (ushort)(key ^ (key >> 16));
        for (int i = 0; i < data.Length; i++)
        {
            byte b = data[i];
            b = (byte)(b + (byte)w);
            b = s_tableR[b];
            b = (byte)(b + (byte)(w >> 8));
            b = s_tableS[b];
            b = (byte)(b - (byte)(w >> 8));
            b = s_tableI[b];
            b = (byte)(b - (byte)w);
            data[i] = b;
            w++;
        }
    }
}
