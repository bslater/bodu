// ---------------------------------------------------------------------------------------------------------------
// <copyright file="RbaExchangeRateProviderTests.LoadFailureLogging.cs" company="Bodu Pty. Ltd.">
...
public partial class RbaExchangeRateProviderTests
{
    ...
    private static (RbaExchangeRateProvider Provider, CapturingLogger Logger) CreateThrowing(Exception exception)
    {
        RbaExchangeRateOptions options = new() { AllowSynchronousNetworkAccess = false, EnableDiskCache = false };
        CapturingLogger logger = new();

        return (new RbaExchangeRateProvider(new ThrowingRbaExchangeRateTableSource(exception), options, logger), logger);
    }
    ...
    {
        (RbaExchangeRateProvider provider, CapturingLogger logger) = CreateThrowing(new HttpRequestException("boom"));
    ...
    {
        (RbaExchangeRateProvider provider, CapturingLogger logger) = CreateThrowing(new ExchangeRateFormatException("boom"));
    ...
    {
        (RbaExchangeRateProvider provider, CapturingLogger logger) = CreateThrowing(new OperationCanceledException());
