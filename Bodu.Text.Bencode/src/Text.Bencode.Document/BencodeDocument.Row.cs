...
public sealed partial class BencodeDocument
{
    ...
    private readonly struct Row
    {
        ...
        /// <param name="integerExceedsInt64">
        /// Whether <paramref name="integer" /> holds the unchecked bit pattern of an unsigned value above
        /// <see cref="long.MaxValue" />.
        /// </param>
        private Row(BencodeValueKind kind, int location, int length, long integer, int numberOfRows, int childCount, int rawLocation, int rawLength, bool integerExceedsInt64)
        {
            Kind = kind;
            Location = location;
            Length = length;
            ...
            RawLength = rawLength;
            IntegerExceedsInt64 = integerExceedsInt64;
        }
        ...
        public static Row Int(long value, int rawLocation, int rawLength) =>
            new(BencodeValueKind.Integer, location: 0, length: 0, integer: value, numberOfRows: 1, childCount: 0, rawLocation: rawLocation, rawLength: rawLength, integerExceedsInt64: false);

        /// <summary>
        /// Creates a row describing an unsigned integer value above <see cref="long.MaxValue" />.
        /// </summary>
        /// <param name="value">The decoded unsigned integer value.</param>
        /// <param name="rawLocation">The absolute byte offset of the encoded <c>i…e</c> token.</param>
        /// <param name="rawLength">The byte length of the encoded token.</param>
        /// <returns>A scalar row whose subtree is a single row.</returns>
        public static Row UInt(ulong value, int rawLocation, int rawLength) =>
            new(BencodeValueKind.Integer, location: 0, length: 0, integer: unchecked((long)value), numberOfRows: 1, childCount: 0, rawLocation: rawLocation, rawLength: rawLength, integerExceedsInt64: true);
        ...
        public static Row Bytes(int location, int length, int rawLocation, int rawLength) =>
            new(BencodeValueKind.ByteString, location: location, length: length, integer: 0, numberOfRows: 1, childCount: 0, rawLocation: rawLocation, rawLength: rawLength, integerExceedsInt64: false);
        ...
        public static Row Container(BencodeValueKind kind, int childCount, int numberOfRows, int rawLocation, int rawLength) =>
            new(kind, location: 0, length: 0, integer: 0, numberOfRows: numberOfRows, childCount: childCount, rawLocation: rawLocation, rawLength: rawLength, integerExceedsInt64: false);
    }
}
